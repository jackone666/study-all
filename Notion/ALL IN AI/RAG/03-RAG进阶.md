---
notion-id: 338a95f7-5835-8144-860b-faaab108bfbe
---
## RAG进阶
**学习目标:**
1. 了解 RAG存在的问题
...
![[1769758879845-64de24f4-8245-4da5-b0bc-9515965a392e.png]]
...
- **基于递归的分块**：重复的利用分块规则不断细分文本块。在langchain中会先通过段落换行符（）进行分割。然后，检查这些块的大小。如果大小不超过一定阈值，则该块被保留。对于大小超过标准的块，使用单换行符（）再次分割。以此类推，不断根据块大小更新更小的分块规则（如空格，句号）。
- **分块大小的选择**：
...
![[1769758879911-b57ecc42-f1e7-49dc-95a8-de8c133ba717.png]]
### 5. 提取上下文与答案无关
...
参考:[https://python.langchain.com/v0.2/docs/how_to/extraction_parse/#using-pydanticoutputparser](https://python.langchain.com/v0.2/docs/how_to/extraction_parse/#using-pydanticoutputparser)
### 7. 答案不完整
**将问题分开提问**
...
![[1769758880019-20deaea5-6f61-4925-b359-a64948f5c23a.png]]
- 网址:[https://python.langchain.com/v0.2/docs/how_to/contextual_compression/#adding-contextual-compression-with-an-llmchainextractor](https://python.langchain.com/v0.2/docs/how_to/contextual_compression/#adding-contextual-compression-with-an-llmchainextractor)

### 9. 答案太具体或太笼统
**幻觉问题 暂无好的方案解决**
### 三. Advanced RAG前沿Paper解读
![[1769758880082-598ffad9-767b-445a-a2cd-1c8b96bf8490.png]]
### 1. 检索过程
...
![[1769758880172-805a2ba7-b847-4a70-8bba-8f4eacde783d.png]]
### 2. Advanced RAG变体
...
### 2.1 T-RAG
![[1769758880239-6070cd29-776a-4885-b024-21c46e65bb51.png]]
- 论文地址：[https://arxiv.org/pdf/2402.07483](https://arxiv.org/pdf/2402.07483)
...
**思路分析:**
9. 用户提出一个查询。
10. 系统进行搜索和检索，从向量数据库中查找相关的文档块，作为上下文使用。
11. 如果查询涉及到组织中的实体，从实体树中提取相关信息并添加到上下文中。
12. **示例**
...
```
13. 系统通过精调的LLM（如Llama-2 7B模型）生成响应。
14. 为了实现精调，使用组织或领域特定的数据集对基础LLM进行微调。
15. 最后，生成的响应返回给用户。

### 2.2 CRAG
可矫正的检索增强生成
![[1769758880320-8ded9cd3-86fd-4781-aaf5-e8d0b14e5c86.png]]
论文地址：[https://arxiv.org/pdf/2401.15884](https://arxiv.org/pdf/2401.15884)
**思路分析:**
16. **Retrieval 阶段：** retriever 先根据 user query 检索到相关的 docs
17. **Knowledge Correction 阶段：** 这里引入了一个轻量级的 Retrieval Evaluator 来评估检索到的 docs 与 user query 之间的相关性分数，并根据这个相关性分数，触发三种 action —— Correct、Incorrect 和 Ambiguous：
...
18. **Generation 阶段：** 将矫正后的 doc 与 user query 进行拼接，交给 LLM Generator 来完成问题的回答。

### 2.3 self-RAG
可自我反思的检索增强生成
![[1769758880383-29813c02-aeab-4ef9-b3fc-aa93dbcb483b.png]]
论文地址：[https://arxiv.org/pdf/2310.11511](https://arxiv.org/pdf/2310.11511)
参考： [https://selfrag.github.io/](https://selfrag.github.io/)
**思路分析:**
19. **Retrieve on demand（按需检索）**
- 首先生成一个初步回答，比如“美国各州的名字来源多种多样。”并根据回答中的需要检索文档。
20. **Generate segment in parallel（并行生成段落）**
- 并行生成（Parallel Generation）：使用初步响应加上按需检索到的文档，系统并行生成多个响应段落，每个段落基于不同的文档和提示。
- 评估生成段落（Evaluate Segments）：系统对每个生成的段落进行评估，判断其相关性和支持度。
21. **Critique outputs and select best segment（评估输出并选择最佳段落）**
...
### 2.4 RAG-Fusion
![[1769758880508-56efad74-6b2f-47e3-b8ba-c3998d3b20d2.png]]
项目地址：[https://github.com/Raudaschl/rag-fusion](https://github.com/Raudaschl/rag-fusion)
参考：[https://mp.weixin.qq.com/s/hxukMEeMzTEOVqd1P1fQLQ](https://mp.weixin.qq.com/s/hxukMEeMzTEOVqd1P1fQLQ)
**思路分析:**
22. 通过LLM将用户的查询翻译成相似但不同的查询。
23. 对原始查询及其生成的类似查询进行向量搜索，实现多个查询生成。
24. 使用RRF结合和精炼所有查询结果。
25. 选择新查询的所有顶部结果，为LLM提供足够的材料，以考虑所有查询和重排的结果列表来创建输出响应。

### 2.5 Rewrite-Retrieve-Read RAG
![[1769758880584-ce8a25ae-fb67-4e7b-8227-3d01b829145a.png]]
论文地址：[https://arxiv.org/pdf/2305.14283](https://arxiv.org/pdf/2305.14283)
**思路分析:**
26. Retrieve-then-read 方法
- 输入（Input）：用户提出查询。
- 检索器（Retriever）：系统从数据库中检索相关文档。
- 文档读取器（Black-box LLM Reader）：使用大型语言模型（LLM）读取检索到的文档并生成响应。
- 输出（Output）：提供给用户的最终响应。
27. Rewrite-retrieve-read 方法
- 输入（Input）：用户提出查询。
...
- 输出（Output）：提供给用户的最终响应。
28. Trainable rewrite-retrieve-read 方法
- 输入（Input）：用户提出查询。
...
![[1769758880657-5e065063-3a30-4ad0-8a62-aae9341d9eeb.png]]