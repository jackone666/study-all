...
Harness Engineering 是 2025 年兴起的一个概念，指的是**系统性地设计、约束和引导 AI Agent 行为的工程实践**，它超越了单纯的 Prompt Engineering，涵盖了从系统架构、工具编排、安全护栏到反馈回路的全链路设计。
...
![image.png](img_70fdcaf350e296d6.png)
...
Agent 自主进化是指 AI Agent 能够在运行过程中**自动改进自身的能力**，而不需要人工重新训练或手动调整。这是从"静态 Agent"到"动态 Agent"的关键跨越。

主要的进化维度包括：

1. **Prompt 自优化**：Agent 根据执行结果自动调整和优化自己的 Prompt

2. **工具学习**：Agent 能够学习使用新工具，甚至自己创建工具

3. **策略进化**：Agent 根据历史经验优化决策策略

4. **知识积累**：Agent 从交互中持续学习和积累领域知识

目前的实现路径：

- **Reflexion**：Agent 对自己的执行结果进行反思，生成改进建议

- **Voyager**：在 Minecraft 中实现的自主学习 Agent，能自动编写和积累技能代码

- **ADAS（Automated Design of Agentic Systems）**：用 Agent 来设计更好的 Agent

- **经验回放（Experience Replay）**：存储成功和失败的经验，用于指导未来决策
...
2. **灾难性遗忘**：Agent 学习新知识时可能忘记旧知识，需要经验回放和知识蒸馏

3. **安全约束**：自主进化的 Agent 可能进化出"走捷径"的策略（Reward Hacking），需要严格的安全护栏

4. **成本控制**：反思和进化过程需要额外的 LLM 调用，成本可能翻倍
...
Agentic Coding 是指使用 AI Agent 来自主完成软件开发任务的新范式。与传统的 AI 辅助编程（如 Copilot 的代码补全）不同，Agentic Coding 中的 Agent 能够**自主规划、编写、测试和调试代码**。

代表性产品和项目：

- **Cursor / Windsurf**：AI 驱动的 IDE，支持 Agent 模式自主编码

- **Devin**：号称"第一个 AI 软件工程师"，能自主完成完整的开发任务

- **OpenHands / SWE-Agent**：开源的软件工程 Agent

- **Claude Code / Gemini CLI**：命令行 Agent 编码工具

Agentic Coding 的典型工作流：
...
4. Agent 运行测试，发现并修复 Bug

5. Agent 提交代码，生成 PR 描述
...
2. **长链路推理（Long-horizon Reasoning）**：一个真实的开发任务可能涉及修改 10+ 个文件、运行多次测试、反复调试。Agent 需要在长链路中保持一致性和目标导向。

3. **环境交互（Environment Interaction）**：Agent 需要与真实的开发环境交互——文件系统、终端、浏览器、数据库等。这要求 Agent 具备：
...
| --- | --- | --- |
| 开发者角色 | 代码编写者 | 需求描述者 + 代码审查者 |
| 核心技能 | 编码能力 | 需求表达 + 架构设计 + Code Review |
| 开发流程 | 写代码 → 测试 → 调试 | 描述意图 → Agent 实现 → Review |
| 质量保证 | 人工测试 + CI | Agent 自测 + 人工 Review |
...
LLM 的 Tool-Use 能力经历了几个关键阶段：

1. **Function Calling（2023）**：OpenAI 率先推出，LLM 能够根据用户意图选择并调用预定义的函数，返回结构化的参数。这是 Agent 使用工具的基础。

2. **Parallel Function Calling（2023）**：支持一次调用多个函数，提升效率。

3. **MCP/Tool Protocol（2024）**：Anthropic 推出 MCP 协议，标准化了工具的定义和调用方式，解决了工具生态碎片化问题。

4. **Computer Use（2024-2025）**：Agent 能够直接操作计算机界面——移动鼠标、点击按钮、输入文字、截屏理解。代表性产品：Claude Computer Use、OpenAI Operator。

5. **Autonomous Tool Creation（2025）**：Agent 不仅能使用工具，还能根据需要自主创建新工具。例如 Voyager 中 Agent 会自动编写 JavaScript 函数作为新技能。
...
Context Engineering 是指**系统性地设计和管理 LLM 输入上下文的工程实践**。它关注的不仅是 Prompt 本身，还包括所有进入模型上下文窗口的信息——系统指令、对话历史、检索结果、工具描述、用户画像等。

与 Prompt Engineering 的区别：

- **Prompt Engineering**：关注"写什么"——如何措辞让模型理解意图

- **Context Engineering**：关注"放什么"——如何选择、组织、压缩上下文信息

核心挑战：

1. **上下文窗口有限**：即使是 128K 的模型，面对复杂任务也可能不够用

2. **信息优先级**：哪些信息最重要？如何排序？

3. **噪声过滤**：无关信息会干扰模型判断（Lost in the Middle 问题）

4. **动态调整**：不同任务阶段需要不同的上下文组合
...
2. **上下文排列（How to arrange）**：研究表明 LLM 存在 **Lost in the Middle** 问题——模型对上下文开头和结尾的信息关注度高，中间部分容易被忽略。因此：
...
1. **Chain of Thought（CoT, 2022）**：通过在 Prompt 中加入"让我们一步步思考"，引导模型展示推理过程，显著提升数学和逻辑推理能力。

2. **Tree of Thought（ToT, 2023）**：将推理过程从线性链扩展为树状结构，模型可以探索多条推理路径，回溯错误分支。

3. **Self-Consistency（2023）**：多次采样不同的推理路径，通过投票选择最一致的答案。

4. **o1/o3 系列（2024-2025）**：OpenAI 推出的推理模型，在推理时使用更多的计算（Test-time Compute），通过内部的"思考链"大幅提升复杂推理能力。

5. **DeepSeek-R1（2025）**：通过纯 RL 训练出强大的推理能力，证明了不依赖人工标注也能获得推理能力。
...
| --- | --- | --- |
| 适用场景 | 简单问答、生成 | 复杂推理、数学、编程 |
| 延迟 | 低（秒级） | 高（十秒到分钟级） |
| 成本 | 低 | 高（Token 消耗大） |
| Agent 策略 | 需要外部 CoT/ToT | 模型内置推理能力 |
...
![image.png](img_639c04f13582052f.png)

**1️⃣ Common Answer**

AI Agent 的记忆系统通常分为几个层次：

1. **工作记忆（Working Memory）**：当前对话的上下文窗口，容量有限（受模型上下文长度限制）

2. **短期记忆（Short-term Memory）**：最近几轮对话的历史，通常存储在内存中

3. **长期记忆（Long-term Memory）**：持久化存储的历史信息，通常使用向量数据库

4. **程序性记忆（Procedural Memory）**：Agent 学到的技能和策略，编码在 Prompt 或代码中
...
2. **程序性记忆（Procedural Memory）**——"会做什么"存储 Agent 学到的技能和策略，如"处理退款投诉的最佳流程"。
...
1. **层级式（Hierarchical）**：一个 Manager Agent 负责任务分配，多个 Worker Agent 执行具体任务。如 AutoGen、CrewAI。

2. **辩论式（Debate）**：多个 Agent 对同一问题给出不同观点，通过辩论达成共识。可以提升回答质量和减少幻觉。

3. **流水线式（Pipeline）**：Agent 按顺序处理任务，每个 Agent 负责一个阶段。如：需求分析 Agent → 编码 Agent → 测试 Agent。

4. **去中心化（Decentralized）**：Agent 之间平等协作，通过消息传递和协商完成任务。
...
2. **Mixture of Agents（MoA）**类似于 Mixture of Experts（MoE），但在 Agent 层面：
...
3. **Agent-as-a-Judge**用 Agent 来评估其他 Agent 的输出质量：

- 比单纯的 LLM-as-Judge 更可靠（Agent 可以使用工具验证事实）

- 可以形成"评审委员会"，多个 Judge Agent 投票

4. **Adversarial Collaboration（对抗式协作）**引入"红队 Agent"来挑战"蓝队 Agent"的输出：
...
2. **一致性问题**：多个 Agent 可能产生矛盾的输出，需要冲突解决机制

3. **调试困难**：多 Agent 系统的行为难以预测和调试

4. **成本爆炸**：N 个 Agent 的成本不是线性增长，而是可能指数增长
...
1. **JSON Mode**：模型保证输出有效的 JSON

2. **Schema 约束**：指定具体的 JSON Schema，模型输出必须符合该 Schema

3. **Grammar-based Decoding**：在解码时用语法规则约束 Token 生成
...
2. **API-level（中等）**：使用 OpenAI 的 response_format 或 Structured Outputs API

  - 优点：API 层面保证格式

  - 缺点：依赖特定模型提供商

3. **Decoding-level（最强）**：在 Token 生成时用语法约束
...
1. **RAG（检索增强生成）**：用外部知识库提供事实依据

2. **Self-Consistency**：多次采样，选择一致性最高的答案

3. **Fact-Checking**：对关键事实声明进行验证

4. **Confidence Scoring**：让模型输出置信度分数，低置信度时拒绝回答
...
2. **Grounding 技术**将 Agent 的输出"锚定"到可验证的数据源：
...
2. Structured Output 约束输出格式（消除工具幻觉）

3. 关键数据必须通过 API 获取，禁止模型自行生成（消除数值幻觉）
...
![image.png](img_2fe1c85ecf3ce46e.png)
...
2. **网络隔离**：限制 Agent 的网络访问范围

3. **资源限制**：CPU、内存、执行时间的上限

4. **权限控制**：禁止危险操作（如 rm -rf、系统调用）
...
| 技术 | 隔离级别 | 启动速度 | 资源开销 | 适用场景 |
| --- | --- | --- | --- | --- |
| Docker | 进程级 | 秒级 | 中 | 通用工具执行 |
| gVisor | 系统调用级 | 秒级 | 低 | 高安全要求 |
| Wasm | 指令级 | 毫秒级 | 极低 | 代码执行 |
| E2B | 云端沙箱 | 秒级 | 按需 | Agentic Coding |
...
2. **快速失败**：超时、超内存立即终止，不等待

3. **可审计**：沙箱内的所有操作都被记录

4. **可恢复**：沙箱销毁后，宿主环境不受影响
...
| --- | --- | --- |
| 技术深度 | 仅提及 Docker 打包、K8s 部署，缺乏细节 | 深入多阶段构建、资源调度、GPU 共享、模型优化 |
| 实践经验 | 无具体场景，泛泛而谈 | 有完整项目实践，包含具体配置和优化效果 |
| 思考维度 | 单一视角，只关注部署本身 | 系统性思考，覆盖构建、编排、调度、优化全链路 |
| 表达方式 | 简单罗列，缺乏结构 | 总分结构，层次清晰，有代码示例 |
| 面试官印象 | 基础了解，可能缺乏实战 | 深度掌握，有丰富架构设计和优化经验 |
...
| 维度 | Common Answer | Impressive Answer |
| --- | --- | --- |
| 架构设计 | 仅提到 Docker 容器隔离 | Agent Controller + Sandbox Pod 动态创建架构 |
| 网络隔离 | 未提及 | NetworkPolicy 单向通信控制 + 禁止出站 |
| 安全加固 | 未提及 | SecurityContext 多维度安全配置 |
| 生命周期 | 容器销毁即清理 | TTL Controller + activeDeadlineSeconds + preStop hook |
| 性能优化 | 未提及 | Pod 预热池 + 镜像预拉取，冷启动 15s→2s |
| 代码实践 | 无 | Java K8s Client + NetworkPolicy YAML 完整示例 |
...
| 维度 | Common Answer | Impressive Answer |
| --- | --- | --- |
| 资源隔离 | 仅分 Namespace | Namespace + ResourceQuota + LimitRange 三层隔离 |
| 调度策略 | 未提及 | PriorityClass + 节点亲和性 + Pod 反亲和性 + 拓扑分布 |
| 优先级管理 | 未提及 | PriorityClass 实现资源紧张时自动抢占 |
| 发布策略 | 未提及 | 独立发布 + 金丝雀发布 + A/B 测试 |
| 模型管理 | 未提及 | 模型服务分离 + 热加载/热切换 |
| 面试官印象 | 了解基本概念 | 有完整的多 Agent 集群管理经验 |
...
| 维度 | Common Answer | Impressive Answer |
| --- | --- | --- |
| 技术深度 | 只提到 HPA CPU 扩缩 | 系统覆盖 HPA 自定义指标、KEDA、VPA、冷启动优化、缩容保护 |
| 实践经验 | 缺乏实际配置和细节 | 提供完整 YAML 配置、具体指标阈值、优化策略 |
| 思考维度 | 单一的水平扩缩思路 | 水平+垂直+事件驱动+冷启动四维系统设计 |
| 表达方式 | 泛泛而谈，缺乏逻辑 | 结构清晰，总分总，有具体数据和案例 |
| 面试官印象 | 基础扎实但缺乏深度 | 系统架构能力强，有实战经验 |
...
| 框架 | 核心技术 | 适用场景 | 优势 | 劣势 |
| --- | --- | --- | --- | --- |
| vLLM | PagedAttention + Continuous Batching | 纯 LLM 推理，追求最高吞吐 | 吞吐最高，内存管理高效，社区活跃 | 仅支持 LLM，不支持多模态 |
| TGI | Flash Attention、量化、Speculative Decoding | HF 生态集成，快速原型 | 生态好，支持多种优化技术，易上手 | 吞吐略低于 vLLM，定制化弱 |
| Triton | 多框架支持（ONNX、TensorRT、TorchScript） | 企业级多模型服务，多模态 | 企业级，支持多模型多框架，可观测性强 | 配置复杂，学习曲线陡峭 |
...
| 维度 | Common Answer | Impressive Answer |
| --- | --- | --- |
| 技术深度 | 只提到 vLLM 和基本概念 | 系统对比三大框架，深入核心优化技术 |
| 实践经验 | 缺乏具体配置和选型逻辑 | 提供完整对比表、决策树、YAML 配置 |
| 思考维度 | 单一技术选型思路 | 从核心技术、适用场景、优化、部署四维分析 |
| 表达方式 | 泛泛而谈，缺乏结构 | 结构清晰，有表格、决策树、配置示例 |
| 面试官印象 | 基础了解，缺乏深度 | 系统架构能力强，有选型经验和实践案例 |
...
| 维度 | Common Answer | Impressive Answer |
| --- | --- | --- |
| 技术深度 | 仅提到滚动更新 | 系统分析 Prompt+模型+代码联合发布、多策略对比、自动回滚 |
| 实践经验 | 缺乏真实场景 | 结合 Argo Rollouts、AnalysisRun、ConfigMap 的具体实践 |
| 思考维度 | 单一关注服务可用性 | 从发布策略、回滚机制、优雅关闭、版本一致性多维度展开 |
| 表达方式 | 简单描述，逻辑松散 | 结构化总分，层次清晰，逐字稿可直接背诵 |
| 面试官印象 | 基础了解，缺乏实战 | 深度思考，有生产环境经验 |
...
3. 等待活跃连接完成或超时（GRACEFUL_SHUTDOWN_TIMEOUT）

4. 超时后强制退出（terminationGracePeriodSeconds 最长 300s）
...
| 维度 | Common Answer | Impressive Answer |
| --- | --- | --- |
| 技术深度 | 仅提到 WebSocket | 系统对比 SSE/WebSocket/gRPC Streaming，根据场景选型 |
| 配置细节 | 泛泛而谈"调整超时参数" | 精确给出 Nginx Ingress 注解配置 |
| 会话保持 | 未提及 | 详细说明 Session Affinity 配置 |
| 连接管理 | 未涉及 | 覆盖连接数限制、背压机制、HPA 联动 |
| 优雅关闭 | 未提及 | 完整说明 preStop hook + SIGTERM + terminationGracePeriodSeconds |
| 面试官印象 | 基础了解 | 有完整的长连接部署实战经验 |
...
| 维度 | Common Answer | Impressive Answer |
| --- | --- | --- |
| 技术深度 | 只提到不同环境用不同配置文件 | 系统阐述 Helm Chart、Kustomize overlay、GitOps 完整方案 |
| 实践经验 | 缺乏实际项目经验 | 结合真实项目实践，分享踩坑和改进过程 |
| 思考维度 | 只考虑配置文件管理 | 考虑 Agent 特殊性（模型、Prompt、工具）、配置安全、模型版本一致性 |
| 表达方式 | 简单罗列，缺乏逻辑 | 总分结构，层次清晰，有具体示例支撑 |
| 面试官印象 | 基础扎实，缺乏生产经验 | 有完整的工程化思维，能解决复杂生产环境问题 |
...
| 维度 | Common Answer | Impressive Answer |
| --- | --- | --- |
| 架构理解 | 简单、用 TS 写的 | 极简架构是刻意的设计选择，避免调试复杂性 |
| Prompt 的作用 | 告诉模型做什么 | System Prompt 承载了所有工程约束，是核心逻辑载体 |
| 工程洞察 | 没有 | 点出"LLM 本身就是最好的编排器"这个核心判断 |

### 容易一起考的题

| 关联题 | 和本题的关系 |
| --- | --- |
| Sub-agent 机制是什么？ | 极简主架构 + Sub-agent 并行是 Claude Code 的完整执行模型 |
| Skill 机制和 RAG 有什么关系？ | Skill 是 Claude Code 的知识复用机制，本质是 RAG + Prompt 注入 |
| Context Engineering 怎么做？ | System Prompt 工程是 Context Engineering 的核心实践 |
...
| 维度 | Common Answer | Impressive Answer |
| --- | --- | --- |
| 无状态设计 | 没提到 | 点出无状态是并行安全的关键设计 |
| 越权防范 | 靠 Prompt 说不能做什么 | 明确区分任务范围、禁止操作、边界处理三层约束 |
| 工程经验 | 没有 | 指出越权根因是 Prompt 不清晰而非模型问题 |
...
![a7c36dfb59df67b61f92028b8e8b2f1d.jpeg](img_ddd4bfcd05cdbd25.jpg)
...
| 维度 | Tool (工具) | Skill (技能) |
| --- | --- | --- |
| **本质** | 原子操作能力，是 Agent 的手脚 | 工作方法或标准流程 |
| **功能** | 解决“能不能做”的问题，如读文件、执行命令、访问网页。 | 解决“应该怎么做”的问题，告诉 Agent 在什么场景下、按什么步骤、调用哪些 Tool。 |
| **关系** | 一个 Skill 可以调用一个或多个 Tool 来完成一个复杂任务。 | Skill 依赖 Tool 来落地执行，没有 Tool，Skill 只是纸上谈兵。 |
...
| 维度 | Common Answer | Impressive Answer |
| --- | --- | --- |
| 协作模式 | 多个 Agent 一起工作 | 点出 Orchestrator + 角色化 Sub-agent 的分层设计 |
| Skill/Tool 区分 | 模糊描述 | 从粒度、语义、复用方式三个维度清晰对比 |
| 工程洞察 | 没有 | 专业化 Agent 比泛化单 Agent 效果更好的原因 |
...
| 维度 | Common Answer | Impressive Answer |
| --- | --- | --- |
| 问题定位 | 能自我进化 | 点出传统 Agent 无法积累经验的根本缺陷 |
| 机制理解 | 保存任务变成 Skill | 完整描述 7 模块闭环，特别提到渐进式加载的工程价值 |
| 数据支撑 | 没有 | 340% 效率提升，有具体数字支撑 |
...
| 挑战 | 解法 |
| --- | --- |
| Skill 质量参差不齐 | 加 Skill 评分机制，执行成功率低的 Skill 自动降权或废弃 |
| 上下文爆炸 | 渐进式加载 + Skill 摘要压缩（只加载 Skill 的关键步骤，不加载全文） |
| Sub-agent 越权 | 工具权限最小化 + Prompt 明确约束 + 高风险操作 HITL |
| 自进化方向跑偏 | 定期人工审查 Skill 库，设置 Skill 有效期，过期需重新验证 |

3️⃣ **Key Differences**

| 维度 | Common Answer | Impressive Answer |
| --- | --- | --- |
| 架构深度 | LLM + 工具 + 记录 | 分层架构：Orchestrator + 角色化 Sub-agent + Skill 引擎 |
| 自进化机制 | 记录任务 | Skill 提炼 + 向量索引 + 渐进式加载 + 评分降权 |
| 工程挑战 | 没有 | 点出 Skill 质量、上下文爆炸、越权、方向跑偏四大挑战及解法 |

### 容易一起考的题

| 关联题 | 和本题的关系 |
| --- | --- |
| SWE-bench 是什么？怎么评估 Coding Agent？ | 自进化 Agent 的效果需要 Benchmark 量化，SWE-bench 是标准评测集 |
| Human-in-the-Loop 怎么设计？ | 高风险操作的 HITL 是 Coding Agent 安全的最后一道防线 |
| Agent 的长链路稳定性怎么保障？ | 超时控制、部分结果保存、断路器是 Coding Agent 工程落地的必选项 |
