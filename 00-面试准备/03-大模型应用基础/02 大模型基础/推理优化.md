...
KV Cache 是 Transformer 模型推理时的优化技术。在自回归生成过程中，每个 token 的生成都需要计算它和之前所有 token 的注意力，这会导致重复计算。KV Cache 通过缓存历史 token 的 Key 和 Value 矩阵，在生成新 token 时只需计算当前 token 的 K、V，然后和缓存的 K、V 一起计算注意力，从而避免重复计算。这可以显著减少推理延迟和显存访问次数。
...
| --- | --- | --- |
| 计算复杂度 | O(n²) | O(n) |
| 显存占用 | 仅模型权重 | 模型权重 + KV Cache |
| 首字延迟 | 相同 | 相同 |
| 后续 token 延迟 | 高 | 低 |
| 适用场景 | 短序列、单次推理 | 长序列、流式生成 |

### 关联题表格

| 问题 | 难度 | 核心考点 |
| --- | --- | --- |
| KV Cache 的显存占用如何估算？ | ⭐⭐ | cache 大小计算公式 |
| PagedAttention 如何优化 KV Cache？ | ⭐⭐⭐ | 分页管理、内存碎片 |
| 多轮对话中 KV Cache 如何管理？ | ⭐⭐ | prompt cache、会话管理 |
| KV Cache 在 batch 推理中的挑战？ | ⭐⭐⭐ | padding、动态长度 |
...
量化是将模型参数从高精度（FP32/FP16）转换为低精度（INT8/INT4）的技术，目的是减少显存占用和加速计算。INT8 量化将 32 位浮点数映射到 8 位整数，精度损失较小，加速效果明显。INT4 量化压缩比更大，但精度损失也更大。GPTQ 是一种后训练量化方法，通过 Hessian 矩阵信息来优化量化过程，减少精度损失。AWQ（Activation-aware Weight Quantization）则考虑激活值的分布，对重要权重进行保护，在保持精度的同时实现更低 bit 量化。
...
| 问题 | 难度 | 核心考点 |
| --- | --- | --- |
| Per-tensor vs Per-channel 量化的区别？ | ⭐⭐ | 量化粒度、精度 |
| 量化感知训练（QAT）和 PTQ 的区别？ | ⭐⭐⭐ | 训练流程、精度 |
| 如何评估量化后的模型精度？ | ⭐⭐ | 评估指标、基准 |
| KV Cache 可以量化吗？如何实现？ | ⭐⭐⭐ | cache 量化、动态量化 |
...
解码策略决定如何从模型输出的概率分布中选择下一个 token。Greedy Search 选择概率最高的 token，简单快速但容易陷入局部最优。Beam Search 维护多个候选序列，每步扩展 top-k 个，最终选择最优，质量更好但计算量大。Top-K 从概率最高的 K 个 token 中随机采样，增加多样性。Top-P（Nucleus Sampling）从累积概率达到 P 的最小 token 集合中采样，动态调整候选数量，平衡质量和多样性。
...
| 维度 | Greedy | Beam Search | Top-K | Top-P |
| --- | --- | --- | --- | --- |
| 确定性 | 完全确定 | 完全确定 | 随机 | 随机 |
| 多样性 | 低 | 低 | 中 | 高 |
| 质量 | 中等 | 高 | 中 | 高 |
| 计算成本 | 低 | 高 | 低 | 低 |
| 适用场景 | 确定性任务 | 质量要求高 | 创意生成 | 创意生成 |

### 关联题表格

| 问题 | 难度 | 核心考点 |
| --- | --- | --- |
| 温度参数如何影响生成结果？ | ⭐⭐ | 概率分布平滑度 |
| 如何避免生成重复内容？ | ⭐⭐ | 重复惩罚、n-gram 惩罚 |
| Beam Search 的 beam size 如何选择？ | ⭐⭐ | 质量与计算成本权衡 |
| Top-P 和 Top-K 可以组合使用吗？ | ⭐⭐ | 组合策略、效果 |
...
FlashAttention 是一种优化的注意力计算算法，通过 tiling 和 recomputation 减少 HBM 访问次数，加速注意力计算并降低显存占用。PagedAttention 将 KV Cache 分页管理，像操作系统虚拟内存一样，解决显存碎片和动态长度问题，支持高效的 batch 推理。Speculative Decoding（推测解码）使用一个小模型快速生成候选序列，然后大模型并行验证这些候选，如果验证通过就接受，否则回退，可以加速 2-3 倍。
...
| 维度 | FlashAttention | PagedAttention | Speculative Decoding |
| --- | --- | --- | --- |
| 优化目标 | 注意力计算 | KV Cache 管理 | 生成速度 |
| 核心技术 | Tiling + Recomputation | 分页管理 | 投机验证 |
| 显存节省 | 50%+ | 减少碎片 | 不直接节省 |
| 加速倍数 | 2-3x | 提升吞吐 | 2-3x |
| 适用场景 | 所有注意力计算 | 长序列、batch 推理 | 自回归生成 |

### 关联题表格

| 问题 | 难度 | 核心考点 |
| --- | --- | --- |
| FlashAttention 如何减少显存占用？ | ⭐⭐⭐ | recomputation、中间结果 |
| PagedAttention 的 block size 如何选择？ | ⭐⭐⭐ | 内存粒度、性能权衡 |
| Speculative Decoding 的加速上限？ | ⭐⭐⭐⭐ | 小模型准确率影响 |
| 连续批处理（Continuous Batching）是什么？ | ⭐⭐⭐ | 动态 batch、资源利用 |
