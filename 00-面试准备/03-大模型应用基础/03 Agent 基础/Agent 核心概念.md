...
AI Agent 就是一种能自主完成任务的 AI，它有四个组件：感知、规划、行动、记忆。感知就是接收输入，规划就是想怎么做，行动就是去执行，记忆就是记住历史。和普通 LLM 的区别嘛，就是它可以用工具，可以做多步操作，不像 LLM 只是问答一下。
...
2. **再说四大组件的工程职责**。感知模块负责把外部信号——用户指令、工具返回结果、甚至图片文件——转换成 Agent 可理解的结构化信息，感知质量直接决定推理准确性；规划模块是 Agent 的大脑，基于 LLM 做任务拆解和策略制定，常见模式有 ReAct、CoT、Plan-and-Solve，它的好坏决定 Agent 面对复杂任务时的可靠性；行动模块把规划转成实际操作，调用 API、执行代码、查询数据库，这里是最容易出错的地方，工具调用失败、格式异常、执行超时都需要兜底处理；记忆模块分短期记忆（当前会话的历史状态，存内存）和长期记忆（跨会话知识，存向量数据库），核心挑战是 Token 窗口有限，怎么做摘要压缩、怎么做相关性检索。

3. **最后说本质区别**。和普通 LLM 的差异体现在三个维度：有状态 vs 无状态——LLM 每次调用都是无状态的，Agent 维护跨步骤的执行状态；单步 vs 多步循环——LLM 一次推理就结束，Agent 是"思考→行动→观察→继续思考"的持续循环；被动 vs 主动——LLM 等人的输入，Agent 能主动发起工具调用和外部操作。实际工程里，Agent 最难搞定的不是 LLM 的推理能力，而是工具调用的可靠性、上下文窗口的管理效率，以及异常恢复机制。

![agent四大组件.png](img_09c36c9677b91a01.png)

**3️⃣ Key Differences**

| 维度 | Common Answer | Impressive Answer |
| --- | --- | --- |
| 技术深度 | 停留在组件定义层面 | 解释各模块工程职责和交互关系，有血有肉 |
| 实践经验 | 无实战视角 | 点出行动模块的失败场景和记忆管理的 Token 挑战 |
| 思考维度 | 笼统说"可以用工具" | 从有状态/多步循环/主动性三个维度精准对比 |
| 给面试官的印象 | 背过定义，但没做过 | 理解落地难点，有实际工程思考 |
...
工具调用失败就重试呗，或者告诉用户失败了。感知模块发现失败就记录一下，规划模块再想想怎么处理，可以换个工具试试。
...
| 维度 | Common Answer | Impressive Answer |
| --- | --- | --- |
| 问题理解 | 把它当一个独立的工程问题 | 从四大组件协作视角来分析 |
| 解决方案 | 只说"重试或告知用户" | 给出重试策略、异常分类、上下文注入、记忆沉淀的完整链路 |
| 工程深度 | 无具体实现思路 | 提到指数退避、结构化 Observation 格式、自我纠错循环 |
| 给面试官的印象 | 了解基本容错思路 | 理解 Agent 四大组件的协作机制，有工程落地经验 |
...
| 关联题 | 和本题的关系 |
| --- | --- |
| ReAct 框架的 T-A-O 循环是如何运作的？ | 规划模块和行动模块的具体工作机制就是 ReAct 循环 |
| Agent 的短期记忆和长期记忆如何实现？ | 记忆模块的深度展开，是四大组件里最复杂的一块 |
| Agent 的感知模块如何处理多模态输入？ | 感知模块的深度展开，决定后续推理质量的第一道关卡 |
...
记忆就分两种，短期的存内存，对话结束就没了，长期的存向量数据库，可以一直留着。检索的话就是用向量相似度查一查。压缩就是把太长的对话截断一下，清理就是定期删掉老的记忆，这样不会占太多空间。
...
3. **检索**。最容易踩的坑是"检索噪音"——向量检索召回的内容看似相关，实际对当前任务没帮助，反而污染上下文。解决方案是用 hybrid retrieval（向量检索 + 关键词检索）提高召回精度，检索后用 LLM 做相关性评分过滤低质量记忆，同时在 System Prompt 里明确告诉模型"如果检索到的记忆与当前任务无关，请忽略它"。

![Agent记忆系统设计.png](img_dc8ea0ccf2bc0d5d.png)

**3️⃣ Key Differences**

| 维度 | Common Answer | Impressive Answer |
| --- | --- | --- |
| 记忆分类 | 只说"短期存内存，长期存向量库" | 区分语义记忆和情景记忆，给出存储方式和检索策略 |
| 短期记忆挑战 | 未提及 Token 限制 | 详细说明滑动窗口、选择性保留、重要性评分三种方案 |
| 记忆压缩 | 只说"清理过期记忆" | 给出四种压缩策略（时间窗口、访问频率、摘要合并、衰减因子） |
| 工程实践 | 无 | 指出检索噪音问题，给出 hybrid retrieval + LLM 评分的解决方案 |
| 给面试官的印象 | 了解概念 | 有完整记忆系统设计经验，踩过坑，知道落地难点 |
...
Token 超限就把之前的对话删掉一些，或者用摘要替代，这样就不会超了。可以设一个最大长度，超过了就截断。
...
| 维度 | Common Answer | Impressive Answer |
| --- | --- | --- |
| 解决方案 | 只说"删掉或截断" | 给出预防、状态外化、重要性过滤三层策略 |
| 工程思维 | 被动应对超限 | 主动设计记忆管理，从源头控制 Token 消耗 |
| 实践深度 | 无具体方案 | 提到状态序列化存外部存储、重要性评分等工程细节 |
| 给面试官的印象 | 知道问题存在 | 有应对长任务 Token 管理的系统性方案 |
...
| 关联题 | 和本题的关系 |
| --- | --- |
| RAG 技术的向量检索是如何工作的？ | 长期记忆的语义检索底层就是 RAG 的向量检索机制 |
| LangChain 的 Memory 模块有哪些类型？ | 框架层面对短期/长期记忆的具体实现，是本题的工程落地 |
| Agent 上下文窗口管理有哪些策略？ | 短期记忆 Token 压缩问题的深度展开 |
...
感知模块就是把各种输入转成 Agent 能懂的格式。图片的话用视觉模型转成文字描述，音频就用 ASR 转成文本。输出应该是结构化的 JSON，方便后面用。感知出错的话就会影响后续的判断，所以要尽量准确。
...
2. **语义解析器层**，把原始输入转为结构化表示：做意图识别判断用户想查询还是创建；做实体抽取提取时间、地点、数量；结合历史记忆补全隐式信息。

3. **输出格式化器层**，生成统一的 JSON 结构，关键是要加置信度标注——比如 `{"value": "北京", "confidence": 0.8}`，让规划模块知道哪些信息可靠、哪些需要二次确认；同时保留原始输入引用，便于追溯。多模态场景下三个工程挑战要注意：模态间信息冲突时要在输出里保留"用户说X、模型识别为Y"的映射，让规划模块自己判断；感知误差会向下传播，所以置信度低于阈值的信息要触发人工确认或多模型交叉验证；多模态感知成本高，用"轻量模型粗分类 + 昂贵模型精感知"两阶段策略控制成本。

**3️⃣ Key Differences**

| 维度 | Common Answer | Impressive Answer |
| --- | --- | --- |
| 感知模块结构 | 只说"接收输入转格式" | 拆解为输入适配器、语义解析器、输出格式化器三层 |
| 多模态处理 | 简单说"图片转描述" | 分析三个工程挑战（模态冲突、误差传播、成本权衡）并给出方案 |
| 置信度标注 | 未提及 | 强调置信度标注对后续规划决策的影响 |
| 给面试官的印象 | 了解感知的作用 | 有完整感知模块设计经验，理解多模态场景的工程难点 |
...
这个问题感知层出错了，那就让模型重新识别一次，或者告诉用户识别可能有误。感知层要尽量准确，出错了影响很大。
...
| 维度 | Common Answer | Impressive Answer |
| --- | --- | --- |
| 处理思路 | 重试或告知用户 | 从感知输出格式设计到规划模块的处理流程，完整链路 |
| 工程细节 | 无 | 提到置信度阈值触发、交叉验证、人工确认节点 |
| 核心原则 | 未提及 | 明确"低置信度感知结果不能直接进入决策链"的工程原则 |
| 给面试官的印象 | 知道感知会出错 | 理解感知误差的传播机制，有系统性的容错设计思路 |
...
| 关联题 | 和本题的关系 |
| --- | --- |
| 多模态大模型（如 GPT-4V）是如何处理图像输入的？ | 感知模块图像适配器底层依赖的技术原理 |
| Prompt Engineering 如何设计让模型输出结构化 JSON？ | 感知模块语义解析器输出格式化的实现手段 |
| Agent 的行动模块如何处理工具调用失败？ | 感知误差传播到行动层时的容错处理，形成完整的容错闭环 |
...
ReAct 就是让模型先想一下（Thought），然后调用工具（Action），工具返回结果后继续想（Observation），一直循环到任务完成。和 CoT 的区别是 CoT 只靠模型自己想，不调用工具。常见问题有格式不对、死循环这些。
...
2. **与纯 CoT 的两个本质差异**。一是信息来源——纯 CoT 所有推理依赖模型参数知识，无法获取实时外部信息；ReAct 可以通过 Action 动态注入最新数据。二是自我纠错能力——纯 CoT 一旦推理走偏，后续只会越偏越远；ReAct 可以通过 Observation 反馈发现错误，在下一个 Thought 里修正。

3. **工程高频踩坑**。格式依赖问题——ReAct 强依赖 LLM 按固定格式输出 Thought/Action/Action Input，模型输出格式一乱解析器就崩，解决方案是做 Output Parser 鲁棒性处理或用支持 structured output 的模型；循环上限问题——没有 max_iterations 限制就会无限循环，Token 和钱都烧没，必须设硬上限同时检测连续相同 Action；Observation 截断问题——工具返回内容很长直接塞进上下文会撑爆 Token，截断太激进又丢失关键信息，这个 trade-off 需要根据场景调参。

![T-A-O与COT的区别.png](img_00904d5b0854379d.png)

**3️⃣ Key Differences**

| 维度 | Common Answer | Impressive Answer |
| --- | --- | --- |
| 技术深度 | 描述 T-A-O 流程，停留在表面 | 解释 Observation 修正认知的核心机制 |
| CoT 对比 | 简单说"CoT 不调用工具" | 从信息来源和自我纠错两个维度精准区分 |
| 实践经验 | 只提到"格式不稳定、死循环"现象 | 给出具体踩坑场景和对应解决思路 |
| 给面试官的印象 | 了解 ReAct 是什么 | 用过 ReAct，踩过坑，知道工程落地难点 |
...
模型一直重复调用说明它没有正确理解工具的返回结果，可以设置一个最大循环次数，超过了就停止，然后告诉用户失败了。
...
| 维度 | Common Answer | Impressive Answer |
| --- | --- | --- |
| 问题根因分析 | 未分析根本原因 | 指出 Observation 解析失败和工具异常是两个主要根因 |
| 解决方案 | 只说设最大次数 | 给出硬上限 + 相同 Action 检测 + Observation 校验 + Prompt 约束三层方案 |
| 工程深度 | 浅层 | 提到 Observation 标准化、Prompt 层面的行为约束等工程细节 |
| 给面试官的印象 | 知道设循环限制 | 理解 ReAct 死循环的机制，有系统性的防护设计 |
...
| 关联题 | 和本题的关系 |
| --- | --- |
| Chain-of-Thought 的工作原理是什么？ | ReAct 的 Thought 阶段内部就在做 CoT 推理，两者是包含关系 |
| Plan-and-Execute 和 ReAct 有什么设计差异？ | ReAct 是在线规划，Plan-and-Execute 是离线规划，是两种规划哲学的对比 |
| LangChain 的 AgentExecutor 是怎么实现 ReAct 的？ | 框架层面对 T-A-O 循环的具体实现，是本题的工程落地 |
...
Plan-and-Execute 是先生成一个完整计划再执行，ReAct 是每步动态决定下一步。Plan-and-Execute 适合复杂任务，因为提前规划方向不偏；ReAct 适合简单快速的场景。LangGraph 里可以定义不同节点来实现这两种模式。
...
2. **场景选型**。ReAct 适合短任务、需要快速迭代、工具反馈对后续决策影响大的场景，比如实时查汇率、查天气；Plan-and-Execute 适合长任务、步骤多、需要全局把控的场景，比如复杂数据分析任务——先理解需求、拆解子任务、分别执行、汇总报告。

3. **LangGraph 实现思路**。通常用三个核心节点：`planner` 节点调用 LLM 把用户任务分解成有序 step 列表存到 state 的 plan 字段；`executor` 节点每次从 plan 里取当前步骤调用对应工具执行，把结果存入 past_steps；`replan` 节点（可选但推荐）在某步骤失败或结果偏离预期时触发重新规划。这种设计每个节点职责单一，通过 LangSmith 可以清楚看到 Planner 生成了什么计划、每个 Executor 步骤的输入输出，定位问题非常方便。

![PAE和ReAct的区别.png](img_e10ed4dba0215f73.png)

**3️⃣ Key Differences**

| 维度 | Common Answer | Impressive Answer |
| --- | --- | --- |
| 技术深度 | 描述两者的表面差异 | 从在线规划 vs 离线规划的维度解释设计哲学差异 |
| 场景选型 | 简单说"复杂任务用 Plan-and-Execute" | 给出具体适用场景示例，说明各自的 trade-off |
| 实践经验 | 未提及具体实现 | 给出 LangGraph 三节点实现思路，并提到 replan 机制 |
| 给面试官的印象 | 了解两种模式的存在 | 清楚两种模式的 trade-off，能根据场景做架构选型 |
...
任务分解就是把大任务拆成小任务。粒度太粗的话每个子任务还是太复杂，Agent 可能完成不好。粒度太细的话子任务太多，调用次数多，成本高延迟也高。需要根据任务复杂度来决定粒度。
...
| 维度 | Common Answer | Impressive Answer |
| --- | --- | --- |
| 分析深度 | 只说了太粗太细的表面问题 | 从三个维度分析，有具体示例 |
| 判断标准 | 模糊（"根据复杂度"） | 有可量化的判断标准（代码实现） |
| 动态调整 | 无 | 自顶向下和自底向上两种策略 |
| 面试官印象 | 知道粒度问题 | 能设计粒度控制机制 |
...
发现数据有问题就让模型重新生成一个新的计划，从头开始执行。或者直接告诉用户数据有问题，让用户来处理。
...
| 维度 | Common Answer | Impressive Answer |
| --- | --- | --- |
| 解决方案 | 从头规划或告知用户 | 设计 replan 增量调整机制，利用 past_steps |
| 工程细节 | 无 | 说明 replan LLM 的输入、增量调整原则、replan 次数上限 |
| 架构理解 | 未体现对 Plan-and-Execute 架构的理解 | 展示对 state 流转、三节点协作的深度理解 |
| 给面试官的印象 | 知道需要重新规划 | 理解 Plan-and-Execute 的动态调整机制，有 LangGraph 实战经验 |
...
| 关联题 | 和本题的关系 |
| --- | --- |
| ReAct 框架的 T-A-O 循环是如何运作的？ | ReAct 是 Plan-and-Execute 的对立设计，理解两者才能做架构选型 |
| LangGraph 的 StateGraph 是如何工作的？ | Plan-and-Execute 在 LangGraph 里的实现依赖 StateGraph 的节点和状态管理 |
| 多 Agent 系统中如何做任务分配？ | Plan-and-Execute 的 Planner 可以扩展为多 Agent 协调器，是架构演进方向 |
...
Chain-of-Thought 就是让模型分步推理。Few-shot 是给几个示例，Zero-shot 是加一句"一步步想"。CoT 能提升准确率是因为把复杂问题拆开了，模型不容易出错。跟 ReAct 的关系就是 ReAct 用了 CoT 的思路。
...
2. **CoT 提升准确率的底层原理**。有两个关键机制：第一，引导注意力——Transformer 面对多步问题直接输出答案容易漏掉中间步骤，CoT 把每个推理步骤固化成 token，后续生成可以显式引用，有效避免跳步；第二，分解复杂度——把需要同时处理多个变量的问题拆成若干简单子步骤，每步的推理难度大幅下降。

3. **与 ReAct 的关系**。CoT 是"在脑子里想清楚"，ReAct 是"想完了还要出去查一下再继续想"。ReAct 的每个 Thought 阶段本质上就是在做 CoT 推理，两者不是替代关系而是组合关系——用 CoT 提升单步 Thought 的质量，用 ReAct 框架串联多步工具调用。

![Chain-of-Thought （CoT）的原理与触发方式.jpeg](img_d9a1657f928c9637.jpg)

**3️⃣ Key Differences**

| 维度 | Common Answer | Impressive Answer |
| --- | --- | --- |
| 技术深度 | 停留在现象层面，只说"分步推理" | 从注意力机制和复杂度分解角度解释底层原理 |
| 实践经验 | 未提及两种方式各自的优缺点 | 分析了设计成本、格式可控性和适用场景 |
| CoT 与 ReAct 的关系 | 简单说"ReAct 用了 CoT" | 清晰阐述互补关系，说明工程中如何组合使用 |
| 给面试官的印象 | 知道 CoT 是什么 | 理解工作机制，能在工程中合理选用和搭配 |
...
如果任务比较复杂就用 Few-shot，简单的就用 Zero-shot，因为 Zero-shot 不用写示例，更方便。
...
| --- | --- | --- |
| 触发方式 | 加 "Let's think step by step" | 提供带推理过程的示例 |
| 准备成本 | 低（无需准备示例） | 高（需要人工标注示例） |
| 效果 | 一般，依赖模型能力 | 更好，示例引导推理格式 |
| 适用场景 | 快速原型、通用任务 | 特定格式输出、专业领域 |
...
| 维度 | Common Answer | Impressive Answer |
| --- | --- | --- |
| 选型标准 | 只说"复杂用 Few-shot" | 从任务稳定性和设计成本两个维度给出判断框架 |
| 工程思维 | 没有提到维护成本 | 提出 prompt 模板库的工程实践，降低长期维护成本 |
| 给面试官的印象 | 知道两种方式的存在 | 有工程落地经验，能做合理的技术选型 |
...
| 关联题 | 和本题的关系 |
| --- | --- |
| ReAct 框架是如何工作的？ | ReAct 的 Thought 阶段本质上是在做 CoT 推理，两者是组合关系 |
| Self-Consistency 是什么？ | Self-Consistency 建立在 CoT 之上，对多条 CoT 路径做投票来提升稳定性 |
| Prompt Engineering 有哪些核心技巧？ | CoT 是 Prompt Engineering 里最重要的推理增强技巧之一 |
...
Self-Consistency 就是多次采样然后投票，能减少模型出错的概率。它是基于 CoT 的，CoT 让模型推理，Self-Consistency 用投票让结果更准。采样越多越准，但成本也越高，要根据情况决定采样几次。
...
2. **与 CoT 的关系**。两者是正交关系，不是谁替代谁：CoT 负责"让单次推理质量更好"，Self-Consistency 负责"从多次推理中选出最好的"。工程标准配置是组合使用——用 CoT 提示生成高质量推理路径，再用 Self-Consistency 对多条路径投票。

3. **工程降本策略**。Self-Consistency 的成本是单次 CoT 的 N 倍，生产环境里有四种降本手段：**自适应采样**——先采 3-5 次，答案一致直接返回，不一致再加采；**大小模型搭配**——小模型做多次采样投票选候选，大模型只做最终一次高质量推理；**缓存复用**——对相似问题复用已有推理路径；**分层采样**——只在关键决策节点用 Self-Consistency，而不是每步都用。

**3️⃣ Key Differences**

| 维度 | Common Answer | Impressive Answer |
| --- | --- | --- |
| 有效性解释 | 只说"减少出错" | 从降低随机性和概率分布近似两个维度解释底层机制 |
| 与 CoT 的关系 | 说"基于 CoT" | 明确两者正交关系，解释如何组合使用 |
| 成本控制 | 只说"采样越多成本越高" | 给出四种具体降本策略（自适应、大小模型、缓存、分层） |
| 给面试官的印象 | 了解基本概念 | 有工程实践经验，能在准确率和成本之间做合理权衡 |
...
对数学题多采样几次，比如 5 次，然后投票选答案，这样比单次准确。成本方面就少采样几次，比如从 10 次降到 5 次。
...
| 维度 | Common Answer | Impressive Answer |
| --- | --- | --- |
| 成本策略 | 只是减少采样次数 | 自适应采样，按置信度动态决定是否追加 |
| 准确率保障 | 依赖采样数量 | 结合答案可验证性做额外校验 |
| 给面试官的印象 | 了解基本原理 | 有工程落地思维，能设计动态策略 |
...
| 关联题 | 和本题的关系 |
| --- | --- |
| Chain-of-Thought 是什么？ | Self-Consistency 建立在 CoT 之上，是对 CoT 路径做集成的方法 |
| LLM 推理中如何控制 Token 成本？ | Self-Consistency 的核心工程挑战就是多次采样带来的 Token 成本 |
| 如何评估 Agent 的推理可靠性？ | Self-Consistency 的投票一致率本身就是一个可靠性指标 |
...
CoD 就是把大任务拆成小任务，CoT 是推理的时候一步步想。CoD 先拆好再执行，CoT 是边推理边解决。对于复杂任务，可以先拆成大的模块，再把每个模块拆成更小的任务，一层一层拆下去直到可以执行。
...
2. **四层分解框架**。对于超复杂任务，我会按四层来拆：第一层目标层按业务边界拆模块，模块间尽量解耦；第二层流程层对每个模块按状态转换拆业务流程；第三层任务层把每个流程拆成原子任务，每个任务可以独立执行和验证；第四层操作层对每个任务拆成具体的工具调用操作。每层的分解依据不同，不能混用。

3. **子任务依赖关系的处理**。依赖关系有四种：串行（B 等 A 完成）、并行（A 和 B 同时跑）、条件（B 的执行取决于 A 的结果）、循环（重复执行直到满足条件）。工程上用 DAG 表示任务和依赖，拓扑排序确定执行顺序，并行任务用 asyncio 并发执行，循环依赖设置最大重试次数防止无限循环。

![COD.png](img_eca1c6fe85733c85.png)

**3️⃣ Key Differences**

| 维度 | Common Answer | Impressive Answer |
| --- | --- | --- |
| CoD vs CoT | 只说"一个拆任务一个推理" | 从目标、粒度、依赖关系、输出五个维度系统对比 |
| 分解策略 | 只说"先拆大再拆小" | 给出四层框架（目标→流程→任务→操作），说明每层的分解依据 |
| 依赖关系 | 未提及 | 列举四种依赖类型，并说明对应工程处理方式（DAG、拓扑排序、asyncio） |
| 给面试官的印象 | 了解基本概念 | 有完整的任务分解方法论，理解复杂任务的工程化处理 |
...
先把这个任务拆成几个部分，比如收集竞品信息、分析对比、写报告，然后依次执行每个步骤。
...
| 维度 | Common Answer | Impressive Answer |
| --- | --- | --- |
| 分解粒度 | 只拆到大模块 | 拆到具体工具调用层级 |
| 依赖分析 | 全部串行执行 | 识别并行机会，显著提升效率 |
| 容错设计 | 未提及失败处理 | 有任务降级策略，保证整体任务可完成 |
| 给面试官的印象 | 会做任务拆解 | 有工程化落地经验，考虑了效率和容错 |
...
| 关联题 | 和本题的关系 |
| --- | --- |
| Chain-of-Thought 和 CoD 的区别？ | CoT 是推理步骤的显式化，CoD 是任务结构的层次化拆解 |
| Agent 如何处理并发子任务？ | CoD 拆解出的无依赖子任务，天然适合并发执行 |
| LangGraph 如何做任务编排？ | LangGraph 的 DAG 图结构本质上就是 CoD 执行计划的工程实现 |
...
![HIL设计.jpeg](img_d28d4dab33bf973c.jpg)

**3️⃣ Key Differences**

| 维度 | Common Answer | Impressive Answer |
| --- | --- | --- |
| 自主性分级 | 笼统说"几个等级" | 给出 L0-L4 五级框架，每级有名称、描述和典型场景 |
| 介入触发标准 | 只说"高风险操作" | 从不可逆程度和置信度两个维度建立判断标准 |
| interrupt() 实现 | 只说"调用后暂停" | 说明 checkpointer 持久化的重要性，以及对产品可用性的影响 |
| 给面试官的印象 | 知道 HITL 是什么 | 能独立设计 HITL 方案，理解状态持久化对产品体验的价值 |
...
| 维度 | Common Answer | Impressive Answer |
| --- | --- | --- |
| 审批设计 | 单次确认 | 两阶段审批，大规模操作加管理员二次确认 |
| 容错设计 | 未提及 | 分批发送、断点续发，防止重复发送 |
| 持久化 | 未提及 | PostgresSaver 保证审批状态跨重启不丢失 |
| 给面试官的印象 | 知道要加确认 | 有完整的高风险操作防护设计经验 |
...
| 关联题 | 和本题的关系 |
| --- | --- |
| LangGraph 的 checkpointer 是什么？ | interrupt() 依赖 checkpointer 做状态持久化，是 HITL 实现的基础 |
| Agent 的任务终止条件如何设计？ | HITL 是主动暂停，终止条件是被动退出，两者共同构成 Agent 的执行控制体系 |
| 如何评估 Agent 的风险等级？ | 风险评估是决定是否触发 HITL 的前置步骤 |
...
![Agent任务终止条件.png](img_ad9475134708c64a.png)

**3️⃣ Key Differences**

| 维度 | Common Answer | Impressive Answer |
| --- | --- | --- |
| 技术深度 | 只提两种机制（终止标记 + max_iterations） | 给出四层防护体系，覆盖所有边界场景 |
| 循环检测 | 笼统说"检测重复操作" | 给出基于 Action 哈希的具体判定逻辑，以及触发后的处理方式 |
| Token 预算 | 未提及 | 说明检测时机和压缩续命策略 |
| 用户体验 | 超限直接报错 | 超限后返回部分完成结果，保证体验 |
| 给面试官的印象 | 知道要限制迭代次数 | 有体系化的防护设计思维，考虑了用户体验和可观测性 |
...
| 维度 | Common Answer | Impressive Answer |
| --- | --- | --- |
| 排查方法 | 看日志，凭感觉 | 用 LangSmith trace 定位具体触发原因，分类处理 |
| 根因分析 | 只说"卡住了" | 区分循环卡壳 vs 任务复杂度超出预期，对应不同优化方向 |
| 优化策略 | 降低 max_iterations 或优化 prompt | 进度感知 prompt + CoD 任务拆解，从根本上解决问题 |
| 给面试官的印象 | 有基本的排查意识 | 有体系化的生产问题排查经验 |
...
| 关联题 | 和本题的关系 |
| --- | --- |
| Agent 的 Token 成本如何控制？ | Token 预算兜底是终止保护的第四层，同时也是成本控制的核心手段 |
| LangSmith 如何用于 Agent 调试？ | 终止原因的记录和 trace 分析，依赖 LangSmith 的可观测性能力 |
| Human-in-the-Loop 是什么？ | HITL 是主动暂停等待人工决策，终止保护是被动防御，两者共同构成 Agent 执行控制体系 |
...
| 维度 | Common Answer | Impressive Answer |
| --- | --- | --- |
| 错误分类 | 只说"临时和永久" | 给出完整分类，包含典型场景和是否可重试 |
| 重试策略 | 只说"重试几次" | 指数退避 + 最大重试次数 + 熔断保护三层策略 |
| 永久性错误 | 只说"检查参数" | 错误反馈给LLM、任务降级、用户确认三种策略 |
| 可观测性 | 只说"记日志" | 日志、监控、分析形成完整闭环 |
| 给面试官的印象 | 知道要处理错误 | 有完整的错误处理体系，理解不同错误类型的应对策略 |
...
| 维度 | Common Answer | Impressive Answer |
| --- | --- | --- |
| 思路 | 重试或直接失败 | 重试 + 熔断 + 降级 + LLM重规划三层隔离 |
| 用户感知 | 整个任务失败 | 部分完成并说明原因，体验更好 |
| 系统健壮性 | 被动响应单次失败 | 主动防止失败扩散，有完整的容错体系 |
...
| 关联题 | 和本题的关系 |
| --- | --- |
| 什么是熔断器模式（Circuit Breaker）？ | 临时性错误处理的核心保护机制，防止级联故障 |
| Agent 如何实现任务降级与优雅降级？ | 永久性错误处理的核心策略，保证任务最大程度完成 |
| LangGraph 里如何捕获节点异常并恢复？ | 工具调用失败处理在 LangGraph 框架中的具体实现方式 |
...
| 维度 | Common Answer | Impressive Answer |
| --- | --- | --- |
| 并发价值 | 只说"提升效率" | 给出量化对比，说明性能提升幅度和原理 |
| 依赖建模 | 只说"有无依赖" | DAG + 四种依赖类型，拓扑排序确定执行顺序 |
| LangGraph实现 | 只说"定义并发节点" | 解释边的定义如何隐式表达DAG并发语义 |
| 资源控制 | 只说"注意资源" | Semaphore 限流的具体实现方案 |
| 给面试官的印象 | 了解并发概念 | 有完整的并发设计经验，理解资源限制和任务调度的工程实现 |
...
| 维度 | Common Answer | Impressive Answer |
| --- | --- | --- |
| 并发实现 | 知道用 gather | 加超时控制 + return_exceptions 异常隔离 |
| 失败处理 | 未提及 | 部分结果降级输出，不影响整体任务完成 |
| 用户体验 | 一个失败全失败 | 最大程度呈现有效结果 |
...
| 关联题 | 和本题的关系 |
| --- | --- |
| Python asyncio 的 gather 和 wait 有什么区别？ | 并发任务提交的底层机制，gather 适合结果聚合，wait 适合灵活控制 |
| LangGraph 的 StateGraph 如何表达任务依赖？ | LangGraph 并发执行的具体实现方式，边的定义即依赖关系 |
| 什么是 Semaphore，如何用它限制并发度？ | 并发资源控制的核心工具，防止过度并发耗尽资源 |
...
![Reflection框架1.jpeg](img_d8152448d2e50de9.jpg)

**3️⃣ Key Differences**

| 维度 | Common Answer | Impressive Answer |
| --- | --- | --- |
| 技术深度 | 描述"总结失败然后重试"的表面流程 | 拆解三层架构（Actor/Evaluator/Self-Reflection），说明各层职责 |
| 核心对比 | 未提两种反馈方式的区别 | 深入对比 Language Feedback vs Scalar Reward 的信息密度差异 |
| 与 ReAct 的关系 | 说"两者不同"，停留表面 | 明确指出互补关系：执行引擎 vs 跨轮优化器 |
| 工程视角 | 无 | 指出反思质量依赖、context 膨胀、无效循环三个实际风险 |
| 给面试官的印象 | 看过论文摘要 | 理解框架设计意图，有工程落地经验 |
...
| 维度 | Common Answer | Impressive Answer |
| --- | --- | --- |
| 反思内容 | 直接传 stderr | 结构化四字段反思，信息密度更高 |
| 反思注入 | 无明确策略 | 召回最近 3 条相关反思注入 System Prompt |
| 兜底机制 | 无 | 设最大重试次数，超限上报人工介入 |
| 给面试官的印象 | 知道"重试" | 理解 Reflexion 落地的工程细节 |
...
| 关联题 | 和本题的关系 |
| --- | --- |
| ReAct 框架的工作原理是什么？ | Reflexion 以 ReAct 为执行引擎，理解 ReAct 是理解 Reflexion 的前提 |
| Agent 的记忆管理有哪些类型？ | Reflexion 的反思存储依赖 Episodic Memory，和记忆管理章节直接相关 |
| 如何控制 Agent 的 Token 消耗？ | 反思累积会膨胀 context，Token 管理是 Reflexion 工程落地的核心约束 |
...
3. **最后是不同任务类型的反思格式差异**。代码生成任务需要细粒度反思，格式包含 `error_type`、`error_location`、`fix_suggestion`、`relevant_snippet`——错误往往很具体。数学推理任务需要高层级反思，格式包含 `error_stage`、`error_type`、`correct_direction`——错误往往在思路层面。在工程上，我们会为不同任务类型设计专门的 Reflection Schema，并在 System Prompt 里要求 LLM 按格式生成，方便后续做结构化检索。

![Reflexion Memory 的设计与维护.png](img_a05e22755fc5ccc9.png)

**3️⃣ Key Differences**

| 维度 | Common Answer | Impressive Answer |
| --- | --- | --- |
| 数据结构 | 只说"存反思内容" | 给出四字段结构，并解释每个字段的工程价值 |
| 管理策略 | 只说"按时间排序" | 详细说明去重、优先级排序、过期清理三个策略，给出优先级公式 |
| 任务差异 | 泛泛说"代码更详细" | 对比两种任务的具体 Reflection Schema 字段差异 |
| 工程量化 | 无 | 给出 k=3-5、阈值 0.85、TTL 7 天等经验值 |
| 给面试官的印象 | 知道要存反思 | 有完整的 Memory 管理体系，理解不同任务的差异化需求 |
...
| 维度 | Common Answer | Impressive Answer |
| --- | --- | --- |
| 冲突检测 | 无 | 通过语义相似度 + fix_suggestion 对比主动检测 |
| 仲裁逻辑 | 只看新近性 | 以 success_flag 为主权重，新近性为辅 |
| 合并策略 | 无 | 提出元反思合并，生成更抽象的高层级反思 |
| 给面试官的印象 | 没想到这个问题 | 有完整的冲突处理机制，体现工程成熟度 |
...
| 关联题 | 和本题的关系 |
| --- | --- |
| Agent 的长期记忆和短期记忆如何设计？ | Reflection Memory 是长期记忆的一种形式，设计原则相通 |
| RAG 的检索策略有哪些？ | Reflection 召回用语义检索，和 RAG 的向量检索原理相同 |
| 如何评估 Agent 的反思质量？ | success_flag 和 attempt_count 就是反思质量的评估指标，和 Agent 评估章节直接相关 |
...
这几种框架都是让模型自我反思，但方式不太一样。Reflexion 是在多次尝试之间做反思，每次失败后记录下来下次用。Self-Refine 是在一次生成后让模型自己批评自己，然后修改。Critique-and-Revise 是让模型先生成批评，再根据批评来修改。Reflexion 适合需要多次尝试的任务，Self-Refine 适合单次就能完成的任务。
...
2. **其次是适用场景的工程判断**。Reflexion 适合"失败原因明显但一次修正不够"的任务，如代码生成（改了一个错误又暴露另一个）、数学推理（思路错了必须重来）；Self-Refine 适合"初稿可用、需要润色"的任务，如文本摘要、翻译、代码注释；Critique-and-Revise 适合"需要多轮细致打磨"的任务，如长文档写作、代码重构。

3. **最后是成本-效果的量化对比**。Reflexion 的 LLM 调用次数是轮数 × 单轮调用数，Memory 需求高；Self-Refine 是 2-3 次，Memory 需求低；Critique-and-Revise 是 2N 次（N=轮数），Memory 需求中等。工程上我们通常混合使用：Reflexion 处理"框架性错误"（思路方向错了），再用 Self-Refine 做"细节润色"，这样能在控制成本的同时保证大方向正确。

![Reflexion 与其他反思框架的对比.png](img_ff4fdb757144cbda.png)

**3️⃣ Key Differences**

| 维度 | Common Answer | Impressive Answer |
| --- | --- | --- |
| 框架区分 | 只说"方式不太一样" | 从跨轮迭代、持久化、成本三个维度精确区分三种框架 |
| 适用场景 | 泛泛区分"多轮 vs 单轮" | 给出代码生成、文本润色、长文档写作等具体场景对应关系 |
| 成本分析 | 只说"Reflexion 更贵" | 给出 LLM 调用次数、Memory 需求的量化对比 |
| 工程实践 | 无 | 提出混合使用策略（Reflexion 处理框架错误 + Self-Refine 做细节优化） |
| 给面试官的印象 | 了解名字 | 理解框架设计哲学，能根据任务特点做工程决策 |
...
可以用 Self-Refine，让模型写完之后再检查一遍，看看结构有没有问题，然后修改。或者用 Reflexion 让它多试几次。
...
| 维度 | Common Answer | Impressive Answer |
| --- | --- | --- |
| 框架选型 | 提了两个框架但没有判断依据 | 明确说明为什么用 Critique-and-Revise 而不是 Reflexion |
| 批评设计 | 无 | 提出三维度结构化批评，避免模糊的"再检查一遍" |
| 成本意识 | 无 | 明确限定 4-6 次 LLM 调用，控制延迟 |
| 给面试官的印象 | 知道框架名字 | 能根据场景特点做精准的框架选型和链路设计 |
...
| 关联题 | 和本题的关系 |
| --- | --- |
| Chain-of-Thought 和 ReAct 有什么区别？ | Reflexion 以 ReAct 为执行基础，理解 CoT/ReAct 是理解反思框架的前提 |
| 如何评估 Agent 的输出质量？ | 三种框架的 Evaluator 设计都涉及质量评估，评估指标的设计是共同挑战 |
| Token 成本优化有哪些策略？ | 三种框架的核心权衡都是反思质量 vs LLM 调用成本，Token 优化直接影响框架选型 |
...
![TOC1.png](img_a863a0da574e59ea.png)

**3️⃣ Key Differences**

| 维度 | Common Answer | Impressive Answer |
| --- | --- | --- |
| 技术深度 | 描述"探索多条路径"的概念 | 拆解思维生成、状态评估、搜索策略三个组件，说明各自的设计选项 |
| 评估机制 | 简单提"打分"，无细节 | 区分独立打分（Value）和投票排序（Vote）两种方式 |
| 成本对比 | 说"ToT 更贵"，无量化 | 给出 10-50 倍的量化估计，并提出三种工程降本策略 |
| 搜索策略 | 未提 BFS/DFS 区别 | 分析两种搜索算法各自适用的场景 |
| 给面试官的印象 | 理解基本概念 | 懂得在成本和效果之间做工程权衡，有实战思考 |
...
| 维度 | Common Answer | Impressive Answer |
| --- | --- | --- |
| 降本思路 | 只说减少分支或深度 | 分结构控制、剪枝、模型路由三层策略 |
| 量化意识 | 无 | 给出复杂度估算对比（625 vs 27） |
| 混合策略 | 未提 | 说明 ToT 与 CoT/ReAct 混用的工程价值 |
| 给面试官的印象 | 知道要省钱 | 有系统性降本思维，理解 ToT 在工程中的适用边界 |
...
| 关联题 | 和本题的关系 |
| --- | --- |
| ReAct 框架的工作原理是什么？ | ToT 和 ReAct 是两种主流推理范式，常放在一起对比成本与效果 |
| Chain of Thought 为什么有效？ | CoT 是 ToT 的基础，理解 CoT 才能理解 ToT 在单路径基础上的扩展 |
| LLM 推理中的 Self-Consistency 是什么？ | Self-Consistency 也是多路径采样后投票，和 ToT 的状态评估机制有重叠，常一起考 |
...
| 维度 | Common Answer | Impressive Answer |
| --- | --- | --- |
| 剪枝时机 | 只说"可以剪" | 区分层级剪枝和路径剪枝，说明各自适合的搜索算法 |
| 剪枝准则 | 只说"按评分" | 细分绝对阈值、Top-k、多样性感知三种，并说明自适应阈值和动态 k |
| 保留率设计 | 无 | 根据任务特点给出收敛型 vs 发散型的不同保留率建议 |
| 搜索配合 | 只说"BFS 和 DFS 都能用" | 分析两种算法下剪枝的不同表现（全局最优 vs 局部最优） |
| 给面试官的印象 | 知道剪枝概念 | 有完整的剪策略设计思维，理解剪枝在工程落地中的细节 |
...
| 维度 | Common Answer | Impressive Answer |
| --- | --- | --- |
| 应对思路 | 被动提高阈值 | 主动通过多样性保护和策略分区来预防 |
| 系统性 | 无 | 有"两头松"的位置感知策略 |
| 持续优化 | 无 | 提到历史数据回溯校准评分的闭环机制 |
| 给面试官的印象 | 知道调参 | 理解误剪本质，有预防和迭代优化的工程思维 |
...
| 关联题 | 和本题的关系 |
| --- | --- |
| BFS 和 DFS 各自的适用场景是什么？ | 剪枝策略必须和搜索算法配合，BFS 做层级剪枝，DFS 做路径剪枝 |
| ToT 的状态评估机制怎么设计？ | 剪枝的判断依据来自状态评估器，两个知识点强耦合 |
| 如何控制 Agent 的 token 消耗？ | 剪枝是 ToT 场景下控制 token 消耗的核心手段，常放在成本优化话题里一起考 |
...
![COT、TOT、GOT选型.png](img_5868858bdf5f66e5.png)

**3️⃣ Key Differences**

| 维度 | Common Answer | Impressive Answer |
| --- | --- | --- |
| 方法区分 | 只说"结构不同" | 从结构特征、优势、局限三个维度精确对比三种方法 |
| 复杂度量化 | 无 | 给出具体的复杂度公式和工程估计值 |
| 选择标准 | 只说"看任务复杂度" | 给出典型场景的选择决策依据，并结合预算给出建议 |
| 成本-效果 | 只说"GoT 更贵" | 给出成本范围和效果排序，有明确的取舍逻辑 |
| 给面试官的印象 | 了解名字 | 有系统性对比框架，能在成本和效果之间做工程权衡 |
...
| 维度 | Common Answer | Impressive Answer |
| --- | --- | --- |
| 选型逻辑 | 简单等于"效果好就用" | 从任务特征（收敛型 vs 发散型）出发，推导出最合适的方法 |
| 参数意识 | 无 | 给出具体的 b、d 参数建议和调用次数估算 |
| 决策框架 | 线性判断 | 给出两步决策树：先看是否需要迭代，再看预算 |
| 给面试官的印象 | 知道方法名字 | 能把技术选型和业务约束结合起来做决策 |
...
| 关联题 | 和本题的关系 |
| --- | --- |
| Monte Carlo Tree Search（MCTS）是什么？ | AoT 常用 MCTS 作为搜索算法，理解 MCTS 才能理解 AoT 的高效性来源 |
| Self-Consistency 和 ToT 有什么区别？ | 两者都做多次采样，区别是 Self-Consistency 靠多数投票，ToT 靠结构化搜索和评估 |
| LangGraph 如何实现多步推理？ | LangGraph 的图状态机可以用来实现 GoT/ToT 的结构，工程实现话题常一起考 |
...
![如何让agent选对工具.png](img_d0203691977a8cd5.png)

**3️⃣ Key Differences**

| 维度 | Common Answer | Impressive Answer |
| --- | --- | --- |
| 工具描述 | 说"写清楚"，无标准 | 给出四要素框架，并强调反适用场景和同类工具区别 |
| 工具路由 | 只说"分组"，无架构细节 | 详细说明两级路由设计，并提出向量检索的进阶方案 |
| Few-shot 使用 | 只说"加例子" | 指出核心价值是建立选择标准，说明正例+反例的搭配必要性 |
| 工程量化 | 无 | 给出每级 5-8 个候选的经验值 |
| 给面试官的印象 | 知道方向但缺细节 | 有完整的工程方法论，踩过坑，知道坑在哪里 |
...
| 维度 | Common Answer | Impressive Answer |
| --- | --- | --- |
| 定位方法 | 泛泛说"看日志" | 说明从成功率指标切入，找具体规律 |
| 修复策略 | 只说改描述或加例子 | 分描述问题、路由问题、边界模糊三类，对症处理 |
| 验证机制 | 无 | 强调 A/B 测试和 badcase 数据集的闭环建设 |
| 给面试官的印象 | 知道要排查 | 有系统性的问题定位和持续优化意识 |

#### 4、容易一起考的题

| 关联题 | 和本题的关系 |
| --- | --- |
| Function Calling 和 Tool Use 有什么区别？ | 工具选择建立在 Function Calling 机制上，理解底层机制才能优化选择准确率 |
| Prompt Engineering 中 description 怎么写才有效？ | 工具描述本质是 Prompt 工程，description 的写法直接影响工具选择准确率 |
| RAG 的向量检索原理是什么？ | 工具路由的进阶方案用到向量检索，两个知识点在工具数量大时会结合考 |
...
| 维度 | Common Answer | Impressive Answer |
| --- | --- | --- |
| 错误分类 | 泛泛提"网络、参数错误" | 区分可重试和不可重试两类，给出具体错误类型列表 |
| 重试策略 | 只说"重试 3 次" | 详细说明指数退避、随机抖动、熔断机制，给出时间参数 |
| 错误传递 | 只说"告诉 LLM" | 对比简单传递和结构化传递，给出 JSON 结构和 Prompt 设计思路 |
| 降级方案 | 只说"告知失败" | 给出功能降级、服务降级、体验降级三层方案 |
| 给面试官的印象 | 知道要处理错误 | 有完整的错误处理体系，理解降级和熔断的重要性 |
...
| 维度 | Common Answer | Impressive Answer |
| --- | --- | --- |
| 限流处理 | 固定等待几秒 | 读 Retry-After，按服务端指定时间退避 |
| 并发协调 | 只考虑单个请求 | 触发熔断，协调所有并发 Agent 暂停调用 |
| 任务调度 | 原地阻塞等待 | 放入延迟队列，不阻塞其他任务 |
| 根因治理 | 无 | 提出主动速率控制，从源头避免触发限流 |
| 给面试官的印象 | 知道要等待 | 有批量任务和分布式场景下的系统性限流处理思维 |
...
| 关联题 | 和本题的关系 |
| --- | --- |
| 熔断器（Circuit Breaker）模式是什么？ | 工具调用的熔断机制就是 Circuit Breaker 在 Agent 场景下的应用 |
| 指数退避和随机抖动为什么一起用？ | 重试策略的核心设计，理解抖动能解释为什么不能只用指数退避 |
| LangChain 的工具调用链怎么处理异常？ | 框架层面的错误处理实现，和工程设计原则结合考察 |
...
| 维度 | Common Answer | Impressive Answer |
| --- | --- | --- |
| 工具池设计 | 只说"放在公共地方" | 集中注册、版本管理、灰度发布，有完整架构 |
| 权限控制 | 只说"配不同工具列表" | 引入 RBAC，角色定义 + 工具权限绑定 + 最小权限原则 |
| 沙箱隔离 | 几乎没提 | 执行沙箱 + 资源配额 + 调用链追踪三层隔离 |
| 审计监控 | 只说"记录日志" | 实时监控关键指标，阈值告警，滥用检测 |
| 给面试官的印象 | 知道要隔离权限 | 有完整的多 Agent 工具管理架构，理解安全隔离的多个层面 |
...
| 维度 | Common Answer | Impressive Answer |
| --- | --- | --- |
| 防护逻辑 | "检查权限不让调" | 说清楚角色 → 工具允许列表 → 拒绝的完整链路 |
| 安全思维 | 只想到拦截 | 拦截 + 审计 + 告警 + 事后溯源的完整闭环 |
| 价值定位 | 说不清楚意义 | 明确指出权限最小化是注入防护的最后兜底 |
...
| 关联题 | 和本题的关系 |
| --- | --- |
| Prompt Injection 如何防护？ | 工具权限隔离是 Prompt Injection 防护的最后一道兜底，两者是配套设计 |
| LangGraph 中如何给节点分配工具？ | LangGraph 节点绑定工具列表，是工具池按角色分发的框架层实现 |
| 分布式 Agent 系统的状态一致性怎么保证？ | 多 Agent 共享工具时也涉及资源竞争和一致性，和 FSM+EDA 的一致性设计同根同源 |
...
| 维度 | Common Answer | Impressive Answer |
| --- | --- | --- |
| 原理理解 | 描述攻击现象 | 指出根本原因：LLM 无法从语义上区分指令和数据 |
| 注入类型 | 区分了直接和间接，但无深度 | 明确间接注入危险性更高，指出攻击面扩展到工具输出 |
| 防护深度 | 列举几个措施，无架构 | 给出四层纵深防护体系，每层有具体技术细节 |
| 权限最小化 | 提到但无细节 | 给出具体的权限设计原则，并说明是最后一道兜底 |
| 给面试官的印象 | 了解概念 | 有安全工程思维，理解攻击面和纵深防护的设计逻辑 |
...
| 维度 | Common Answer | Impressive Answer |
| --- | --- | --- |
| 防护层次 | 单一过滤 | 隔离 + 预处理 + 权限兜底三层 |
| 技术细节 | 无具体方案 | 标签隔离、长度限制、HTML 清洗、正则检测 |
| 兜底思维 | 没有 | 即使注入成功，权限限制保证危害有限 |
...
| 关联题 | 和本题的关系 |
| --- | --- |
| 多 Agent 工具权限隔离怎么设计？ | 权限最小化是 Prompt Injection 防护的最后兜底，两者是配套设计 |
| RAG 中检索内容如何处理？ | 检索结果也是间接注入的潜在载体，需要同样的隔离和清洗设计 |
| Agent 的审计日志怎么设计？ | 注入攻击的事后溯源依赖完善的审计日志体系 |
...
2. **其次是四种工程缓解策略**。
  ...
  2. 第二，结构化输出约束：用 JSON Schema 或 Pydantic 强制规定输出格式，比如 `{"answer": "...", "confidence": "high/medium/low", "source": "..."}` ，confidence 字段让模型显式暴露不确定性，工程上用 Instructor 库或 OpenAI Structured Outputs 实现。

  3. 第三，Self-Check 二次验证：把问题和初始回答再发给模型，让它判断"回答是否有明显事实错误"，进阶做法是多模型交叉验证，或对同一问题多次采样检测答案一致性——一致性低说明模型不确定。

  4. 第四，不确定性显式化：在 System Prompt 里告知"不确定时请明确说出来，不要猜测"，并在 Few-shot 示例里示范如何表达不确定性。实际项目里，这几种策略组合使用，目标是把幻觉率控制在业务可接受的范围内。

![幻觉避免1.jpeg](img_09c7134393591bc9.jpg)

**3️⃣ Key Differences**

| 维度 | Common Answer | Impressive Answer |
| --- | --- | --- |
| 幻觉分类 | 只说"不准确信息"，无分类 | 区分事实幻觉和指令幻觉，解释各自的产生机制 |
| RAG 策略 | 说"基于文档回答" | 说明 Grounding 的 Prompt 设计，并指出检索质量是新风险 |
| 结构化输出 | 提到格式约束 | 说明 confidence 字段的价值，给出具体工具 |
| Self-Check | "让模型检查自己" | 提出多模型交叉验证和采样一致性检测的进阶方案 |
| 给面试官的印象 | 知道几个方法 | 有系统性缓解体系，理解各策略适用场景和局限性 |
...
| 维度 | Common Answer | Impressive Answer |
| --- | --- | --- |
| 解决思路 | 只想到 Prompt 优化 | RAG + 结构化输出 + 参数校验三层组合 |
| 技术深度 | 无具体方案 | 向量库检索、JSON Schema、confidence 字段、静态校验 |
| 工程闭环 | 没有 | 低置信度走人工确认，校验失败重试，有完整 fallback |
...
| 关联题 | 和本题的关系 |
| --- | --- |
| RAG 的检索质量如何评估？ | RAG Grounding 缓解幻觉的效果直接取决于检索质量 |
| Structured Outputs / Instructor 怎么用？ | 结构化输出约束是缓解幻觉的核心工程手段 |
| Agent 的评估指标有哪些？ | 幻觉率是 Agent 质量评估的核心指标之一 |
...
FSM 就是定义了一组状态和转移规则，流程清晰可预测，但灵活性不够，状态多了会很复杂。LangGraph 是基于图结构的，更灵活，支持更复杂的 Agent 场景，比纯粹的 FSM 更适合实际使用。
...
2. **其次是 FSM 的核心局限：状态爆炸**。随着业务复杂度增加，状态数量会指数级增长。比如一个订单处理 Agent，如果要处理"正常下单"、"库存不足"、"支付失败"、"部分退款"、"全额退款+重新下单"等场景的各种组合，状态和转移的数量会变得难以管理，维护成本极高，还容易遗漏边界状态。

3. **最后是 LangGraph 对 FSM 的两个关键扩展**。第一个是条件边（Conditional Edges）：允许根据运行时的状态值动态决定下一个节点，让"确定转移"变成"条件转移"，灵活性大幅提升，同时保留图结构的可视化和可审计优势。第二个是循环支持（Cycles）：传统 DAG 不允许循环，但 LangGraph 允许循环边，让"执行→评估→重试"这类迭代模式变得自然，这正是 ReAct、Reflexion 这类需要多轮迭代的 Agent 所需要的。工程决策原则：流程相对固定的场景用确定边，需要 LLM 动态决定路由的场景用条件边，同时配合日志和监控弥补可预测性的不足。

![基于有限状态机（FSM）的 Agent 流程控制设计.png](img_39d461e148737842.png)

**3️⃣ Key Differences**

| 维度 | Common Answer | Impressive Answer |
| --- | --- | --- |
| FSM 核心价值 | 说"流程清晰"，无深度 | 明确提出可预测性、可测试性、可审计性三个工程价值 |
| 状态爆炸 | 提到"状态多了复杂"，无细节 | 用订单处理场景说明状态爆炸的具体来源 |
| FSM vs 图结构 | 说"LangGraph 更灵活" | 从 deterministic vs dynamic、测试难度、适用场景三维对比 |
| LangGraph 扩展 | 说"基于图结构" | 精确指出条件边和循环支持两个关键扩展点及其意义 |
| 给面试官的印象 | 了解工具 | 理解架构设计的权衡逻辑，能根据场景选择合适的控制策略 |
...
这个流程比较固定，可以用 FSM 或者 LangGraph 都行。定义好每个步骤的状态，然后按顺序执行，出了错就处理异常。
...
| 维度 | Common Answer | Impressive Answer |
| --- | --- | --- |
| 架构选型 | "FSM 或 LangGraph 都行" | 明确说出混用确定边 + 条件边，并解释原因 |
| 异常处理 | "出了错就处理异常" | 具体设计了重试上限、兜底状态、人工接管的状态机 |
| 可测试性 | 没提 | 强调条件写在代码里而非 LLM 决策，每条路径可测试 |
...
| 关联题 | 和本题的关系 |
| --- | --- |
| LangGraph 的 StateGraph 怎么使用？ | LangGraph 是 FSM 思想的工程实现，StateGraph 就是 FSM 的图结构表达 |
| ReAct 和 Plan-and-Solve 的区别？ | ReAct 的"执行→观察→推理"循环需要 FSM 的循环边支持 |
| Agent 流程的可测试性怎么保证？ | FSM 的可测试性优势正是来自确定性的状态转移，是选择 FSM 的核心理由 |
...
3. **最后是异常状态的测试策略**。
  ...
  2. 失败：模拟各种失败场景（网络错误、参数错误、权限错误），验证是否转移到正确的错误处理状态。

  3. 资源耗尽：模拟内存、连接数、API 调用次数耗尽，验证是否能优雅降级。覆盖率目标：状态覆盖 100%、转移覆盖 100%、路径覆盖关键业务路径 80%+、异常场景覆盖 90%+。

**3️⃣ Key Differences**

| 维度 | Common Answer | Impressive Answer |
| --- | --- | --- |
| 测试设计 | 只说"为每个转移写测试" | 引入状态转移矩阵，区分单步测试和路径测试 |
| 边界状态 | 只说"单独写测试用例" | 细分初始/终止/自循环/历史状态依赖四种类型 |
| 异常测试 | 只说"测试超时和失败" | 详细说明超时、失败、资源耗尽三种异常的测试方法 |
| 覆盖率指标 | 只说"状态转移的覆盖比例" | 给出状态、转移、路径、异常四个维度的覆盖率目标 |
| 给面试官的印象 | 知道要测试 FSM | 有完整的 FSM 测试方法论，理解测试的多个维度和工程目标 |
...
| 维度 | Common Answer | Impressive Answer |
| --- | --- | --- |
| 退出条件 | 只说"最大重试次数" | 重试次数 + 绝对超时两个条件组合，解释各自的必要性 |
| 测试覆盖 | "模拟一直发无效输入" | 三个专项用例，覆盖退出、超时、中途恢复三条路径 |
| 设计严密性 | 没有 | 指出只有单一退出条件的风险 |
...
| 关联题 | 和本题的关系 |
| --- | --- |
| FSM 与 EDA 融合设计中的幂等性怎么保证？ | FSM 的幂等性测试和状态转移测试是同一体系的延伸 |
| LangGraph 的 StateGraph 如何做单元测试？ | LangGraph 是 FSM 的实现框架，其测试方法和 FSM 测试方法论直接对应 |
| CI/CD 流水线里如何做 Agent 的回归测试？ | FSM 测试应完全自动化，每次代码变更后在 CI 中运行状态转移回归测试 |
...
FSM 可以和事件驱动架构结合，每个状态转移都发送一个事件到消息队列，其他服务监听这些事件来执行操作。状态需要持久化存储，防止服务重启后状态丢失。分布式环境下要保证一致性，可以用分布式锁或事务，还要考虑幂等性避免重复执行。
...
2. **其次是状态持久化与恢复**。存储格式：用 Protobuf 持久化状态（体积小、解析快），用 JSON 持久化历史事件（易读，用于审计）。存储时机：每次状态转移后立即持久化保证可靠性，但频繁持久化影响性能，工程上用批量+异步持久化优化，风险是崩溃后丢失最近状态，需结合事件溯源恢复。恢复策略：服务重启后，加载持久化状态 → 检查是否过期 → 重放未处理事件 → 恢复转移逻辑。

3. **其次是分布式一致性保证**。乐观锁（CAS）：检查状态版本号，版本匹配才允许转移，适合冲突少的场景，性能好。悲观锁：转移前获取锁，冲突严重时使用，性能差，有死锁风险。分布式事务：状态转移涉及多个服务时，用 Saga 模式（最终一致性、性能好、不阻塞）而非 2PC（强一致性但阻塞、性能差），Agent 场景更适合 Saga。

4. **最后是三层幂等性设计**。事件去重：给每个事件分配唯一 ID，处理前检查是否已处理，已处理直接返回，不重复执行，事件 ID 持久化存储防止重启后重复。状态幂等：如果状态已经是目标状态，转移直接返回成功而非报错。补偿机制：状态转移涉及多个操作时，用 Saga 模式的补偿事务处理回滚，保证整体一致性。

![FSM 与 EDA 的融合设计图谱.png](img_134aa1ae33ef38b6.png)

**3️⃣ Key Differences**

| 维度 | Common Answer | Impressive Answer |
| --- | --- | --- |
| 融合模式 | 只说"结合使用" | 区分两种模式，并说明双向融合的工程价值 |
| 状态持久化 | 只说"存储状态" | 对比 Protobuf vs JSON、批量+异步优化、事件溯源恢复 |
| 一致性保证 | 只说"分布式锁或事务" | 详细说明乐观锁、悲观锁、Saga vs 2PC 的选型逻辑 |
| 幂等性 | 只说"避免重复执行" | 给出事件去重、状态幂等、补偿机制三个层面的设计 |
| 给面试官的印象 | 了解基本概念 | 有完整的分布式 FSM 设计能力，理解一致性、可靠性、幂等性的工程实现 |
...
重复投递会导致工具调用执行两次，可能产生重复操作。可以在处理事件前判断一下是否已经处理过，比如用一个 Set 记录处理过的事件 ID。
...
| 维度 | Common Answer | Impressive Answer |
| --- | --- | --- |
| 幂等层次 | 只有事件 ID 去重一层 | 事件层去重 + 状态层版本控制两层，互为兜底 |
| 技术细节 | "用 Set 记录" | Redis/DB 存储 + TTL，CAS 版本号检查 |
| 故障思维 | 没有 | 考虑事件去重存储本身故障的 fallback 场景 |
...
| 关联题 | 和本题的关系 |
| --- | --- |
| Saga 模式 vs 2PC 怎么选？ | FSM+EDA 的分布式事务设计核心就是 Saga vs 2PC 的权衡 |
| 消息队列的 at-least-once vs exactly-once 语义？ | EDA 融合 FSM 时，消息语义决定了幂等性设计的复杂度 |
| 多 Agent 工具共享的状态一致性怎么保证？ | 多 Agent 共享工具和 FSM+EDA 的一致性问题是同一类分布式一致性问题 |
