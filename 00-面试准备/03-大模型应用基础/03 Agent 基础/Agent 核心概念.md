...
重复调用同一工具通常有两个根本原因：Observation 的内容没有被模型正确理解，或者工具每次都返回了异常导致模型认为任务没完成。处理要从三层来设计：
**第一，设置硬上限 `max_iterations`**，并且做连续相同 Action 检测——如果连续 N 次 Action 相同，直接中断并返回当前最优结果；
**第二，在 Observation 注入前做校验**，如果工具返回格式异常，要先做标准化处理再注入，避免 LLM 因为解析不了 Observation 而陷入迷茫；
**第三，在 Prompt 里明确告知模型"如果工具已返回足够信息，请直接给出最终答案**，不要再次调用相同工具"，从 Prompt 层面约束循环行为。
这三层结合能有效防止 ReAct Agent 的死循环。
