...
- 任务分解、委派、汇聚、容错
- Orchestrator 核心实现
- Orchestrator-Worker 模式是多 Agent 系统的核心架构，关键在于任务分解、委派、汇聚、容错四个环节
...
1. **接口标准化**：Agent Card 清晰描述 Agent 的输入/输出格式，便于 Orchestrator 组合
1. **独立部署**：每个 Agent 独立部署，通过 A2A 协议解耦，可以独立扩缩容
...
1. **skills 要细粒度**：每个 skill 描述一个具体能力，而非笼统的"数据处理"
1. **examples 要典型**：提供 2-3 个典型输入示例，帮助 Orchestrator 理解适用场景
1. **capabilities 要准确**：`streaming` 和 `pushNotifications` 影响 Orchestrator 的调用策略
...
| --- | --- |
| A2A 架构、Task 生命周期、多 Agent 协作实践 | A2A 的 Task 是一个完整的状态机，理解状态转换是设计健壮多 Agent 系统的关键：；[*] --> Submitted : 任务提交；Submitted --> Working : Worker 接受。 |
| A2A 协议是什么？它解决了什么问题？ | Agent 间协作需要大量定制化代码；无法复用其他团队/厂商的 Agent 能力；任务委派、状态同步、结果汇聚没有标准格式；Agent Card：每个 Agent 发布自己的能力声明（类似名片）；Task：标准化的任务对象，有完整的生命周期管理。 |
| A2A 协议的 Task 生命周期是什么？如何处理长时间运行的任务？ | A2A 的 Task 是一个完整的状态机，理解状态转换是设计健壮多 Agent 系统的关键：；[*] --> Submitted : 任务提交；Submitted --> Working : Worker 接受。 |
| A2A 中 Orchestrator Agent 和 Worker Agent 如何协作？ | Orchestrator-Worker 模式是多 Agent 系统的核心架构，关键在于任务分解、委派、汇聚、容错四个环节：；participant Orchestrator；participant WorkerA as Worker A（搜索）。 |
| A2A 和 MCP 在实际项目中如何配合使用？ | A2A 层：Agent 间的任务委派、状态同步、结果传递；MCP 层：每个 Agent 内部的工具调用（搜索、数据库、代码执行等）；A2A 和 MCP 是互补关系，分别解决不同层次的问题，在实际系统中通常同时使用：；User["用户请求"] --> Orchestrator["Orchestrator Agent"]。 |
| Agent Card 是什么？如何设计一个好的 Agent Card？ | Agent Card 是 A2A 协议中的能力声明文档，是 Agent 服务发现的核心机制，通常以 JSON 格式暴露在固定路径（/.well-known/agent.json）：；"name": "DataAnalysisAgent"；"description": "专业的数据分析 Agent，擅长 SQL 查询、统计分析和数据可视化"。 |
| 多 Agent 系统中如何处理 Agent 之间的循环依赖和死锁？ | 多 Agent 系统的循环依赖是一个系统性问题，需要预防 + 检测 + 恢复三层防御：；原则：Agent 调用关系必须是 DAG（有向无环图）；Orchestrator → Worker A → Worker B ✓（单向）。 |
