...
![image.png](img_804532b56e6a45f6.png)
...
| 示例 | `search(query)` | 搜索工具 | "市场调研技能"（搜索+分析+报告） |
...
Agent Skill 是 Agent 能用的能力，比如查天气、发邮件。和普通函数的区别是 Skill 有描述，Agent 可以理解。设计时要简单可靠，评估看准确率和响应速度。
...
2. **其次是四个核心设计原则**：单一职责——每个 Skill 只做一件事，"发邮件"和"验证邮箱格式"应该是两个 Skill，单一职责让 Skill 易于理解、测试和组合；幂等性——同一 Skill 被调用多次结果一致，对 Agent 的重试机制至关重要；可观测性——内置调用追踪和性能监控，方便排查问题；语义化描述——描述要包含做什么、输入输出、适用场景、示例，让LLM能精准理解和选用。

3. **最后是质量评估**。用五个量化指标：可调用成功率（>95%）、平均响应时间（非IO <1s，IO操作 <5s）、错误恢复率（>80%）、LLM选用准确率（>90%）、参数传递准确率（>95%）。这些指标通过日志和监控收集，LLM选用准确率低说明描述写得不好，参数准确率低说明 inputSchema 定义不清晰。

![image.png](img_321937d5b82792cb.png)

**3️⃣ Key Differences**

| 维度 | Common Answer | Impressive Answer |
| --- | --- | --- |
| 概念理解 | 只说"给Agent用的能力" | 从目标用户、描述方式、可组合性、元数据等维度对比 Skill 与函数的区别 |
| 设计原则 | 只说"简单可靠" | 四个具体原则：单一职责、幂等性、可观测性、语义化描述 |
| 质量评估 | 只说"准确快速稳定" | 五个量化指标 + 指标低时的诊断思路 |
| 给面试官的印象 | 了解 Skill 概念 | 有完整的 Skill 设计方法论，知道如何设计和评估 |
...
写好函数名和注释，参数类型标注清楚，Agent 就能理解了。加个参数校验，防止传错参数。
...
| 维度 | Common Answer | Impressive Answer |
| --- | --- | --- |
| 描述质量 | 写函数注释 | 语义化描述包含场景、参数约束、副作用 |
| 参数验证 | 手动校验 | JSON Schema 约束 + 自动验证 |
| 幂等性 | 未考虑 | 幂等键防止 Agent 重试时重复发送 |
...
| 关联题 | 和本题的关系 |
| --- | --- |
| 如何写好工具的 description，提升 LLM 的工具选择准确率？ | Skill 语义化描述原则的核心，description 质量直接影响 LLM 选用准确率 |
| 什么是幂等性，Agent 里为什么特别重要？ | Skill 设计四原则之一，Agent 重试机制依赖幂等性保证结果一致 |
| 如何为 Skill 添加可观测性？ | Skill 设计四原则之一，内置追踪和监控是 Skill 区别于普通工具的重要特征 |
...
Skill 库就是把很多 Skill 放在一起，按功能分类，这样容易找到。Skill 之间可能有的依赖，需要处理好。要写好文档和测试，方便维护。扩展性可以通过插件机制来实现，让用户可以自定义 Skill。
...
2. **其次是注册表机制**。Skill 库的核心是 `SkillRegistry`，负责注册、发现、按类别列举、模糊搜索。注册时按 `(name, version)` 二元组做键，支持多版本共存，`get()` 方法默认返回最新版本。

3. **然后是依赖处理**。Skill 之间的依赖有三种处理方式：依赖注入（构造时传入依赖 Skill 实例，耦合最低）、依赖声明（在类属性里声明依赖名，运行时从 Registry 取）、依赖隔离（每个 Skill 完全独立，组合逻辑上移到 Agent 层）。工程上优先用依赖隔离，保持 Skill 的原子性。

4. **最后是可扩展性**。两个机制：插件机制（`load_plugin(path)` 动态 import 并自动注册模块里的所有 Skill 实例），和版本控制（Registry 按 `(name, version)` 存储，支持灰度切换和回滚）。文档和测试是可维护性保证——每个 Skill 要有 docstring（含参数、返回值、示例、注意事项）和对应的单元测试 + 集成测试。

![AgentSkill设计.png](img_57e5e681b31a5674.png)

**3️⃣ Key Differences**

| 维度 | Common Answer | Impressive Answer |
| --- | --- | --- |
| 组织结构 | 只说"按功能分类" | 给出完整分层目录，并说明每层的职责 |
| 注册表 | 未提及 | 说明 SkillRegistry 的设计，含版本管理机制 |
| 依赖处理 | 只说"处理依赖" | 给出三种方式并说明工程优先级 |
| 可扩展性 | 只说"插件机制" | 区分插件动态加载和版本控制两个维度 |
| 给面试官的印象 | 了解 Skill 库的概念 | 有完整的 Skill 库设计经验，覆盖从组织到扩展的全流程 |
...
把 Skill 放到一个公共的地方，每个 Agent 去取就好了。要注意并发的问题，可以加锁。版本不同的话可以让 Agent 自己指定版本。
...
| 维度 | Common Answer | Impressive Answer |
| --- | --- | --- |
| 并发安全 | 说"加锁" | 从根本上设计无状态 Skill，避免锁的需要 |
| 版本兼容 | 说"自己指定版本" | 说明 Registry 单例 + 多版本共存的具体机制 |
| 副作用处理 | 未提及 | 提出幂等设计，覆盖有副作用 Skill 的生产场景 |
| 给面试官的印象 | 知道要共享 | 理解多 Agent 系统的工程约束，设计到位 |
...
| 关联题 | 和本题的关系 |
| --- | --- |
| LangChain 的 Tool 和 Skill 有什么区别？ | Tool 是 LangChain 框架内的 Skill 实现，理解 Skill 库有助于理解 Tool 注册和管理机制 |
| 如何给 Agent 的工具调用做权限控制？ | Skill 库的注册表天然是权限控制的切入点，可以在 `get()` 层加 ACL |
| Skill 库如何做灰度发布和回滚？ | 和版本控制机制直接相关，Registry 按版本存储是灰度切换的基础 |
...
技能可以在启动时注册到一个注册中心，Agent 需要用的时候去查询。可以用 Map 存储技能名称和实现类的映射，根据用户意图选择合适的技能。
...
| 维度 | Common Answer | Impressive Answer |
| --- | --- | --- |
| 设计完整性 | 只说了 Map 存储 | 注册-发现-路由完整设计 |
| 路由策略 | 无 | LLM 语义路由 vs Embedding 路由对比 |
| 扩展性 | 无 | 支持运行时热注册 |
| 面试官印象 | 知道概念 | 能设计技能系统架构 |
...
技能链就是一个技能的输出作为下一个技能的输入，串行执行。技能并行就是多个技能同时执行，最后汇总结果。有依赖关系的用串行，没有依赖的用并行。
...
| 维度 | Common Answer | Impressive Answer |
| --- | --- | --- |
| 技术深度 | 只说了串行和并行 | 引入 DAG 调度解决复杂依赖 |
| 代码实现 | 无 | 有串行/并行/DAG 三种实现 |
| 可视化 | 无 | 有 mermaid 依赖图 |
| 面试官印象 | 知道概念 | 能设计技能编排引擎 |
...
SkillContext 就是技能执行时需要的上下文信息，可以用一个 Map 存储，里面放用户输入、历史对话、工具结果等。技能执行时从 Map 里取需要的数据。
...
| 维度 | Common Answer | Impressive Answer |
| --- | --- | --- |
| 数据结构 | 用 Map 存所有数据 | 分层设计（用户层/历史层/执行层/元数据层） |
| 线程安全 | 未考虑 | 不可变设计，天然线程安全 |
| 可扩展性 | 无 | 链式扩展方法，支持 DAG 和链式两种场景 |
| 面试官印象 | 知道需要上下文 | 能设计生产级不可变上下文 |
...
可以在技能执行前后打日志，记录技能名称、执行时间、是否成功。如果失败了记录异常信息。
...
| 维度 | Common Answer | Impressive Answer |
| --- | --- | --- |
| 覆盖维度 | 只有日志 | Trace + Metrics + Logs 三维覆盖 |
| 实现方式 | 手动打日志 | AOP 统一切入，业务代码零侵入 |
| LLM 特性 | 无 | 额外统计 Token 成本 |
| 面试官印象 | 知道要打日志 | 能设计生产级可观测体系 |
...
技能失败了可以重试几次，重试还是失败就返回错误。可以有一个默认的降级回答，比如"暂时无法处理，请稍后再试"。
...
| --- | --- | --- |
| 瞬时错误 | 网络超时、限流 | 指数退避重试 |
| 业务错误 | 参数不合法、权限不足 | 不重试，直接返回错误 |
| 依赖不可用 | 下游服务宕机 | 熔断 + 降级 |
| 幂等性问题 | 写操作已执行 | 检查幂等键，避免重复执行 |

**3️⃣ Key Differences**

| 维度 | Common Answer | Impressive Answer |
| --- | --- | --- |
| 错误分类 | 无区分，统一重试 | 区分可重试/不可重试错误 |
| 重试策略 | 固定间隔重试 | 指数退避 + 熔断器防雪崩 |
| 降级层次 | 只有兜底回答 | 缓存 > 简化版 > 兜底三级降级 |
| 面试官印象 | 知道要重试 | 能设计生产级容错体系 |
...
可以给技能加权限控制，不同用户能用不同的技能。敏感技能需要验证用户身份才能执行。
...
| 维度 | Common Answer | Impressive Answer |
| --- | --- | --- |
| 控制粒度 | 只说了用户权限 | RBAC + 输入校验 + 执行隔离三层 |
| 安全意识 | 无 | 考虑了 Prompt 注入攻击 |
| 资源隔离 | 无 | 高风险技能独立线程池，防止资源耗尽 |
| 面试官印象 | 知道要鉴权 | 能设计多层次安全防护体系 |
...
在 Spring AI 里可以用 `@Bean` 注册一个函数，然后在调用 LLM 时把这个函数传进去，LLM 就能调用它了。
...
| 维度 | Common Answer | Impressive Answer |
| --- | --- | --- |
| 实现完整性 | 只说了注册 @Bean | 定义→注册→挂载→执行完整流程 |
| 代码规范 | 无 | 有 DTO 设计、@Description 注解 |
| 框架理解 | 模糊 | 清楚 FunctionCallbackWrapper 的用法 |
| 面试官印象 | 知道 Spring AI 能做 | 能写出可运行的完整代码 |
...
| 关联题 | 和 Skills 的关系 |
| --- | --- |
| Function Calling 原理 | Skills 底层依赖 Function Calling 触发 |
| ReAct 框架 | Agent 通过 ReAct 决策调用哪个 Skill |
| 线程池设计 | 技能并行执行依赖线程池隔离 |
| 熔断器（Sentinel/Resilience4j） | 技能容错的核心组件 |
| Spring AOP | 技能可观测性的实现方式 |
| Token 成本控制 | LLM 技能需要统计和控制 Token 消耗 |
