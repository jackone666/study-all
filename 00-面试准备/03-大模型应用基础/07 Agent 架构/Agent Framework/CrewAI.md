...
![image.png](img_576a8e5e8853436e.png)
...
Crew 就是一个小组，里面包含多个 Agent 和多个 Task。Agent 负责执行任务，Task 是具体的工作内容。Crew 把 Agent 和 Task 组织起来，按顺序或者并行执行。
...
| --- | --- | --- |
| 结构性 | 只说"包含关系"，无职责区分 | 从职责分层 + 映射关系两维说明 |
| 技术深度 | 不了解上下文链传递机制 | 明确 Task 输出作为后续上下文 |
| 面试官印象 | 知道基本概念，但关系模糊 | 清晰理解三者的设计意图和协作方式 |
...
Sequential 就是顺序执行，一个任务接一个任务做。Hierarchical 是分层执行，有主次之分。具体怎么选要看场景。
...
| 维度 | Common Answer | Impressive Answer |
| --- | --- | --- |
| 结构性 | 只说了字面意思，无实质区别 | 从调度策略、协作模式、优缺点对比 |
| 技术深度 | 不了解上下文注入和 Manager 角色 | 明确 Sequential 上下文链、Hierarchical Manager 协调 |
| 实践经验 | 无选型建议 | 给出清晰的场景匹配原则 |
| 面试官印象 | 概念模糊 | 有实际使用经验，能指导选型 |
...
定义几个 Task：搜索数据、分析数据、写报告。第一个 Task 的输出传给第二个，第二个传给第三个。用 CrewAI 的 context 参数来传递。
...
| 维度 | Common Answer | Impressive Answer |
| --- | --- | --- |
| 结构性 | 只说"传给下一个"，无具体设计 | 4 步任务拆解 + 依赖定义 + 上下文注入 |
| 技术深度 | 不了解 context 参数用法 | 明确 context 接收 Task 列表，自动拼接注入 |
| 实践经验 | 无上下文长度意识 | 提到输出过长需精简摘要 |
| 面试官印象 | 有基本概念，但设计不完整 | 有完整的设计思路和实践细节 |
...
| 维度 | Common Answer | Impressive Answer |
| --- | --- | --- |
| 结构性 | 随意罗列三种记忆，无逻辑层次 | 按记忆类型分层讲解，每类包含机制、实现、场景 |
| 技术深度 | 仅描述功能，无技术细节 | 涉及 RAG、Vector DB、NER、Embedding 等核心技术 |
| 实践经验 | 泛泛而谈"要注意控制性能" | 提供完整代码示例，说明配置参数和实际应用场景 |
| 面试官印象 | 候选人了解概念但缺乏深入理解 | 候选人有扎实的工程实践能力，能落地复杂功能 |
...
自定义 Tool 就是写一个 Python 类，继承 `BaseTool`，然后实现 `run` 方法。绑定给 Agent 就是在创建 Agent 的时候把 Tool 传进去。错误处理就是在 `run` 方法里用 try-except 捕获异常，返回错误信息。如果 Tool 执行失败了，Agent 会收到错误消息，然后根据情况重试或者报错。CrewAI 好像有自动重试机制，可以设置重试次数。
...
| 维度 | Common Answer | Impressive Answer |
| --- | --- | --- |
| 结构性 | 混乱描述，无代码示例 | 分层讲解：Tool 实现 → Agent 绑定 → 错误处理 |
| 技术深度 | 仅提到继承和 try-except | 涉及 Pydantic 验证、异步支持、多层错误处理机制 |
| 实践经验 | 泛泛而谈"有重试机制" | 提供完整可运行的代码示例，包含参数配置和最佳实践 |
| 面试官印象 | 候选人了解基础用法 | 候选人有丰富的实战经验，能处理复杂工程问题 |
...
这个系统需要四个 Agent，每个负责不同的审查维度。代码分析 Agent 看代码质量，安全审查 Agent 检查安全问题，性能审查 Agent 分析性能，汇总 Agent 把结果整合起来。Task 依赖就是先并行执行前面三个，然后汇总 Agent 等它们都完成后再汇总。输出格式可以是 JSON 或者 Markdown 报告。每个 Agent 都需要配置合适的 Prompt，让它们知道要查什么。可能还需要一些工具来运行代码或者扫描漏洞。
...
| 维度 | Common Answer | Impressive Answer |
| --- | --- | --- |
| 结构性 | 简单罗列四个 Agent，无详细设计 | 完整的系统设计：Agent 角色 → Task 依赖 → 输出格式 → 执行流程 |
| 技术深度 | 仅提到"配置合适的 Prompt" | 提供完整代码实现，包含工具、Agent、Task、Crew 的详细配置 |
| 实践经验 | 泛泛而谈"并行执行、汇总结果" | 使用 Hierarchical Process、JSON Schema 验证、错误处理等工程实践 |
| 面试官印象 | 候选人了解基本概念 | 候选人有系统设计能力，能落地复杂多 Agent 系统 |
...
Sequential 就是顺序执行，一个任务完成再做下一个。Hierarchical 有个 Manager Agent 来协调，可以动态分配任务。CrewAI 比 LangGraph 简单，适合快速开发，LangGraph 更灵活但更复杂。
...
2. **其次说 CrewAI 的设计本质**。Agent 里的 backstory 字段本质上是通过 System Prompt 让 LLM 进行角色扮演，让同一个底层模型表现出差异化的专业能力；Task 的 `expected_output` 约束了 LLM 的输出格式，减少后续解析的不确定性。理解这两点，才算真正理解 CrewAI 的设计意图。

3. **最后说与 LangGraph 的抽象层次差异**。CrewAI 是声明式、高抽象层，只需描述"有哪些角色、有哪些任务"，内部的 Prompt 构造和循环控制都由框架处理，上手快但灵活性受限。LangGraph 是图式、低抽象层，需要显式定义节点、边和条件路由，开发成本高但可以精确控制状态流转，更适合生产环境的关键业务。选型建议：多专家角色协作的明确任务用 CrewAI，需要精确状态管理和错误恢复的用 LangGraph。

**3️⃣ Key Differences**

| 维度 | Common Answer | Impressive Answer |
| --- | --- | --- |
| 技术深度 | 停留在概念列举，无设计意图分析 | 解释了 backstory 的 Prompt 本质、expected_output 的约束作用 |
| 实践经验 | 无场景举例 | 给出了 Sequential 适合流水线、Hierarchical 适合动态调度的判断 |
| 思考维度 | 简化为"简单 vs 复杂" | 从抽象层次（声明式 vs 图式）精准对比，给出选型建议 |
| 给面试官的印象 | 了解基本概念，没用过的感觉 | 有实际使用经验，理解框架设计取舍 |
...
可以写详细一点的 backstory 和任务描述，让 Manager 更清楚该怎么做，或者换成 Sequential 模式，这样更可控。
...
| 维度 | Common Answer | Impressive Answer |
| --- | --- | --- |
| 技术深度 | 只知道改描述或换模式 | 分析了不可预测的根本原因，给出针对性的多层解法 |
| 实践经验 | 无具体配置细节 | 提到 manager_llm 独立配置，区分了适用场景 |
| 思考维度 | 被动应对 | 从框架设计层理解问题，主动设计约束策略 |
| 给面试官的印象 | 会基本配置 | 有 CrewAI 生产踩坑经验 |
...
| 关联题 | 和本题的关系 |
| --- | --- |
| CrewAI vs LangGraph 如何选型？ | 两者抽象层次不同，是 CrewAI 定位理解的核心对比维度 |
| LLM 角色扮演（System Prompt）原理是什么？ | backstory 本质是 System Prompt 注入，理解此原理才能写好 Agent 配置 |
| AutoGen 和 CrewAI 都是多 Agent 框架，区别在哪？ | AutoGen 对话驱动、CrewAI 角色驱动，适用场景和设计哲学都不同 |
