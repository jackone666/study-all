...
多 Agent 系统优点很多，任务复杂的时候就应该用，简单任务就不用。可以根据具体情况来判断。用的好的话效果很好，分工明确，可以提高效率。不用的话就是任务比较简单，一个 Agent 就能搞定。
...
2. **再看任务结构是否天然适合拆分**。我有一个"三问判断法"：任务是否超过单 Agent 的 Context Window 上限？是否有可并行的子任务？是否有需要不同专业能力的环节？三个都是"否"就不上。

3. **最后做适用边界判断**。适合多 Agent：任务链路长（5步以上）、子任务边界清晰、对准确性要求高、需要不同领域专业能力。不适合：任务简单直接、对延迟极其敏感、预算有限。

**3️⃣ Key Differences**

| 维度 | Common Answer | Impressive Answer |
| --- | --- | --- |
| 技术深度 | 停留在"复杂用、简单不用"的直觉判断 | 给出"三问判断法"等可操作的工程决策标准 |
| 实践经验 | 无具体判断标准 | 明确列出任务复杂度阈值和适用边界 |
| 思考维度 | 只讲优点，缺点一笔带过 | 强调多 Agent 的真实代价，体现不盲目追新的工程判断力 |
| 给面试官的印象 | 了解基本概念 | 有工程落地经验，判断力强 |
...
可以设计三个 Agent，一个分析竞品，一个分析用户，一个分析趋势，最后汇总结果就好了。
...
2. **主从模式（Orchestrator-Worker）**：一个 Orchestrator Agent 负责任务拆解和调度，多个 Worker Agent 并行执行子任务，最后汇总结果。适合可并行的复杂任务

3. **对等协作模式（Peer-to-Peer）**：多个 Agent 平等协商，通过消息传递达成共识。适合需要多视角验证的场景（如辩论式推理）
...
| 维度 | Common Answer | Impressive Answer |
| --- | --- | --- |
| 技术深度 | 只提到"三个 Agent 汇总" | 明确说明并行执行方案（asyncio.gather）和汇总策略（LLM 综合） |
| 实践经验 | 无工具配置和性能优化思考 | 考虑到专业化工具配置、并行带来的延迟优化 |
| 思考维度 | 关注分工 | 同时关注分工、并行、汇总三个维度 |
| 给面试官的印象 | 有基本思路 | 有完整的工程设计方案 |
...
| 关联题 | 和本题的关系 |
| --- | --- |
| LangGraph 中如何实现 Agent 并行执行？ | 多 Agent 并行是核心优势，需要知道框架层的具体实现 |
| 多 Agent 系统的 Token 成本如何控制？ | 多 Agent 优势背后有真实成本，选型时必须考虑 ROI |
| 单 Agent + 多工具 vs 多 Agent 如何选？ | 适用边界问题的进阶版，考察对两种架构的深度理解 |

---

### Supervisor 模式 vs Peer-to-Peer 模式的架构对比

![image.png](img_41e128c178a2f693.png)
...
Supervisor 模式有一个主 Agent 来管所有的子 Agent，优点是好管理，缺点是 Supervisor 挂了全挂。Peer-to-Peer 就是 Agent 之间直接通信，优点是灵活，缺点是复杂度高。一般任务复杂就用 Supervisor，流程固定就用 Peer-to-Peer。
...
2. **Peer-to-Peer 模式的核心权衡**：优势是没有单点故障、延迟低、适合高并发流水线；缺点是全局状态分散导致调试极难，Agent 数量增加时通信关系呈 N² 增长，没有统一的冲突解决入口。

3. **生产选型建议**：任务流程复杂、需要动态决策选 Supervisor；任务流程固定、追求低延迟的流水线选 Peer-to-Peer。生产环境往往是混合架构——用 Supervisor 做顶层编排，内部子流程用 Peer-to-Peer 流水线。

**3️⃣ Key Differences**

| 维度 | Common Answer | Impressive Answer |
| --- | --- | --- |
| 技术深度 | 泛泛描述优缺点 | 用拓扑结构类比，分析单点瓶颈、N² 复杂度等具体问题 |
| 实践经验 | 无具体工程判断标准 | 给出明确的选型场景，并提出混合架构方案 |
| 思考维度 | 非此即彼地对比 | 体现生产级系统设计思维，提出混合架构 |
| 给面试官的印象 | 知道概念 | 对架构权衡有深入理解，有实际落地经验 |
...
在 LangGraph 里定义一个 Supervisor 节点，让它来决定下一步调用哪个子 Agent，子 Agent 完成后再回到 Supervisor 就行了。
...
| 维度 | Common Answer | Impressive Answer |
| --- | --- | --- |
| 技术深度 | 一句话描述实现思路 | 给出完整可运行代码，解释条件路由的设计逻辑 |
| 实践经验 | 无代码细节 | 说明 add_conditional_edges 和回边设计的工程意图 |
| 思考维度 | 描述"是什么" | 解释每个设计决策背后的原因 |
| 给面试官的印象 | 知道 LangGraph 能做 | 有实际实现经验，代码可直接落地 |
...
| 关联题 | 和本题的关系 |
| --- | --- |
| LangGraph 的 StateGraph 和 MessageGraph 有什么区别？ | Supervisor 模式依赖 StateGraph 的共享状态机制 |
| 多 Agent 系统如何防止死循环？ | Supervisor 路由设计必须考虑终止条件，否则会无限循环 |
| 微服务架构中多 Agent 如何通信？ | Peer-to-Peer 模式在跨服务场景下的延伸，引出消息队列方案 |

---

### 多 Agent 的任务分解与动态分配策略

![image.png](img_c5fa11da92b2a6b4.png)
...
路由 Prompt 就是告诉 LLM 有哪些 Agent 可以用，让它决定调用谁。写清楚每个 Agent 是干什么的，让 LLM 根据当前情况选择就好了。还要设置一个结束条件，让任务能正常结束。
...
2. **必须加防死循环规则**。显式在 Prompt 中写入约束，比如"避免连续调用同一 Agent""如果上一步输出已满足需求则输出 FINISH"，否则 LLM 在某些情况下会陷入无限循环。

3. **结构化输出格式**。要求 LLM 只输出固定选项（如 `researcher/coder/writer/FINISH`），不要让它自由发挥，这样 parse 结果不会出错，系统可靠性更高。

**3️⃣ Key Differences**

| 维度 | Common Answer | Impressive Answer |
| --- | --- | --- |
| 技术深度 | 描述路由 Prompt 的大概思路 | 给出具体的 Prompt 模板，解释每个设计决策的原因 |
| 实践经验 | 无防死循环等工程细节 | 明确提出防死循环规则和结构化输出的工程价值 |
| 思考维度 | 关注"写什么" | 关注"为什么这样写"，体现 Prompt 工程经验 |
| 给面试官的印象 | 知道需要写路由 Prompt | 有路由 Prompt 工程经验，考虑到可靠性和边界情况 |
...
T1 和 T2 可以同时跑，等它们都完成后再跑 T3，T3 完成后跑 T4，按这个顺序执行就行了。
...
| 维度 | Common Answer | Impressive Answer |
| --- | --- | --- |
| 技术深度 | 描述执行顺序 | 用 DAG 数据结构建模，说明 asyncio.gather 的并行原理 |
| 实践经验 | 无代码实现 | 给出 DAG 数据结构定义，体现工程落地思路 |
| 思考维度 | 关注顺序 | 关注并行优化，说明并行带来的耗时收益 |
| 给面试官的印象 | 理解依赖关系 | 有任务调度工程经验 |
...
| 关联题 | 和本题的关系 |
| --- | --- |
| Python asyncio 的 gather 和 wait 有什么区别？ | 多 Agent 并行执行的底层机制，gather 是最常用的并发等待方式 |
| LangGraph 如何实现条件分支和循环？ | 动态分配策略在 LangGraph 中的具体实现依赖条件边和循环图结构 |
| 如何防止多 Agent 系统中的任务重复执行？ | DAG 调度中的幂等性保证，生产中必须考虑的可靠性问题 |
...
3. **工具调用层（工具层）**。能执行代码、写文件、发网络请求的工具是最大风险点。安全工具执行器需要做两道检查：先验证 Agent 是否有该工具的执行权限，再做静态代码分析拦截危险导入（如 `os`、`subprocess`）。最终在沙箱（RestrictedPython 或 Docker 容器）中执行，防止注入攻击。
...
![image.png](img_80f6bf5216a726ab.png)
...
![image.png](img_13c12aade28ff0b5.png)
