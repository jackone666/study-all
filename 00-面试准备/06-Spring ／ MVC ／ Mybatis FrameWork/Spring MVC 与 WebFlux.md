...
Filter 是 Servlet 规范里的，Interceptor 是 Spring 的。Filter 先执行，Interceptor 后执行。Filter 可以做字符编码过滤，Interceptor 适合做权限校验。
...
2. **执行时机不同**：Filter 在请求进入容器后立即执行（早于 DispatcherServlet）；Interceptor 在 DispatcherServlet 内部执行，能访问 Handler 上下文。

3. **能力不同**：Filter 只能操作 Request/Response；Interceptor 能访问 Handler 方法信息、ModelAndView，适合做参数校验、权限鉴权、耗时统计。

4. **Agent 场景实践**：鉴权/限流用 Filter（尽早拦截非法请求）；工具调用权限校验、traceId 注入、耗时打点用 Interceptor（能获取 Controller 方法注解信息）。

**3️⃣ Key Differences**

| 维度 | Common Answer | Impressive Answer |
| --- | --- | --- |
| 结构性 | 说了基本区别但较零散 | 4 个维度对比，层次清晰 |
| 技术深度 | 不知道执行顺序的本质原因 | 清楚容器管理和框架调度的边界 |
| 实践经验 | 没有场景化建议 | 给出 Agent 鉴权的分层实践方案 |
| 面试官印象 | 背过区别但没有实战 | 能根据场景选型，有架构思维 |
...
| 关联题 | 和本题的关系 |
| --- | --- |
| @ControllerAdvice + @ExceptionHandler 如何处理全局异常？ | Spring MVC 异常处理机制，考察统一错误响应设计 |
| Spring MVC 如何实现异步请求（DeferredResult/SSE）？ | 长轮询/SSE 场景下请求处理流程的变化，考察异步编程能力 |
| HandlerMethodArgumentResolver 如何自定义？ | 自定义参数解析器，考察对 MVC 扩展点的理解 |
...
@ControllerAdvice 是全局的异常处理器，配合 @ExceptionHandler 可以捕获 Controller 抛出的异常，然后返回统一的错误格式。一般定义一个通用的错误响应类，包含 code、message、data 字段。
...
2. **注意事项**：多个 @ControllerAdvice 用 `@Order` 控制优先级；Filter 中的异常不会被 @ControllerAdvice 捕获，需要单独处理。

**3️⃣ Key Differences**

| 维度 | Common Answer | Impressive Answer |
| --- | --- | --- |
| 结构性 | 只说了注解用法 | 原理→代码实现→错误码体系→注意事项 |
| 技术深度 | 不知道异常解析器链的查找顺序 | 清楚 ExceptionHandlerExceptionResolver 的工作机制 |
| 实践经验 | 没有分类处理不同异常 | 业务异常、校验异常、未知异常分层处理 |
| 面试官印象 | 会用但不够系统 | 有完整的错误处理体系设计 |
...
可以用 Spring 的 SseEmitter 来实现 SSE 流式输出。创建一个 SseEmitter 对象，然后在异步线程里不断调用 send 方法发送数据。WebSocket 是双向的，SSE 是单向的，如果只需要服务端推送就用 SSE。
...
| --- | --- | --- |
| 方向 | 单向（服务端→客户端） | 双向 |
| 协议 | HTTP/1.1（自动重连） | 独立协议（ws://） |
| 代理兼容 | 好（标准 HTTP） | 差（需要代理支持升级） |
| Agent 场景 | **流式文本输出**（首选） | 语音对话、实时协作 |
...
| 维度 | Common Answer | Impressive Answer |
| --- | --- | --- |
| 结构性 | 只说了 SseEmitter 基本用法 | 完整代码→选型对比→生产配置 |
| 技术深度 | 不知道超时和资源管理 | 有超时设置、onCompletion 回调 |
| 实践经验 | 没考虑 Nginx 配置 | 知道 proxy\_buffering 和心跳机制 |
| 面试官印象 | 能实现基本功能 | 有生产环境部署经验 |
...
HandlerMethodArgumentResolver 是 Spring MVC 的参数解析器接口，可以自定义参数解析逻辑。比如实现一个 @CurrentUser 注解，让 Controller 方法直接获取当前用户，不用每次手动从 token 里解析。需要实现 supportsParameter 和 resolveArgument 两个方法，然后通过 WebMvcConfigurer 注册。
...
| 维度 | Common Answer | Impressive Answer |
| --- | --- | --- |
| 结构性 | 只说了概念 | 接口定义→完整代码→使用方式→场景扩展 |
| 技术深度 | 不知道如何注册 resolver | 清楚 WebMvcConfigurer 的扩展点 |
| 实践经验 | 没有提供完整实现 | 给出可运行的完整代码示例 |
| 面试官印象 | 了解基本概念 | 能独立实现自定义注解和参数解析 |
...
@RequestBody 是用来接收请求体的，Spring 会根据请求的 Content-Type 选择合适的 HttpMessageConverter 来解析。比如 application/json 就用 Jackson 来反序列化成 Java 对象。也可以自定义 HttpMessageConverter 来处理其他格式。
...
| 维度 | Common Answer | Impressive Answer |
| --- | --- | --- |
| 结构性 | 只说了基本概念 | 核心处理器→工作流程→源码图→自定义示例 |
| 技术深度 | 不知道 Converter 的选择机制 | 清楚 Content-Type 协商和遍历匹配逻辑 |
| 实践经验 | 没有提到自定义 Converter | 给出 Protobuf Converter 实现示例 |
| 面试官印象 | 知道基本用法 | 理解底层机制，能扩展处理新格式 |
...
| 关联题 | 和本题的关系 |
| --- | --- |
| @ResponseBody 和 @RestController 的区别？ | 返回值处理的对称面，考察 HttpMessageConverter 的双向工作 |
| Spring MVC 的内容协商（Content Negotiation）机制？ | 根据 Accept 头选择响应格式，和 Converter 选择机制相关 |
| 如何自定义 Jackson 的序列化/反序列化行为？ | ObjectMapper 配置，Long 精度丢失、日期格式等常见问题 |
...
| 方案 | 示例 | 优点 | 缺点 |
| --- | --- | --- | --- |
| URL 路径 | `/api/v1/users` | 直观、Nginx 方便、缓存友好 | URL 变长 |
| 请求头 | `Accept: application/vnd.myapp.v2+json` | RESTful 纯粹、URL 简洁 | 不直观、调试麻烦 |
| 自定义注解 | `@ApiVersion(1)` | 优雅、自动路由 | 实现复杂 |
| 请求参数 | `?version=2` | 简单 | 不优雅、缓存失效 |
...
| 维度 | Common Answer | Impressive Answer |
| --- | --- | --- |
| 结构性 | 只列举了几种方式 | 方案对比表→最佳实践→代码示例 |
| 技术深度 | 不知道各方案的优缺点 | 从 URL、缓存、调试等多维度分析 |
| 实践经验 | 没有生产环境建议 | 给出 Nginx 路由和 Swagger 集成方案 |
| 面试官印象 | 知道基本概念 | 有完整的版本管理架构设计 |
...
| 维度 | Common Answer | Impressive Answer |
| --- | --- | --- |
| 结构性 | 只说了基本思路 | 策略模式设计→完整代码→生产考虑 |
| 技术深度 | 没有设计模式应用 | 用策略模式解耦输出逻辑 |
| 实践经验 | 没有考虑异步和监控 | 有超时设置和监控指标建议 |
| 面试官印象 | 能实现基本功能 | 有架构设计思维和生产经验 |
...
| 关联题 | 和本题的关系 |
| --- | --- |
| Swagger/OpenAPI 文档如何集成？ | API 文档化，和版本管理配合 |
| Spring HATEOAS 是什么？ | RESTful 成熟度模型的最高级别 |
| API 限流和熔断如何在 Controller 层实现？ | API 治理的重要组成部分 |
...
| 维度 | Common Answer | Impressive Answer |
| --- | --- | --- |
| 结构性 | 只说了基本概念 | 完整代码→分组示例→顺序控制→跨字段校验 |
| 技术深度 | 不知道 @GroupSequence 的作用 | 清楚分组校验的顺序控制机制 |
| 实践经验 | 没有提供完整实现 | 给出可直接运行的完整代码 |
| 面试官印象 | 了解基本用法 | 能独立实现复杂校验逻辑 |
...
| 维度 | Bean Validation | JSON Schema |
| --- | --- | --- |
| 适用场景 | 固定结构的 Java 对象 | 动态 JSON 结构 |
| 校验时机 | 编译期确定 | 运行时解析 Schema |
| 灵活性 | 低（需修改代码） | 高（配置 Schema） |
| Agent 场景 | 外层请求校验 | 工具参数校验 |
...
| 维度 | Common Answer | Impressive Answer |
| --- | --- | --- |
| 结构性 | 只说了基本思路 | 选型对比→混合方案→代码实现→错误设计 |
| 技术深度 | 不知道如何结合使用 | 清楚两种方案的优势和互补性 |
| 实践经验 | 没有错误信息设计建议 | 给出结构化错误返回格式 |
| 面试官印象 | 了解基本概念 | 有完整的校验架构设计 |
...
| 关联题 | 和本题的关系 |
| --- | --- |
| Spring MVC 的数据绑定原理（DataBinder）？ | 参数绑定和校验的底层机制 |
| 如何实现请求参数的类型转换（Converter/Formatter）？ | 自定义类型转换，如字符串转枚举 |
| @RequestParam 和 @PathVariable 的校验方式？ | 非 @RequestBody 参数的校验方式不同 |
...
2. **JWT 场景**：自定义 `JwtAuthenticationFilter` 插入到 `UsernamePasswordAuthenticationFilter` 之前：
...
| 维度 | Common Answer | Impressive Answer |
| --- | --- | --- |
| 结构性 | 只说了基本概念 | 集成原理→执行流程图→JWT 示例→场景扩展 |
| 技术深度 | 不知道 Filter 的执行顺序 | 清楚 DelegatingFilterProxy 的代理机制 |
| 实践经验 | 没有自定义 Filter 示例 | 给出 JWT Filter 的完整实现 |
| 面试官印象 | 了解基本概念 | 理解底层机制，能扩展安全策略 |
...
| 关联题 | 和本题的关系 |
| --- | --- |
| OAuth 2.0 和 JWT 的区别？Spring Security OAuth2 如何集成？ | 认证授权体系，Agent API 的安全设计 |
| CSRF 防护的原理？为什么 REST API 通常禁用 CSRF？ | 安全防护策略，无状态 API 的特殊处理 |
| Spring Security 的方法级安全（@PreAuthorize）如何实现？ | 细粒度权限控制，和 AOP 的关系 |
...
4. **HandlerMapping**：请求映射处理器（URL → Controller）

5. **HandlerAdapter**：处理器适配器（执行 Controller 方法）
...
9. **FlashMapManager**：Flash 属性管理器（Redirect 参数传递）
...
| 特性 | RequestMappingHandlerMapping | BeanNameUrlHandlerMapping |
| --- | --- | --- |
| 映射方式 | @RequestMapping 注解 | Bean 名称 |
| 灵活性 | 高（支持路径变量、通配符） | 低（仅精确匹配） |
| 适用场景 | REST API、复杂路由 | 简单路由、遗留系统 |
...
| 维度 | Common Answer | Impressive Answer |
| --- | --- | --- |
| 结构性 | 没有结构，随意列举 | 类型→优先级→对比表→代码示例 |
| 技术深度 | 不知道匹配顺序 | 清楚 HandlerMapping 的遍历逻辑 |
| 实践经验 | 没有自定义示例 | 给出版本路由的完整实现 |
| 面试官印象 | 了解基础概念 | 掌握底层机制，能灵活扩展 |
...
| 维度 | Common Answer | Impressive Answer |
| --- | --- | --- |
| 结构性 | 没有方案，只说"可以" | 方案一（自定义）→方案二（条件）→场景实践 |
| 技术深度 | 不知道如何自定义 | 给出完整的 HandlerMapping 实现 |
| 实践经验 | 没有代码示例 | 包含 RequestWrapper、Controller 完整代码 |
| 面试官印象 | 了解基本思路 | 能解决实际问题，架构能力强 |
...
| 关联题 | 和本题的关系 |
| --- | --- |
| Spring MVC 的拦截器执行顺序？如何自定义拦截器？ | 请求处理链路，和 HandlerMapping 的配合 |
| @RequestMapping 的 params、headers 属性如何使用？ | 条件路由的替代方案 |
| Spring Cloud Gateway 的路由规则如何配置？ | 微服务场景下的路由扩展 |
...
| 维度 | Common Answer | Impressive Answer |
| --- | --- | --- |
| 结构性 | 只知道基本概念 | 策略→配置→示例→场景扩展 |
| 技术深度 | 不知道如何配置 | 清楚 ContentNegotiationConfigurer 的配置项 |
| 实践经验 | 没有代码示例 | 给出完整的多格式响应实现 |
| 面试官印象 | 了解基础 | 掌握内容协商机制，能灵活配置 |
...
| 维度 | Common Answer | Impressive Answer |
| --- | --- | --- |
| 结构性 | 只知道"添加转换器" | 依赖→配置→三种方案→场景实践 |
| 技术深度 | 不知道具体配置 | 给出完整的 HttpMessageConverter 配置 |
| 实践经验 | 没有代码示例 | 包含 Protobuf 定义、Controller 实现 |
| 面试官印象 | 了解基本概念 | 能解决多协议问题，工程能力强 |
...
| 关联题 | 和本题的关系 |
| --- | --- |
| Jackson 自定义序列化如何实现？@JsonSerialize 的用法？ | JSON 格式的深度定制 |
| Protobuf 和 JSON 的性能对比？各自适用场景？ | 格式选型，性能优化 |
| Spring MVC 如何处理大文件上传？MultipartFile 的使用？ | 文件处理，和内容协商的配合 |
...
| 维度 | Common Answer | Impressive Answer |
| --- | --- | --- |
| 结构性 | 只知道基本概念 | 原理→流程→处理机制→场景实践 |
| 技术深度 | 不知道 WebAsyncManager | 清楚异步请求的完整处理流程 |
| 实践经验 | 没有代码示例 | 给出超时、错误、全局异常处理 |
| 面试官印象 | 了解基础 | 掌握异步机制，能处理复杂场景 |
...
| 维度 | Common Answer | Impressive Answer |
| --- | --- | --- |
| 结构性 | 只知道"用异步" | 线程池配置→任务服务→Controller→状态管理 |
| 技术深度 | 不知道如何避免线程耗尽 | 清楚异步架构的完整设计 |
| 实践经验 | 没有代码示例 | 给出深度搜索、多步推理的完整实现 |
| 面试官印象 | 了解基本概念 | 能设计异步架构，解决性能问题 |
...
| 关联题 | 和本题的关系 |
| --- | --- |
| Spring @Async 注解的原理？如何自定义线程池？ | 异步执行，和 DeferredResult 的配合 |
| CompletableFuture 的使用场景？如何组合多个异步任务？ | 异步编程，复杂任务编排 |
| Tomcat 线程池参数如何调优？如何监控线程池状态？ | 性能优化，避免线程耗尽 |
...
| --- | --- | --- |
| 依赖 | Apache Commons FileUpload | Servlet 3.0+ 内置 |
| 性能 | 较慢（需要磁盘缓冲） | 较快（内存优先） |
| 配置 | XML/Java Config | Servlet Config |
| 适用场景 | Servlet 2.x 或需要兼容 | Servlet 3.0+ 推荐 |
...
| 维度 | Common Answer | Impressive Answer |
| --- | --- | --- |
| 结构性 | 只知道基本概念 | 对比表→配置→分片方案→场景实践 |
| 技术深度 | 不知道两种实现的区别 | 清楚 MultipartResolver 的优缺点 |
| 实践经验 | 没有代码示例 | 给出分片上传、文档索引的完整实现 |
| 面试官印象 | 了解基础 | 掌握文件处理，能解决实际问题 |
...
| 维度 | Common Answer | Impressive Answer |
| --- | --- | --- |
| 结构性 | 只知道"上传后处理" | 流式上传→提取器→分块→向量化→入库 |
| 技术深度 | 不知道流式处理 | 清楚流式处理的完整架构 |
| 实践经验 | 没有代码示例 | 给出 PDF/Word 提取、分块、向量化的完整实现 |
| 面试官印象 | 了解基本概念 | 能设计流式处理架构，解决性能问题 |
...
| 关联题 | 和本题的关系 |
| --- | --- |
| Spring @Async 的原理？如何自定义线程池？ | 异步处理，和文件上传的配合 |
| 向量数据库（如 Milvus、Pinecone）如何集成？ | 知识库向量化入库 |
| 如何实现文件的断点续传？ | 大文件上传的扩展 |
...
2. **WebMvcAutoConfiguration 加载条件**：
...
| --- | --- | --- | --- |
| 默认方式 | ✅ 启用 | ✅ 可扩展 | 大多数场景 |
| @EnableWebMvc | ❌ 禁用 | ✅ 完全自定义 | 需要完全控制 |
| WebMvcConfigurer | ✅ 启用 | ✅ 可扩展 | 推荐方式 |
...
| 维度 | Common Answer | Impressive Answer |
| --- | --- | --- |
| 结构性 | 只知道"会冲突" | 影响分析→加载条件→对比表→推荐方式 |
| 技术深度 | 不知道加载条件 | 清楚 @ConditionalOnMissingBean 的作用 |
| 实践经验 | 没有代码示例 | 给出三种配置方式和 Agent 统一配置 |
| 面试官印象 | 了解基本概念 | 掌握自动配置机制，能合理选择配置方式 |
...
| 维度 | Common Answer | Impressive Answer |
| --- | --- | --- |
| 结构性 | 只知道"分别注册" | 自动发现机制→统一配置→组件实现→使用示例 |
| 技术深度 | 不知道如何自动注册 | 清清楚组件自动发现和排序机制 |
| 实践经验 | 没有代码示例 | 给出完整的自动注册架构 |
| 面试官印象 | 了解基本概念 | 能设计可扩展的架构，工程能力强 |
...
| 关联题 | 和本题的关系 |
| --- | --- |
| Spring Boot 自动配置原理？@Conditional 注解的作用？ | 自动配置机制，和 WebMvcConfigurer 的配合 |
| HandlerMethodArgumentResolver 如何自定义？ | 参数解析，WebMvcConfigurer 的扩展点之一 |
| Spring MVC 拦截器执行顺序？如何控制？ | 拦截器配置，WebMvcConfigurer 的扩展点之一 |
...
3. **DefaultHandlerExceptionResolver**：处理 Spring MVC 内置异常

4. **自定义 HandlerExceptionResolver**：自定义异常解析器
...
| ConversionNotSupportedException | 500 | 类型转换不支持 |
| TypeMismatchException | 400 | 类型转换失败 |
| HttpMessageNotReadableException | 400 | 消息不可读 |
| HttpMessageNotWritableException | 500 | 消息不可写 |
| MethodArgumentNotValidException | 400 | 参数校验失败 |
| MissingServletRequestPartException | 400 | 请求部分缺失 |
| BindException | 400 | 绑定异常 |
| NoHandlerFoundException | 404 | 处理器未找到 |
...
| 维度 | Common Answer | Impressive Answer |
| --- | --- | --- |
| 结构性 | 只知道基本概念 | 异常类型表→自定义解析器→@ExceptionHandler |
| 技术深度 | 不知道处理哪些异常 | 清楚内置异常和自定义异常的处理 |
| 实践经验 | 没有代码示例 | 给出完整的异常处理架构 |
| 面试官印象 | 了解基础 | 掌握异常处理机制，能设计分层异常体系 |
...
| 维度 | Common Answer | Impressive Answer |
| --- | --- | --- |
| 结构性 | 只知道"定义异常类" | 分层异常→错误码规范→统一响应→全局处理 |
| 技术深度 | 不知道如何分层 | 清楚异常分层的设计原则 |
| 实践经验 | 没有代码示例 | 给出完整的异常体系和使用示例 |
| 面试官印象 | 了解基本概念 | 能设计分层异常体系，工程能力强 |
...
| 关联题 | 和本题的关系 |
| --- | --- |
| Spring @ControllerAdvice 的原理？如何实现全局异常处理？ | 全局异常处理，和异常解析器的配合 |
| HTTP 状态码的设计原则？如何选择合适的状态码？ | 错误码设计，和 HTTP 状态码的映射 |
| 如何实现异常的监控和告警？ | 异常处理，生产环境的实践 |
...
| 参数 | 说明 | 推荐值 |
| --- | --- | --- |
| maxThreads | 最大线程数 | 200-500（根据 CPU 核数） |
| minSpareThreads | 最小空闲线程数 | 10-50 |
| acceptCount | 等待队列长度 | 100-200 |
| maxConnections | 最大连接数 | 10000（NIO 模式） |
| connectionTimeout | 连接超时时间 | 30000ms |
...
| 维度 | Common Answer | Impressive Answer |
| --- | --- | --- |
| 结构性 | 只知道"调大参数" | 参数说明→调优策略→NIO 对比→性能优化实践 |
| 技术深度 | 不知道调优公式 | 清楚线程池参数的计算方法 |
| 实践经验 | 没有代码示例 | 给出完整的性能优化配置 |
| 面试官印象 | 了解基本概念 | 掌握性能优化方法，能解决实际问题 |
...
| 维度 | 优化措施 | 预期提升 |
| --- | --- | --- |
| 线程池 | Tomcat 800线程 + 业务 400线程 + LLM 1600线程 | QPS +200% |
| 连接数 | HTTP 500连接 + DB 50连接 + Redis 200连接 | 延迟 -50% |
| 序列化 | Protobuf + Kryo + FastJson2 | 序列化时间 -70% |

**3️⃣ Key Differences**

| 维度 | Common Answer | Impressive Answer |
| --- | --- | --- |
| 结构性 | 只知道"调大参数" | 线程池→连接数→序列化→监控 |
| 技术深度 | 不知道具体数值 | 清楚每个维度的优化细节 |
| 实践经验 | 没有代码示例 | 给出完整的三维优化方案 |
| 面试官印象 | 了解基本概念 | 能设计性能优化方案，解决高并发问题 |
...
| 关联题 | 和本题的关系 |
| --- | --- |
| JVM 性能调优？GC 参数如何配置？ | 整体性能优化，和 Spring MVC 的配合 |
| 数据库连接池如何调优？HikariCP 的最佳实践？ | 连接数优化，和数据库性能的关系 |
| 缓存策略如何设计？Redis 的性能优化？ | 性能优化，减少数据库压力 |
...
4. **Agent 场景**：SSE 流式输出 LLM 响应、大量并发工具调用、实时事件推送，适合 WebFlux。

**3️⃣ Key Differences**

| 维度 | Common Answer | Impressive Answer |
| --- | --- | --- |
| 结构性 | 只说了"响应式 vs 传统" | 4 个维度对比+适用场景+性能分析 |
| 技术深度 | 不知道线程模型差异 | 清楚事件循环 vs 线程池模型 |
| 实践经验 | 没有具体场景 | 有 Agent 场景的选型分析 |
| 面试官印象 | 背过概念 | 理解两种框架的适用边界 |
...
| 维度 | Common Answer | Impressive Answer |
| --- | --- | --- |
| 结构性 | 只说了 Mono/Flux 定义 | 操作符+背压机制+完整代码 |
| 技术深度 | 不知道背压策略 | 清楚 request(n) 机制和 5 种策略 |
| 实践经验 | 没有代码示例 | 有完整的背压控制示例 |
| 面试官印象 | 背过概念 | 理解响应式编程的核心机制 |
...
可以用 WebFlux 的 Flux 返回 SSE 事件。在 Controller 里返回 Flux<String>，用 MediaType.TEXT*EVENT*STREAM_VALUE。LLM 调用返回流，然后转成 Flux。
...
| 维度 | Common Answer | Impressive Answer |
| --- | --- | --- |
| 结构性 | 只说了 Flux 返回 | 完整 Controller+Service+异常处理+前端 |
| 技术深度 | 不知道 SSE 事件格式 | 清楚 ServerSentEvent 的构建 |
| 实践经验 | 没有重试机制 | 有 backoff 重试和完整错误处理 |
| 面试官印象 | 会用 WebFlux | 有生产环境完整的 SSE 方案 |
...
| 关联题 | 和本题的关系 |
| --- | --- |
| Reactor 的操作符有哪些？如何组合多个 Flux？ | Reactor 核心操作，响应式编程基础 |
| Spring WebFlux 如何实现 WebSocket？ | 另一种实时通信方式，和 SSE 对比 |
| WebFlux 如何集成 R2DBC 访问数据库？ | 响应式数据库访问，完整的响应式技术栈 |
