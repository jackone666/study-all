...
Feign 是声明式的 HTTP 客户端，用接口 + 注解定义 HTTP 请求。底层用动态代理生成实现类，然后调用。负载均衡是靠 Ribbon 或 Spring Cloud LoadBalancer 实现的。重试可以配置 Retryer。

**2️⃣ Impressive Answer**

1. **动态代理生成**：Feign 启动时扫描@FeignClient 接口，用 JDK 动态代理生成实现类，方法调用被拦截成 HTTP 请求（RequestTemplate）。

2. **负载均衡**：早期用 Ribbon（已废弃），现在用 **Spring Cloud LoadBalancer**。核心是 `LoadBalancerClient.choose(serviceId)` 从注册中心选实例，支持轮询、权重、Zone 感知等策略。

3. **重试机制**：Spring Cloud OpenFeign 默认不重试（幂等 GET 除外），可配置 `Retryer` 定义重试条件（间隔、次数、哪些异常）。但**POST/PUT 等非幂等操作要谨慎重试**，需业务侧保证幂等性。

4. **解码与编码**：请求参数用 Encoder 编码（Form/JSON），响应用 Decoder 解码（Jackson/Gson）。可自定义 Encoder/Decoder 处理特殊格式（如 Protobuf）。

**3️⃣ Key Differences**

| 维度 | Common Answer | Impressive Answer |
| --- | --- | --- |
| 结构性 | 说了代理和负载均衡但较浅 | 代理→负载均衡→重试→编解码全链路 |
| 技术深度 | 不知道 Ribbon 已废弃 | 知道 LoadBalancer 替代方案 |
| 实践经验 | 没说重试的风险 | 强调非幂等操作的重试陷阱 |
| 面试官印象 | 会用 Feign | 理解全链路，能处理复杂场景 |
...
2. **限流策略**：基于 Redis + Lua 脚本实现令牌桶/滑动窗口，按用户 ID 或 API Key 限流；超阈值返回 429。

3. **路由隔离**：Agent 相关服务单独分组（如 `agent-service`），Gateway 配置 Predicate 路由到对应服务集群。

4. **降级兜底**：配合 Sentinel 做服务降级，后端服务不可用时返回友好错误，避免 Agent 卡死。
...
Nacos 既能做注册中心也能做配置中心，Eureka 只能做注册中心。Nacos 支持 AP 和 CP 两种模式，Eureka 只有 AP 模式。Nacos 的配置中心支持动态刷新，修改配置后服务端会推送给客户端。
...
2. **配置中心原理**：客户端发起**长轮询**请求（默认 30s 超时），服务端挂起请求；配置变更时立即返回变更的 dataId，客户端再拉取最新配置。比定时轮询更实时，比 WebSocket 更轻量。

3. **健康检查差异**：Eureka 靠客户端心跳（默认 30s）；Nacos 临时实例也靠心跳，但持久实例由**服务端主动探测**（TCP/HTTP），更适合非 Java 服务。

4. **选型建议**：新项目首选 Nacos（注册 + 配置一体化）；Eureka 已停止维护（Netflix OSS 进入维护模式）。

**3️⃣ Key Differences**

| 维度 | Common Answer | Impressive Answer |
| --- | --- | --- |
| 结构性 | 只说了 AP/CP 区别 | 注册中心→配置中心→健康检查→选型，全面对比 |
| 技术深度 | 不知道 Distro 和 Raft 协议 | 清楚临时/持久实例的不同一致性协议 |
| 实践经验 | 不知道长轮询的实现原理 | 能解释配置推送的具体机制 |
| 面试官印象 | 知道 Nacos 比 Eureka 好 | 理解底层协议差异，能做技术选型 |
...
可以在 Gateway 配置不同的路由规则，根据请求头或参数判断用哪个模型服务。灰度的话可以按比例分流，比如 10% 的流量走新模型。用 Nacos 的配置中心动态修改路由规则。
...
2. **动态切换**：路由规则存 Nacos 配置中心，修改后 Gateway 通过长轮询实时感知，**无需重启**即可切换流量比例或全量切换。

3. **回滚机制**：监控灰度版本的错误率和延迟，超过阈值自动回滚到稳定版（配合 Sentinel 熔断）。

**3️⃣ Key Differences**

| 维度 | Common Answer | Impressive Answer |
| --- | --- | --- |
| 结构性 | 只说了按比例分流 | 元数据标签→三种路由策略→动态切换→回滚 |
| 技术深度 | 不知道 Nacos 元数据的用法 | 有具体的配置示例和路由策略 |
| 实践经验 | 没考虑用户维度一致性 | 用户 ID 哈希保证体验一致 |
| 面试官印象 | 知道灰度概念 | 有完整的灰度发布方案，考虑了回滚 |
...
2. **实践建议**：必选依赖用构造器注入，可选依赖用 setter 注入 + `@Autowired(required=false)`；避免字段注入（虽然代码最少，但可测试性最差）。
...
| 技术深度 | 不知道 Map 自动注入的特性 | 利用 Spring 的 Map<String, Interface> 自动收集所有实现 |
...
3. **背压（Backpressure）**：下游控制上游生产速率，避免内存溢出。Reactor 默认策略是 `onNext` 拉动式，Flux 会自动处理。

4. **Agent 场景**：流式输出（SSE）用 `Flux<ServerSentEvent<String>>`；多工具并行调用用 `Flux.mergeSequential()` 保持顺序，`Flux.combineLatest()` 合并结果。
...
| 关联题 | 和本题的关系 |
| --- | --- |
| WebFlux 如何集成 MyBatis（非响应式 ORM）？ | 阻塞 ORM 在响应式场景的线程池隔离方案，考察混合架构能力 |
| @Transactional 在 WebFlux 中如何生效？ | 响应式事务管理器（TransactionalOperator）与传统事务的区别 |
| Spring Boot 3 中 WebFlux 和 Virtual Thread 如何选型？ | 考察对 Java 21 虚拟线程与响应式编程的对比理解 |
...
2. **进阶优化**：如果某个 API 非必需（如天气），可以用 `Mono.zipDelayError()` 延迟错误处理，优先返回已完成的结果；配合 `Mono.firstWithSignal()` 实现"谁先返回用谁"的竞速模式。
