...
IoC 就是控制反转，把对象的创建交给 Spring 容器管理，不用自己 new 了。Bean 的生命周期大概就是创建、初始化、使用、销毁这几个阶段。可以用 @PostConstruct 做初始化，@PreDestroy 做销毁。
...
3. **实例化阶段**：通过构造器或工厂方法创建 Bean 实例

4. **属性赋值阶段**：Spring 对 Bean 的属性进行依赖注入（@Autowired、@Value）

5. **初始化阶段**：执行各种初始化回调（@PostConstruct、InitializingBean.afterPropertiesSet、init-method）

6. **使用阶段**：Bean 在容器中被应用程序使用

7. **销毁阶段**：容器关闭时执行销毁回调（@PreDestroy、DisposableBean.destroy、destroy-method）

8. **关键扩展点**：`BeanPostProcessor` 是最核心的扩展点，AOP 代理、@Autowired 注入、@Async 异步都是通过它实现的。理解生命周期就是理解 Spring 的扩展能力。

9. **实践注意**：`@PostConstruct` 在属性注入之后执行，可以安全使用注入的依赖；但此时 AOP 代理还未创建，不要在 `@PostConstruct` 中依赖 AOP 行为。

**3️⃣ Key Differences**

| 维度 | Common Answer | Impressive Answer |
| --- | --- | --- |
| 结构性 | 只说了"创建、初始化、销毁" | 完整链路 8 个阶段，每步都有对应机制 |
| 技术深度 | 不知道 BeanPostProcessor 的作用 | 清楚 AOP 代理在 postProcessAfterInitialization 创建 |
| 实践经验 | 没有提到扩展点 | 知道 @PostConstruct 和 AOP 的时序关系 |
| 面试官印象 | 背过概念但不深入 | 理解生命周期的设计意图和扩展能力 |
...
BeanPostProcessor 是在 Bean 初始化前后执行的，InitializingBean 是 afterPropertiesSet 方法，@PostConstruct 是注解。它们的顺序应该是 @PostConstruct 先执行，然后是 InitializingBean，最后是 BeanPostProcessor。
...
2. **BeanPostProcessor 应用**：在初始化前后对 Bean 进行增强，如 AOP 代理创建、属性校验、日志记录等

3. **InitializingBean 应用**：实现接口的 afterPropertiesSet 方法，依赖注入完成后执行必要初始化

4. **@PostConstruct 应用**：JSR-250 标准，用于依赖注入完成后的初始化逻辑，推荐使用注解而非接口

**3️⃣ Key Differences**

| 维度 | Common Answer | Impressive Answer |
| --- | --- | --- |
| 结构性 | 顺序混乱，不知道先后 | 清晰的 6 步执行链路 |
| 技术深度 | 不知道 BeanPostProcessor 的全局性 | 区分了全局增强 vs 单 Bean 初始化 |
| 实践经验 | 没有提到优先级控制 | 知道 @Order、@DependsOn 的用法 |
| 面试官印象 | 记忆混乱 | 理解执行顺序和各扩展点的定位 |
...
**1️⃣ Common Answer**
可以在 Bean 初始化方法里调用预热逻辑，比如用 @PostConstruct 注解标记一个方法，在里面创建一些测试连接来预热连接池。

**2️⃣ Impressive Answer**

1. **选择 @PostConstruct**：在 LLM 客户端 Bean 中添加 @PostConstruct 注解的方法

2. **预热逻辑**：方法中调用客户端发送少量测试请求（如健康检查），建立连接池连接

3. **异常处理**：捕获预热异常，记录日志并决定是否阻止 Bean 创建

4. **优雅降级**：预热失败时标记 Bean 为降级状态，运行时重试预热
...
| 维度 | Common Answer | Impressive Answer |
| --- | --- | --- |
| 结构性 | 简单描述，无代码示例 | 完整方案设计，包含代码实现 |
| 技术深度 | 仅提及预热概念 | 包含异常处理、降级策略 |
| 实践经验 | 无生产环境考虑 | 考虑日志、重试、降级等生产实践 |
| 面试官印象 | 基础理解，缺乏工程思维 | 具备生产环境问题解决能力 |
...
| 关联题 | 和本题的关系 |
| --- | --- |
| Spring 三级缓存与循环依赖如何解决？ | 生命周期中的实例化阶段涉及循环依赖处理 |
| @DependsOn 如何控制 Bean 加载顺序？ | 生命周期扩展点，解决依赖顺序问题 |
| Spring Bean 的作用域有哪些？Prototype 的生命周期有什么不同？ | 不同作用域的生命周期管理差异 |
...
循环依赖就是两个 Bean 互相依赖，比如 A 依赖 B，B 又依赖 A。Spring 通过三级缓存来解决，可以解决 setter 注入的循环依赖，但构造器注入不行。
...
4. **为什么需要三级**：二级缓存只能存普通对象，三级缓存可以存工厂，支持在循环依赖时提前创建 AOP 代理（如 @Async、@Transactional）。

**3️⃣ Key Differences**

| 维度 | Common Answer | Impressive Answer |
| --- | --- | --- |
| 结构性 | 只说了"三级缓存" | 三级缓存定义+解决流程+为什么需要三级 |
| 技术深度 | 不知道三级缓存各自作用 | 清楚每级缓存的作用和 AOP 代理场景 |
| 实践经验 | 不知道构造器注入不支持 | 区分了 setter 和构造器注入的差异 |
| 面试官印象 | 背过概念但不深入 | 理解循环依赖的完整解决机制 |
...
singletonObjects 存完整的 Bean，earlySingletonObjects 存提前暴露的 Bean，singletonFactories 存工厂。需要三级是因为要支持 AOP，如果只有两级就不能提前创建代理对象。
...
| 维度 | Common Answer | Impressive Answer |
| --- | --- | --- |
| 结构性 | 只说了三级缓存名字 | 每级缓存的作用+为什么需要三级 |
| 技术深度 | 不知道 AOP 代理的时机 | 清楚三级缓存支持延迟代理创建 |
| 实践经验 | 没有代码示例 | 有完整的 AOP 循环依赖流程 |
| 面试官印象 | 背过定义 | 理解三级缓存的设计意图 |
...
可以用 @Lazy 注解，延迟加载其中一个 Bean。或者重构代码，把公共依赖抽取出来，不让它们互相依赖。也可以用 ApplicationContext 手动获取 Bean。
...
| 维度 | Common Answer | Impressive Answer |
| --- | --- | --- |
| 结构性 | 只说了 @Lazy | 三种方案+完整代码+对比分析 |
| 技术深度 | 不知道事件驱动解耦 | 知道依赖倒置原则的应用 |
| 实践经验 | 没有方案对比 | 有设计权衡和推荐方案 |
| 面试官印象 | 会用但不深入 | 有架构设计能力 |
...
| 关联题 | 和本题的关系 |
| --- | --- |
| @Autowired 和 @Resource 的区别？ | 依赖注入方式，循环依赖和注入方式相关 |
| Spring Bean 的作用域有哪些？ | 不同作用域的循环依赖处理策略不同 |
| Spring 事务 @Transactional 为什么不能用于 private 方法？ | AOP 代理机制，和循环依赖中的代理创建相关 |
...
2. **Spring 循环依赖解决方案**：setter/字段注入的循环依赖靠**三级缓存**解决：
...
3. **构造器注入失效原因**：构造器执行时 Bean 还没暴露，无法注入循环依赖。解决：字段/ setter 注入，或其中一个加 `@Lazy`。

4. **最佳实践**：推荐构造器注入（不可变、易测试），循环依赖时局部用@Lazy 打破环，而不是全局改用字段注入。
...
2. **Spring 官方推荐**：Spring 4.3+ 单构造器可省略 @Autowired；Spring 团队明确推荐构造器注入作为首选方式。

3. **Agent 场景**：工具注册器需要注入多个 `ToolProvider` 实现时，用构造器注入 `List<ToolProvider>` 保证不可变性，配合 `@Qualifier` 或 `@Primary` 控制优先级。
...
3. **最佳实践**：优先使用**构造器注入**（强制依赖、不可变、便于测试），可选依赖用 Setter 注入，避免字段注入。Spring 4.3+ 单构造器可省略 @Autowired。
...
2. **Agent 场景**：LLM Provider 接口有多个实现（OpenAIProvider、ClaudeProvider、QwenProvider），通过 @Qualifier("openai") 或 @Primary 指定默认实现，避免 `NoUniqueBeanDefinitionException`。
...
3. **Agent 场景**：工具调用结果缓存时，key 使用 `toolName::hash(params)`（参数哈希避免 key 过长），value 使用 JSON 序列化，过期时间根据工具特性设置（如天气 10 分钟，搜索 1 小时）。
...
2. **缓存降级**：Redis 不可用时降级到本地缓存（Caffeine），保证服务可用性：
...
2. **Spring 的选择策略**：Spring Boot 2.x 起默认优先使用 CGLIB（`spring.aop.proxy-target-class=true`），减少因没有接口导致代理失败的问题；也可以强制指定。

3. **自调用失效是核心陷阱**：同一个 Bean 内部方法 A 调用方法 B，B 上的 AOP 不生效，因为调用绕过了代理对象，直接走 `this`；解决方案是注入自身代理（`AopContext.currentProxy()`）或拆分到另一个 Bean。

4. **@Transactional 失效场景**：private 方法不能被 CGLIB 子类重写，方法必须是 public 且非 final；另外同类自调用也会导致事务失效。
...
2. **责任链模式**：`ReflectiveMethodInvocation` 维护 interceptorList 和 currentIndex，`proceed()` 递归调用下一个拦截器。
...
2. **@Around 统一拦截**：用 `@Around("@annotation(toolAuth)")` 匹配注解，在 `ProceedingJoinPoint.proceed()` 前后分别做鉴权和耗时统计；用 `try-finally` 确保耗时日志必然记录，不受异常影响。

3. **鉴权逻辑**：proceed 前从上下文（ThreadLocal 或请求头）取 token，校验失败直接抛 `AuthException`，不执行工具；鉴权通过才放行。

4. **日志结构化**：记录工具名（`joinPoint.getSignature().getName()`）、入参摘要、耗时、是否成功，便于后续 tracing 关联；生产上配合 MDC 注入 traceId。
...
2. **异常类型问题**：默认只回滚 `RuntimeException` 和 `Error`，检查型异常（如 `IOException`）不回滚。需指定 `rollbackFor = Exception.class`。

3. **自调用问题**：同一个类内部方法调用不经过代理，事务失效。
...
2. **事务管理器配置**：多数据源时未指定 `@Transactional(transactionManager = "xxxTransactionManager")`。

3. **数据库引擎不支持**：如 MySQL 的 MyISAM 引擎不支持事务。
...
传播机制有 REQUIRED、REQUIRES*NEW、SUPPORTS、NOT*SUPPORTED、MANDATORY、NEVER、NESTED。REQUIRED 是加入当前事务，REQUIRES_NEW 是新建事务。
...
2. **典型应用**：`BeanFactoryPostProcessor` 的经典实现是 `PropertySourcesPlaceholderConfigurer`，负责解析 `${}` 占位符替换配置值；`BeanPostProcessor` 的经典实现是 `AutowiredAnnotationBeanPostProcessor`（处理 @Autowired 注入）和 `AbstractAutoProxyCreator`（创建 AOP 代理）。

3. **AOP 代理创建时机**：AOP 代理是在 `BeanPostProcessor.postProcessAfterInitialization()` 阶段创建的，这就是为什么 `BeanFactoryPostProcessor` 中注入的 Bean 不会被 AOP 代理——它们在代理创建之前就已经实例化了。

4. **使用注意**：`BeanFactoryPostProcessor` 中不要提前触发 Bean 实例化（如 getBean()），否则这些 Bean 会跳过后续的 BeanPostProcessor 处理链，导致注入、AOP 等功能失效。
...
2. **生产级考虑**：加 `@Order(Ordered.LOWEST_PRECEDENCE)` 确保在 AOP 代理之后执行（扫描的是代理后的 Bean）；工具注册表支持热更新，配合配置中心动态启停工具。
...
2. **访问权限限制**：@Transactional 只能标注在 public 方法上。protected/private 方法即使加上也不会生效，因为 Spring AOP 默认只代理 public 方法。

3. **异常类型不匹配**：默认只回滚 RuntimeException 和 Error。如果抛的是受检异常（Checked Exception），需要显式指定 `@Transactional(rollbackFor = Exception.class)`。

4. **捕获异常未抛出**：方法内部 try-catch 把异常吞了，事务管理器感知不到异常，自然不会回滚。解决：catch 后手动调用 `TransactionAspectSupport.currentTransactionStatus().setRollbackOnly()`。

5. **异步方法失效**：@Async 方法的事务 propagation 可能不符合预期，因为异步方法在另一个线程执行，事务上下文无法传递。解决：在异步方法上单独加@Transactional。
...
2. **跨服务 Saga 模式**：Agent 多步工具调用涉及多个微服务时，用 Saga 编排：每个步骤有正向操作和补偿操作，步骤 3 失败则逆向执行步骤 2→1 的补偿。Spring Cloud State Machine 或自研引擎可实现。

3. **幂等 + 重试**：每个工具调用设计幂等性（唯一键/状态机），失败后安全重试；配合**本地消息表 + 事务性轮询**，确保补偿操作必然执行。

4. **用户感知设计**：长事务不阻塞 HTTP 响应，采用异步回调或 SSE 推送执行进度，用户可随时查看中间状态。
...
2. **TransactionSynchronizationManager 的核心作用**：通过 ThreadLocal 维护当前线程的事务资源（Connection、Session 等）和同步回调。`@Transactional` 方法内所有 DAO 操作通过 `DataSourceUtils.getConnection()` 获取的都是**同一个 Connection**，这就是事务生效的根本原因。

3. **事务同步回调**：`TransactionSynchronization` 接口提供 `beforeCommit()`、`afterCommit()`、`afterCompletion()` 等钩子。典型应用：事务提交后发消息（避免事务未提交就发消息导致消费者查不到数据）。
...
可以把每一步都用 REQUIRES\_NEW 传播级别，这样每步都是独立事务，失败了不影响前面的。或者用 try-catch 把每步包起来，失败了记录日志继续执行。
...
| 维度 | Common Answer | Impressive Answer |
| --- | --- | --- |
| 方案数量 | 只说了 REQUIRES\_NEW | 三种方案对比，各有适用场景 |
...
2. **循环依赖解决流程**：A 创建时发现依赖 B → 暴露 A 的 ObjectFactory 到三级缓存 → B 创建时发现依赖 A → 从三级缓存获取 A 的 ObjectFactory → 生成 A 的早期引用（如果是 AOP 则生成代理）→ 放入二级缓存 → B 完成初始化 → A 完成初始化 → A 从二级缓存移到一级缓存。

3. **为什么需要三级而非两级**：如果 A 需要被 AOP 代理，代理对象必须在实例化后、初始化前创建。三级缓存中的 ObjectFactory 延迟执行，只有真正发生循环依赖时才创建代理，避免无循环依赖时也创建代理的性能浪费。两级缓存无法做到这个延迟创建。

4. **Prototype 无法解决**：Prototype 作用域的 Bean 不缓存，每次都创建新实例，Spring 不解决其循环依赖，会直接抛异常。
...
2. **@Lazy 的解决原理**：注入的不是真实 Bean，而是一个 CGLIB 生成的代理对象。首次调用代理方法时，代理才从 Spring 容器获取真实 Bean。这样在构造阶段只需要一个代理占位，不触发真实 Bean 的创建，打破了循环依赖。
...
4. **最佳实践**：优先通过重构消除循环依赖（提取中间层、事件驱动），而非用 @Lazy 绕过。@Lazy 只是技术手段，治标不治本，可能掩盖设计问题。
...
2. **obtainFreshBeanFactory()**：创建 BeanFactory，加载 BeanDefinition（解析 @Component、@Bean 等）。

3. **prepareBeanFactory()**：配置 BeanFactory，注册 BeanPostProcessor（如 ApplicationContextAwareProcessor）、设置类加载器。

4. **postProcessBeanFactory()**：子类扩展点，可以修改 BeanFactory（如添加 Web 环境特有的 Scope）。

5. **invokeBeanFactoryPostProcessors()**：执行 BeanFactoryPostProcessor（如 @PropertySource、@ConfigurationClassPostProcessor 解析配置类）。

6. **registerBeanPostProcessors()**：注册 BeanPostProcessor（如 AutowiredAnnotationBeanPostProcessor、CommonAnnotationBeanPostProcessor）。

7. **initMessageSource()**：初始化国际化资源（MessageSource）。

8. **initApplicationEventMulticaster()**：初始化事件广播器（ApplicationEventMulticaster）。

9. **onRefresh()**：子类扩展点，创建 Web 容器（如 Tomcat、Jetty）。

10. **registerListeners()**：注册监听器，广播早期事件。

11. **finishBeanFactoryInitialization()**：初始化所有非懒加载的单例 Bean（实例化、属性注入、初始化回调）。

12. **finishRefresh()**：完成刷新，发布 ContextRefreshedEvent，启动生命周期 Bean（如 SmartLifecycle）。
...
14. **Agent 场景**：在 `onRefresh()` 步骤启动 Agent Server（如 gRPC Server），在 `finishRefresh()` 后发布 `AgentReadyEvent`，通知下游服务 Agent 已就绪。
...
2. **Agent 场景实战**：开发环境用 `@Profile("dev")` + Mock LLM + 本地向量库，测试环境用真实 LLM + 测试向量库，生产环境用企业级 LLM + 云向量库，通过 `@ConditionalOnProperty` 控制审计日志、计费等功能的开关。
...
2. **单例模式**：Bean 默认 Singleton 作用域，通过三级缓存（`singletonObjects`）保证全局唯一。不是传统的私有构造器单例，而是容器级别的单例。

3. **代理模式**：AOP 的核心，JDK 动态代理（接口）和 CGLIB（类）。`@Transactional`、`@Async`、`@Cacheable` 都是通过代理实现的。

4. **模板方法模式**：`JdbcTemplate`、`RestTemplate`、`TransactionTemplate`——定义算法骨架，子类/回调实现具体步骤。

5. **观察者模式**：`ApplicationEvent` + `ApplicationListener`，事件发布-订阅机制，实现组件间松耦合通信。

6. **策略模式**：`Resource` 接口的多种实现（`ClassPathResource`、`FileSystemResource`、`UrlResource`），根据前缀自动选择策略。

7. **适配器模式**：`HandlerAdapter` 适配不同类型的 Controller（注解式、实现 Controller 接口式），统一调用接口。
...
2. **默认同步执行**：`SimpleApplicationEventMulticaster` 默认在发布线程中依次调用所有监听器。这意味着监听器抛异常会影响发布者，耗时操作会阻塞主流程。
...
4. **事务事件**：`@TransactionalEventListener(phase = AFTER_COMMIT)` 保证事务提交后才触发监听器，避免"事务未提交，消费者查不到数据"的问题。这是发消息场景的最佳实践。

5. **执行顺序**：多个监听器通过 `@Order` 控制顺序；同步场景下按 Order 值从小到大执行。
...
3. **核心优势**：工具调用逻辑和审计逻辑**完全解耦**。新增审计需求（如合规审计、成本统计）只需加一个监听器，不修改任何已有代码，符合开闭原则。

4. **生产考虑**：异步监听器配置独立线程池，避免审计任务影响业务线程；监听器内部做好异常处理和重试，保证审计数据不丢失。
...
2. **Agent 场景实战**：Agent 调用工具后，通过事件机制通知日志系统记录调用链、计费系统计算费用、审计系统记录操作，工具执行代码保持简洁，易于维护。
...
2. **spring.factories 示例**（Spring Boot 2.7 之前）：
...
2. **Agent 场景实战**：自定义 Agent Starter，通过自动配置让用户只需引入依赖和配置 `agent.enabled=true`，就能开箱即用 Agent 功能。
...
1. **PropertySource 加载优先级**（从高到低）：
  ...
  5. JVM 系统属性（`System.getProperties()`）

  6. 操作系统环境变量

  7. JAR 包外的 `application-{profile}.properties`

  8. JAR 包内的 `application-{profile}.properties`

  9. JAR 包外的 `application.properties`

  10. JAR 包内的 `application.properties`
...
2. **Agent 场景实战**：Agent 的 LLM 配置（模型名、temperature、API Key）通过 Nacos 配置中心管理，使用 `@RefreshScope` 实现动态热更新，无需重启服务。
...
2. **注意事项**：默认单线程执行，上一个任务未完成时下一个任务会阻塞。需要并行执行时配置线程池。
...
2. **Agent 场景实战**：Agent 执行完成后，异步保存日志到数据库（`@Async("agentExecutor")`），异步发送通知（`@Async("notificationExecutor")`），两者使用不同的线程池隔离，避免互相影响。
...
2. **@ControllerAdvice**：全局异常处理注解，标注在类上，该类中的 `@ExceptionHandler` 方法会应用到所有 Controller。
...
2. **精确匹配优先**：`@ExceptionHandler` 会优先匹配最具体的异常类型。例如：
...
2. **@ConfigurationProperties**：批量注入配置到 Bean，类型安全，支持嵌套对象，适合复杂配置。

3. **Environment**：编程式读取配置，灵活但繁琐，适合动态读取。
...
2. **声明式事务（@Transactional）**：通过注解声明事务，Spring AOP 自动管理事务，代码更简洁。
...
2. **@TransactionalEventListener（Spring 4.2+）**：
...
2. **ApplicationContext**：是 BeanFactory 的**子接口**，在基础功能上做了大量增强：
...
3. **选择建议**：开发中优先使用 ApplicationContext（如 ClassPathXmlApplicationContext、AnnotationConfigApplicationContext），BeanFactory 主要在框架内部使用。
...
2. **实践注意**：如果容器启动慢，可以检查是否有过多的 Bean 在步骤⑪被预实例化，或者 BeanPostProcessor 执行时间过长。
...
2. **实践建议**：优先使用 `ApplicationRunner`，参数更友好；如果需要监听父子容器事件，用 `@EventListener`。
...
3. **解决原理**：在创建 Bean 时，先将半成品的 Bean（属性未注入）暴露出去，让其他 Bean 可以引用，等所有 Bean 创建完成后再注入属性。
...
2. **AOP 代理的时机**：如果 A 被 `@Async`、`@Transactional` 等注解标注，且发生了循环依赖，A 的代理对象会在 `getEarlyBeanReference` 中提前创建，而不是在 `BeanPostProcessor.postProcessAfterInitialization`。

3. **实践注意**：如果项目中没有 AOP 或循环依赖，三级缓存其实可以简化为两级（Spring 源码注释也提到过）。
