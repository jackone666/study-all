...
- 查询优化新特性
- Agent 场景应用
- 降序索引（Descending Index）：支持 CREATE INDEX idx ON table(a DESC, b ASC)，避免 ORDER BY a DESC, b...
- 不可见索引（Invisible Index）：CREATE INDEX idx_name ON table(col) INVISIBLE...
...
<td>
深入 ReadView 可见性算法、Next-Key Lock 机制
</td>
</tr>
<tr>
<td>
...
<tr>
<td>
死锁检测与处理（innodb<em>deadlock</em>detect）
</td>
<td>
并发事务中的锁冲突问题
</td>
...
<td>
答：意向锁（IS/IX）；WiredTiger 乐观并发控制；db.currentOp() 排查锁等待；Agent 场景应用
</td>
</tr>
<tr>
<td>
...
1. **Buffer Pool内存管理**：LRU链表（分young区和old区，防止全表扫描污染热数据）、Free链表（空闲页）、Flush链表（脏页）

1. **脏页刷新机制**：后台线程定期刷新、redo log写满时强制刷新、Buffer Pool不足时淘汰脏页

1. **Change Buffer**：对非唯一二级索引的写操作先缓存到Change Buffer，后台合并，减少随机IO

1. **Agent场景实践**：Agent高频写入会话记录时，合理设置`innodb_buffer_pool_size`（建议物理内存的60-80%），减少磁盘IO
...
<td>
深入LRU分区、脏页刷新策略、Change Buffer机制
</td>
</tr>
<tr>
<td>
...
<td>
结合Agent场景给出buffer<em>pool</em>size配置建议
</td>
</tr>
<tr>
<td>
...
1. **MyISAM适用场景**：只读或读多写少、不需要事务（历史统计报表）、全文检索（5.6前）

1. **Memory引擎适用场景**：临时数据（Agent执行中间状态缓存）、高频读写的小数据集（在线用户状态表）、数据丢失可接受

1. **Agent场景选型实践**：会话表/知识库文档→InnoDB；模型调用统计临时聚合→Memory；历史归档只读数据→MyISAM
...
<tr>
<td>
Buffer Pool与redo log的关系（WAL机制）
</td>
<td>
存储引擎的持久化机制
</td>
<td>
答：这题可以按“定义 → 核心机制 → 工程落地”三步答；结合本题重点强调：存储引擎的持久化机制，最后补一个风险点或优化手段。
</td>
</tr>
<tr>
<td>
...
1. **Next-Key Lock**：Record Lock + Gap Lock，锁定记录本身及其左侧间隙，是InnoDB默认的行锁算法
...
1. **示例**：表中有id=1,5,10，查询WHERE id=7（不存在），加Gap Lock(5,10)；查询WHERE id>5，加Next-Key Lock(5,10]和(10,+∞)

1. **Agent场景实践**：会话表并发插入时，如果按user*id范围查询加了间隙锁，其他事务插入同user*id的新会话会被阻塞，需要合理设计索引避免大范围间隙锁
...
1. **降低隔离级别**：从RR降为RC，RC级别不使用Gap Lock，大幅减少锁冲突（但需评估幻读影响）

1. **分区写入**：按user_id hash分区，不同用户的写入操作互不干扰

1. **乐观锁替代悲观锁**：会话状态更新使用version字段CAS，避免SELECT FOR UPDATE的锁等待

1. **批量写入优化**：Agent执行日志采用批量INSERT，减少锁获取次数
...
<td>
深入理解Record Lock退化机制、RC vs RR的锁差异
</td>
</tr>
<tr>
<td>
...
<td>
答：加锁范围确定规则；等值查询命中记录：退化为Record Lock；等值查询未命中：退化为Gap Lock（锁定查询值所在的间隙）；范围查询：Next-Key Lock锁定范围内所有记录及间隙
</td>
</tr>
</table>
...
1. 从库IO线程连接主库，请求binlog

1. 主库dump线程推送binlog给从库

1. 从库IO线程写入relay log

1. 从库SQL线程读取relay log并重放SQL
...
1. **半同步复制（Semi-sync）**：主库等待至少一个从库确认收到binlog后才返回，保证至少一个从库有数据，但增加写入延迟（通常1-10ms）

1. **MGR（MySQL Group Replication）**：基于Paxos协议的分布式一致性，多主或单主模式，自动故障转移，数据强一致，适合金融级场景
...
1. **Agent场景实践**：Agent平台推荐半同步复制，在数据安全和性能间取得平衡；核心计费模块可考虑MGR
...
1. **强制读主库策略**：写操作后的读请求路由到主库（如：Agent创建会话后立即查询会话详情）

1. **会话一致性**：同一用户会话内的读请求路由到同一节点（Session Consistency）

1. **半同步+读主库**：对强一致性要求高的操作（Token扣减后查余额）强制读主库

1. **业务层容忍**：对延迟不敏感的读操作（历史会话列表、统计报表）允许读从库，接受最终一致性

1. **Agent场景实践**：会话创建→写主库；会话详情查询→读主库（强一致）；历史会话列表→读从库（最终一致）
...
<tr>
<td>
binlog格式（ROW/STATEMENT/MIXED）对复制的影响
</td>
<td>
主从复制的数据一致性机制
</td>
...
<td>
答：这题可以按“定义 → 核心机制 → 工程落地”三步答；结合本题重点强调：主从复制的故障恢复和切换，最后补一个风险点或优化手段。
</td>
</tr>
</table>
...
1. **Agent场景实践**：知识库文档按tenant_id分库，同租户文档在同一库，避免跨分片查询；文档ID使用雪花算法生成
...
<td>
深入理解Seata AT/TCC、Sharding-JDBC聚合机制
</td>
</tr>
<tr>
<td>
...
1. **扩容方案**：一致性哈希减少数据迁移量，或预先分配足够多的逻辑分片（如256个），扩容时只迁移部分分片

1. **数据隔离**：行级隔离（所有表加tenant_id字段）vs 库级隔离（大租户独立库），根据安全要求选择
...
<tr>
<td>
Sharding-JDBC vs MyCat的选型
</td>
<td>
分库分表中间件的选择
</td>
<td>
答：这题可以按“定义 → 核心机制 → 工程落地”三步答；结合本题重点强调：分库分表中间件的选择，最后补一个风险点或优化手段。
</td>
</tr>
<tr>
<td>
...
1. **连接数计算**：HikariCP建议公式：connections = (core*count × 2) + effective*spindle_count，避免过多连接导致上下文切换

1. **Agent场景实践**：Agent并发推理时，每个请求可能触发多次DB查询，需要合理设置maximumPoolSize，避免连接等待成为瓶颈
...
<td>
深入理解leakDetectionThreshold、长事务影响
</td>
</tr>
<tr>
<td>
...
1. **读写分离连接池**：主库连接池（写操作，较小）+ 从库连接池（读操作，较大），分别配置

1. **异步化减少连接占用**：使用异步查询（CompletableFuture + 线程池），减少单个请求的连接持有时间

1. **连接池监控**：接入Prometheus + Grafana，监控activeConnections趋势，提前预警

1. **Agent场景实践**：Agent推理时的DB查询尽量批量化（批量查知识库文档），减少连接获取次数；非关键路径查询异步化
...
<tr>
<td>
MySQL最大连接数（max_connections）与连接池的关系
</td>
<td>
数据库层面的连接限制
</td>
...
<td>
答：这题可以按“定义 → 核心机制 → 工程落地”三步答；结合本题重点强调：应用层面的并发控制机制，最后补一个风险点或优化手段。
</td>
</tr>
</table>
---
...
| --- | --- |
| MySQL 索引的数据结构为什么选 B+ 树而不是 B 树或哈希？ | B+ 树非叶子节点只存键，扇出高、树高低，查询磁盘 IO 少；叶子节点有序并用链表连接，天然支持范围查询和排序；哈希索引等值查询快但不支持范围，B 树叶子和非叶子都存数据，范围扫描和缓存效率不如 B+ 树。 |
| 联合索引的最左前缀原则和索引下推（ICP）是什么？如何做索引优化？ | 深入 ICP 原理、跳跃扫描、覆盖索引机制；结合 EXPLAIN 分析、Agent 场景索引设计。 |
| 慢 SQL 如何排查和优化？EXPLAIN 各字段怎么看？ | 开启慢查询日志：SET GLOBAL slow_query_log = ON，SET GLOBAL long_query_time = 2（超过 2 秒记录）；分析慢查询日志：使用 mysqldumpslow -s t -t 10 /path/to/slow.log，按执行时间排序取 Top 10；实时监控：使用 SHOW PROCESSLIST 或 SELECT * FROM information_schema.processlist WHERE TIME > 5 查看当前执行中的慢查询；Performance Schema：SELECT * FROM performance_schema.events_statements_summary_by_digest ORDER BY SUM_TIMER_WAIT DESC LIMIT 10，按语句类型统计耗时；id：查询序号，id 越大越先执行，id 相同从上到下执行。 |
| MySQL 8.0 有哪些重要的新特性？对索引和查询优化有什么影响？ | 降序索引（Descending Index）：支持 CREATE INDEX idx ON table(a DESC, b ASC)，避免 ORDER BY a DESC, b ASC 需要额外的 filesort；不可见索引（Invisible Index）：CREATE INDEX idx_name ON table(col) INVISIBLE，索引对优化器不可见，用于测试索引效果或安全删除索引；函数索引（Functional Index）：支持 CREATE INDEX idx ON table((YEAR(create_time)))，解决索引列函数操作失效问题；窗口函数（Window Functions）：支持 ROW_NUMBER()、RANK()、DENSE_RANK()、LAG()、LEAD() 等，替代复杂的自连接子查询；示例：SELECT user_id, amount, ROW_NUMBER() OVER (PARTITION BY user_id ORDER BY amount DESC) AS rank FROM orders。 |
| MySQL 的四种事务隔离级别分别是什么？ | 四种隔离级别是 Read Uncommitted、Read Committed、Repeatable Read 和 Serializable；隔离越高并发越低，分别解决脏读、不可重复读和幻读问题，MySQL InnoDB 默认 Repeatable Read，并通过 MVCC 和间隙锁降低幻读风险。 |
| MVCC 的实现原理是什么？RR 级别下如何解决幻读？ | 隐藏字段：每行数据包含 trx_id（事务ID）和 roll_pointer（回滚指针）；undo log 版本链：数据修改时，旧版本写入 undo log，通过 roll\_pointer 形成版本链，实现数据的多版本管理；ReadView：包含四个关键字段：m_ids（活跃事务ID列表）、min_trx_id（最小活跃事务ID）、max_trx_id（下一个要分配的事务ID）、creator_trx_id（当前事务ID）；RC 级别：每次 SELECT 都生成新的 ReadView，能看到其他事务已提交的修改；RR 级别：只在第一次 SELECT 时生成 ReadView，后续查询复用，确保可重复读。 |
| MySQL 死锁是怎么产生的？如何检测和处理？ | 互斥条件：资源不能被多个事务同时占用；请求与保持条件：事务持有资源的同时请求其他资源；不剥夺条件：资源不能被强制释放，只能由持有者主动释放；循环等待条件：多个事务形成资源等待的环形链；自动检测：InnoDB 的 innodb_deadlock_detect = ON（默认开启），后台线程定期检测死锁。 |
| binlog、redo log、undo log 各自的作用是什么？两阶段提交如何保证一致性？ | 核心思想：先写日志，再写数据页；优势：① 随机写改为顺序写，提升性能；② 崩溃后通过日志恢复，避免数据丢失；redo log 采用循环写入，写满后覆盖旧日志，通过 checkpoint 机制标记已持久化的数据页；Prepare 阶段：事务执行完毕，InnoDB 写入 redo log 并标记为 prepare 状态；Binlog 写入：MySQL 服务器层写入 binlog。 |
| InnoDB 和 MyISAM 的核心区别是什么？ | 事务支持：InnoDB支持事务（ACID），MyISAM不支持；锁粒度：InnoDB支持行锁，并发性能好；MyISAM只支持表锁，高并发时锁冲突严重；外键：InnoDB支持外键约束，MyISAM不支持；崩溃恢复：InnoDB有崩溃恢复能力（通过redo log），MyISAM崩溃后需要修复；全文索引：6前只有MyISAM支持全文索引，5.6后InnoDB也支持。 |
| InnoDB 的表空间结构（页/区/段）是怎样的？Buffer Pool 如何管理内存？ | 表空间物理结构：页（16KB，最小IO单位）→ 区（64个页=1MB）→ 段（多个区组成，分叶子节点段和非叶子节点段）；Buffer Pool内存管理：LRU链表（分young区和old区，防止全表扫描污染热数据）、Free链表（空闲页）、Flush链表（脏页）；脏页刷新机制：后台线程定期刷新、redo log写满时强制刷新、Buffer Pool不足时淘汰脏页。 |
| Agent 场景下如何选择存储引擎？什么时候考虑 Memory 引擎？ | InnoDB适用场景：需要事务（会话记录、Token计费）、高并发写入（Agent执行日志）、数据安全要求高；MyISAM适用场景：只读或读多写少、不需要事务（历史统计报表）、全文检索（5.6前）；Memory引擎适用场景：临时数据（Agent执行中间状态缓存）、高频读写的小数据集（在线用户状态表）、数据丢失可接受。 |
| 行锁、表锁、意向锁（IS/IX）分别是什么？ | 行锁：锁定单行记录，InnoDB支持，并发性能好；表锁：锁定整张表，MyISAM默认，InnoDB也支持（如DDL操作）；意向锁：InnoDB特有的表级锁，分为意向共享锁（IS）和意向排他锁（IX），用于协调行锁和表锁，避免锁冲突。意向锁是自动加的，不需要手动申请。 |
| 间隙锁（Gap Lock）和 Next-Key Lock 的加锁范围如何确定？ | 等值查询命中记录：退化为Record Lock；等值查询未命中：退化为Gap Lock（锁定查询值所在的间隙）；范围查询：Next-Key Lock锁定范围内所有记录及间隙。 |
| Agent 并发写入会话记录时，如何通过索引设计减少锁冲突？ | 精确索引减少锁范围：会话表建立(user*id, session*id)联合唯一索引，等值查询退化为Record Lock，避免Gap Lock；降低隔离级别：从RR降为RC，RC级别不使用Gap Lock，大幅减少锁冲突（但需评估幻读影响）；分区写入：按user_id hash分区，不同用户的写入操作互不干扰。 |
| MySQL 主从复制的基本流程是什么？ | 从库SQL线程读取relay log并重放SQL。 |
| 半同步复制 vs 异步复制 vs MGR（组复制）有什么区别？ | 数据安全：异步<半同步<MGR；写入性能：MGR<半同步<异步；运维复杂度：异步<半同步<MGR。 |
| Agent 平台读写分离架构下，如何处理主从延迟导致的数据不一致？ | 延迟监控：通过Seconds*Behind*Master监控延迟，超过阈值告警；强制读主库策略：写操作后的读请求路由到主库（如：Agent创建会话后立即查询会话详情）；会话一致性：同一用户会话内的读请求路由到同一节点（Session Consistency）。 |
| 什么时候需要分库分表？水平分表和垂直分表的区别？ | 什么时候需要：单表数据量超过千万、单库连接数瓶颈、单机IO瓶颈；水平分表：按行拆分，数据结构相同，如按user_id取模分表；垂直分表：按列拆分，将大表拆成小表，如将大字段、冷热数据分离。 |
| 分库分表后如何解决跨分片查询、分布式事务、全局 ID 问题？ | 避免跨分片：业务设计时尽量让相关数据在同一分片（按user_id分片，同一用户数据在同一库）；中间件聚合：Sharding-JDBC在应用层做结果合并，支持跨分片ORDER BY、GROUP BY、LIMIT；冗余存储：对需要全局查询的字段建立宽表（ES或ClickHouse），分库分表存明细，宽表存聚合；Seata AT模式：自动生成undo log，两阶段提交，对业务侵入小；TCC模式：Try-Confirm-Cancel，性能更好但需要业务实现补偿逻辑。 |
| Agent 知识库多租户场景，如何设计分库分表策略？ | 大租户：独立库（VIP租户独占资源，数据隔离强）；中小租户：按tenant_id hash分片，多租户共享分片；文档表：document*id（雪花算法）、tenant*id（分片键）、content、embedding_id；分片规则：tenant_id % 16 → 16个分片。 |
| 数据库连接池的作用是什么？常见参数有哪些？ | 作用：复用数据库连接，避免频繁创建/销毁连接的开销，控制并发连接数；常见参数：maximumPoolSize（最大连接数）、minimumIdle（最小空闲连接）、connectionTimeout（连接超时）、idleTimeout（空闲超时）、maxLifetime（连接最大生命周期）。 |
| 如何诊断和优化 MySQL 连接池耗尽问题？ | 监控连接池状态：HikariCP的activeConnections、idleConnections、pendingThreads指标；查看MySQL连接：SHOW PROCESSLIST查看当前连接状态，识别长时间运行的查询；连接泄漏检测：开启HikariCP的leakDetectionThreshold（如30秒），打印泄漏堆栈；慢查询占用连接：优化SQL，添加索引，设置statement_timeout；连接泄漏：确保Connection在finally块或try-with-resources中关闭。 |
| Agent 高并发推理场景下，连接池如何配置才能避免成为瓶颈？ | maximumPoolSize：根据公式计算，通常10-20，不要盲目调大；minimumIdle：设置为maximumPoolSize的50%，保持预热连接；connectionTimeout：3000ms（3秒），超时快速失败而非无限等待；idleTimeout：600000ms（10分钟），及时释放空闲连接；maxLifetime：1800000ms（30分钟），定期重建连接防止连接老化。 |
