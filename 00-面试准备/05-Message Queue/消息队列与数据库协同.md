...
1️⃣ Common Answer
RocketMQ 事务消息就是两阶段提交。第一阶段发送半消息到 Broker，Broker 收到后存起来但不投递给消费者。然后执行本地事务，如果本地事务成功就提交半消息，失败就回滚。如果 Broker 收不到确认，就会回查本地事务状态，根据结果决定提交还是回滚。这样就能保证消息不丢失。
...
2. **主从同步**：配置 `asyncMaster` 或 `syncMaster`，保证消息在从机同步后再返回成功

3. **消息重试**：消费者消费失败时自动重试（默认 16 次），可配置重试策略

4. **死信队列**：重试失败的消息进入死信队列，人工介入处理

5. **事务回查兜底**：即使第一阶段成功但第二阶段失败，回查机制也能保证最终一致性

3️⃣ Key Differences

| 维度 | Common Answer | Impressive Answer |
| --- | --- | --- |
| 技术深度 | 只知道两阶段提交概念 | 能详细说明半消息机制、回查机制、状态流转 |
| 实践经验 | 缺乏异常场景考虑 | 能说出网络中断、应用崩溃等异常处理方案 |
| 思考维度 | 仅关注流程正确性 | 关注消息可靠性、性能优化、生产实践 |
| 表达方式 | 平铺直叙 | 用时序图、代码示例辅助说明 |
| 面试官印象 | 基础掌握，缺乏深度 | 理解透彻，有生产实战经验 |
...
1️⃣ Common Answer
用 RocketMQ 事务消息就行了。发送半消息，然后执行订单插入，成功了就提交消息，库存系统消费消息扣库存。如果消费失败就重试，保证最终一致。
...
| 异常场景 | 处理方案 |
| --- | --- |
| 订单插入成功，但 Commit 指令丢失 | Broker 回查订单表状态，发现订单成功，提交消息 |
| 订单插入失败，但 Rollback 指令丢失 | Broker 回查订单表状态，发现订单不存在，回滚消息 |
| 库存服务消费失败（网络异常） | RocketMQ 自动重试（默认 16 次），直到成功 |
| 库存服务消费失败（业务异常） | 重试多次后进入死信队列，人工介入处理 |
| 消息重复投递 | 库存服务通过唯一索引保证幂等性 |
| 库存服务消费成功但 ACK 丢失 | RocketMQ 会重新投递，幂等性设计保证不会重复扣减 |

**生产环境优化建议**

1. **消息轨迹追踪**：为每个消息设置唯一的 `messageId`，便于问题排查

2. **监控告警**：监控半消息堆积量、回查次数、死信队列消息数

3. **降级策略**：如果 MQ 不可用，降级为定时任务轮询订单表通知库存

4. **性能优化**：批量消费消息，减少网络开销

3️⃣ Key Differences

| 维度 | Common Answer | Impressive Answer |
| --- | --- | --- |
| 技术深度 | 简单说用事务消息 | 详细设计生产者、消费者、幂等性方案 |
| 实践经验 | 缺乏异常场景考虑 | 考虑了网络中断、重复消费、消费失败等场景 |
| 思考维度 | 仅关注功能实现 | 关注可靠性、可维护性、监控告警 |
| 表达方式 | 口头描述 | 用时序图、代码示例、表结构设计说明 |
| 面试官印象 | 了解基本概念 | 有完整的生产实战经验 |
...
1️⃣ Common Answer
本地消息表就是建一张表存消息，订单插入的时候同时插入消息记录，用同一个事务。然后定时任务扫描状态为待发送的消息，发送成功后更新状态。如果发送失败就重试，直到成功为止。
...
1️⃣ Common Answer
用本地消息表，支付成功后插入多条消息记录，然后定时任务发送到不同的 MQ topic。如果某个下游失败了就重试，直到成功。
...
1️⃣ Common Answer
幂等性就是用数据库唯一索引，或者用 Redis 存一个 key 表示已经处理过。消费的时候先查一下，如果已经处理过就跳过。
...
| 维度 | Common Answer | Impressive Answer |
| --- | --- | --- |
| 技术深度 | 简单说用 Redis 或数据库 | 详细设计 4 种方案，每种都有完整代码 |
| 实践经验 | 缺乏方案对比和场景选择 | 能根据不同场景选择合适的方案，有对比表格 |
| 思考维度 | 仅关注功能实现 | 关注可靠性、性能、复杂度的权衡 |
| 表达方式 | 口头描述 | 用代码示例、SQL、对比表格详细说明 |
| 面试官印象 | 了解基本概念 | 有完整的架构设计能力 |
...
1️⃣ Common Answer
用 Redis 存一个 key，消费的时候先查一下，如果已经处理过就跳过。或者用数据库唯一索引，插入一条积分记录，如果重复就报错。
...
| 异常场景 | 处理方案 |
| --- | --- |
| 消息重复投递 | 第一重：数据库唯一索引拦截<br/>第二重：查询订单状态二次确认 |
| 并发重复消费 | 唯一索引保证只有一个消费者能插入成功，其他消费者抛出 DuplicateKeyException |
| 积分增加失败 | 抛出异常，触发 RocketMQ 重试 |
| 积分明细已插入但用户积分未增加 | RocketMQ 重试后，查询订单状态发现 status=0，重新增加积分 |
| 积分明细已插入且用户积分已增加，但 ACK 丢失 | RocketMQ 重新投递，唯一索引冲突，查询状态确认已生效 |
| 数据库唯一索引冲突但状态未生效 | 异常情况，发送告警，人工介入 |
...
| 维度 | Common Answer | Impressive Answer |
| --- | --- | --- |
| 技术深度 | 简单说用 Redis 或数据库 | 详细设计双重幂等性保障，考虑并发、异常场景 |
| 实践经验 | 缺乏异常场景考虑 | 考虑了并发重复消费、状态异常、监控告警 |
| 思考维度 | 仅关注功能实现 | 关注业务幂等性、技术幂等性、可维护性 |
| 表达方式 | 口头描述 | 用代码示例、SQL、异常处理表格、测试用例 |
| 面试官印象 | 了解基本概念 | 有完整的生产实战经验 |
...
| 关联题 | 和本题的关系 |
| --- | --- |
| RocketMQ 消息重复投递的原因 | 重复投递是幂等性问题的根源 |
| 分布式事务的解决方案 | 事务消息和本地消息表都需要考虑幂等性 |
| 数据库唯一索引的使用 | 唯一索引是实现幂等性的重要手段 |
| Redis 分布式锁的实现 | 分布式锁是实现幂等性的另一种方案 |
| 如何设计高可用的积分系统 | 幂等性是高可用系统的基础 |
...
1️⃣ Common Answer
STATEMENT 记录 SQL 语句，ROW 记录每一行的变化，MIXED 是混合模式。STATEMENT 节省空间但可能有问题，ROW 准确但空间大，MIXED 是折中方案。
...
1️⃣ Common Answer
用 ROW 格式，因为 ROW 记录每一行的变化，比较准确。STATEMENT 可能有问题，MIXED 不太好控制。
...
| 关联题 | 和本题的关系 |
| --- | --- |
| MySQL 主从复制的原理 | Binlog 是主从复制的基础 |
| Canal 的架构原理 | Canal 基于 Binlog 实现 CDC |
| 如何保证数据一致性 | ROW 格式是保证数据一致性的关键 |
| Elasticsearch 的数据同步方案 | Binlog + Canal 是常用的同步方案 |
...
1️⃣ Common Answer
Canal 就是模拟 MySQL Slave，向 Master 发送 dump 请求，Master 推送 Binlog 给 Canal，Canal 解析后发送到 MQ。为了保证数据不丢失，Canal 会记录消费位点，如果消费失败可以重试。
...
1️⃣ Common Answer
用 Canal 监听 MySQL 的 Binlog，解析订单表的变更，然后发送到 RocketMQ，消费者消费消息后更新到 ES。如果消费失败就重试。
...
4. **索引分区**：按时间分区索引，如 `orders_202403`、`orders_202404`
...
1️⃣ Common Answer
用 Canal 监听 MySQL 的 Binlog，解析变更后发送到 RocketMQ，然后有两个消费者，一个更新 ES，一个更新缓存。如果消费失败就重试，保证最终一致性。
...
1️⃣ Common Answer
用 Canal 监听订单表的变更，状态变更后发送消息到 MQ，消费者更新缓存。如果缓存更新失败就重试，保证最终一致性。
...
| 关联题 | 和本题的关系 |
| --- | --- |
| 缓存一致性方案 | 延迟双删是缓存一致性的重要方案 |
| Canal 的架构原理 | Canal 是实现缓存同步的关键组件 |
| Redis 的使用场景 | 缓存是 Redis 的典型使用场景 |
| 如何保证高并发下的数据一致性 | 缓存一致性是高并发数据一致性的重要部分 |
