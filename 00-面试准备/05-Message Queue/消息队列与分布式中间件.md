...
1. **技术选型**：排除了 Kafka（事务消息支持弱）和 RabbitMQ（吞吐量不够），最终选择 RocketMQ

1. **落地效果**：日均 500 万订单，消息延迟控制在 100ms 以内，通过事务消息保障了订单与库存的一致性
...
<td>
<code>log.flush.interval.messages=1</code>
</td>
<td>
<code>flushDiskType=SYNC_FLUSH</code>
</td>
<td>
保证消息落盘
</td>
...
<td>
<code>log.flush.interval.messages=10000</code>
</td>
<td>
<code>flushDiskType=ASYNC_FLUSH</code>
</td>
<td>
性能优先
</td>
...
<td>
<code>brokerRole=SYNC_MASTER</code>
</td>
<td>
保证副本一致性
</td>
...
<td>
详细说明 18 级别机制、SCHEDULE_TOPIC、定时扫描流程
</td>
</tr>
<tr>
<td>
...
- **Leader 选举**：
  ...
  1. 收到多数节点投票（N/2 + 1）则成为 Leader

  1. Leader 定期发送心跳维持统治地位
...
1. 从 ISR 中选择 LEO 最高的副本作为新 Leader
...
<td>
全面理解 Kafka 和 RocketMQ 的高可用架构设计
</td>
</tr>
<tr>
<td>
...
<td>
CommitLog 统一存储 + ConsumeQueue 索引
</td>
</tr>
<tr>
<td>
...
<td>
ZooKeeper/内部 <em>_consumer</em>offsets
</td>
<td>
ConsumeQueue 索引
</td>
...
<td>
知道会重平衡，不清楚 Eager vs Cooperative
</td>
<td>
理解 Stop the World vs 增量式重平衡
</td>
</tr>
<tr>
<td>
...
<td>
理解 TransactionCoordinator 的角色和完整流程
</td>
</tr>
<tr>
<td>
...
<td>
理解 consume-transform-produce 的完整链路
</td>
</tr>
<tr>
<td>
...
<td>
需要两次 IO（ConsumeQueue → CommitLog）
</td>
<td>
一次 IO 直接读分区文件
</td>
...
<td>
少（1 个 CommitLog + N 个 ConsumeQueue）
</td>
<td>
多（每个分区多个日志文件）
</td>
...
<td>
Broker 端 hashcode 初筛 + Consumer 端精确匹配
</td>
<td>
Broker 端完全过滤
</td>
...
<td>
深入分析 Tag hashcode 初筛、SQL92 表达式引擎
</td>
</tr>
<tr>
<td>
...
<td>
深入分析 Kafka 单线程 poll + 多线程处理、RocketMQ 线程池消费
</td>
</tr>
<tr>
<td>
...
<td>
完整优化方案（4 步）、监控指标、10 分钟 → 30 秒优化案例
</td>
</tr>
</table>
...
1. 库存服务消费 `OrderCreated` → 扣减库存 → 发布 `InventoryReserved` 事件

1. 支付服务消费 `InventoryReserved` → 扣款 → 发布 `PaymentCompleted` 事件

1. 物流服务消费 `PaymentCompleted` → 创建物流单 → 发布 `ShipmentCreated` 事件

**补偿流程**（任一步骤失败）：

1. 支付失败 → 发布 `PaymentFailed` 事件

1. 库存服务消费 `PaymentFailed` → 回滚库存 → 发布 `InventoryReleased` 事件

1. 订单服务消费 `InventoryReleased` → 取消订单 → 发布 `OrderCancelled` 事件
...
1. 解析 binlog，提取数据变更（INSERT/UPDATE/DELETE）
...
1. **消息去重**：使用 Redis Set 记录已删除的缓存 key，避免重复删除

1. **监控告警**：监控 Canal 消费延迟、MQ 消费延迟、缓存命中率

1. **降级策略**：Canal 或 MQ 故障时，降级为定时任务刷新缓存
...
<td>
RocketMQ Namespace + Kafka 多集群双方案
</td>
</tr>
<tr>
<td>
...
<td>
给出 Kafka SASL+ACL 和 RocketMQ ACL 的完整配置示例
</td>
</tr>
<tr>
<td>
...
<td>
Kafka Quota + RocketMQ 流控的详细配置和代码
</td>
</tr>
<tr>
<td>
...
<tr>
<td>
<strong>Kafka 迁移</strong>
</td>
<td>
简单提 reassign 工具
</td>
<td>
给出完整的迁移计划生成、执行、验证、限速命令
</td>
</tr>
<tr>
<td>
<strong>Kafka 扩容</strong>
</td>
<td>
提加机器、重分配
</td>
<td>
完整的扩容流程：新增 Broker、生成计划、执行、验证
</td>
</tr>
<tr>
<td>
<strong>RocketMQ 迁移</strong>
</td>
<td>
提双写双读
</td>
...
<tr>
<td>
<strong>实战 SOP</strong>
</td>
<td>
无
</td>
<td>
从准备到清理的完整 5 阶段迁移 SOP
</td>
</tr>
</table>
...
<tr>
<td>
<strong>Broker 排查</strong>
</td>
<td>
提落盘、磁盘故障
</td>
<td>
消息落盘检查 + 副本同步检查 + 磁盘故障检查的命令
</td>
</tr>
<tr>
<td>
<strong>Consumer 排查</strong>
</td>
<td>
提 offset 提交时机
</td>
<td>
错误示例 + 正确示例 + 消费日志 + Rebalance 监听
</td>
</tr>
<tr>
<td>
...
<tr>
<td>
<strong>消费倾斜表现</strong>
</td>
<td>
简单提某个分区 lag 高
</td>
<td>
完整的监控脚本 + lag 计算逻辑 + 告警机制
</td>
</tr>
<tr>
<td>
...
<td>
完整的 MainConsumer + LocalConsumer 架构
</td>
</tr>
<tr>
<td>
...
| --- | --- |
| 消息顺序性与分区策略 | 消息顺序性通常靠同一业务 key 路由到同一分区或队列，并由单消费者串行处理来保证局部有序；如果跨分区就无法全局有序，需要业务按 key 分片、幂等处理和乱序补偿。 |
| 问题 1：如何保证消息的顺序性？ | 消息顺序性通常靠同一业务 key 路由到同一分区或队列，并由单消费者串行处理来保证局部有序；如果跨分区就无法全局有序，需要业务按 key 分片、幂等处理和乱序补偿。 |
| 死信队列与重试机制 | 消费失败先区分临时错误和永久错误：临时错误走有限次数重试和指数退避，超过阈值进入死信队列；永久错误直接死信并告警，后续通过人工修复、补偿任务或回放机制恢复。 |
| 问题 2：消息消费失败怎么处理？死信队列和重试机制怎么用？ | 消费失败先区分临时错误和永久错误：临时错误走有限次数重试和指数退避，超过阈值进入死信队列；永久错误直接死信并告警，后续通过人工修复、补偿任务或回放机制恢复。 |
| 消息积压排查与优化 | 消息积压先看生产速率、消费速率、消费者异常和下游耗时，定位是生产突增、消费慢还是下游瓶颈；优化手段包括扩消费者、增加分区、批量消费、优化慢逻辑、限流降级和临时旁路处理。 |
| 问题 3：线上消息积压了，怎么排查和优化？ | 消息积压先看生产速率、消费速率、消费者异常和下游耗时，定位是生产突增、消费慢还是下游瓶颈；优化手段包括扩消费者、增加分区、批量消费、优化慢逻辑、限流降级和临时旁路处理。 |
| 问题 4：Kafka 消费者组重平衡是什么？如何避免频繁重平衡？ | Rebalance 是消费者组成员或分区变化后重新分配分区的过程，会暂停消费并造成延迟抖动；避免频繁重平衡要稳定心跳、调大合理超时、减少消费者频繁上下线、使用静态成员和 cooperative sticky 分配策略。 |
| 消息追踪与可观测性 | 消息追踪要把 traceId/messageId 贯穿生产、Broker、消费和下游调用，记录 topic、partition、offset、重试次数、消费延迟和处理结果；可观测指标重点看积压量、端到端延迟、失败率、死信数和消费吞吐。 |
| 问题 5：如何构建消息系统的可观测性？消息追踪怎么做？ | 消息系统可观测性要记录 messageId、traceId、topic、partition、offset、生产时间、消费时间、重试次数和处理结果；通过 Trace 串起生产、Broker、消费和下游调用，并用积压量、消费延迟、失败率和死信数做告警。 |
| 消息队列选型对比 | 深入原理分析（零拷贝、副本机制、事务消息）；5 维度框架 + 场景化建议 + 实战案例；需求分析 → 技术对比 → 落地效果的完整决策链路。 |
| 问题 6：Kafka、RocketMQ、RabbitMQ 怎么选？各自适用什么场景？ | 深入原理分析（零拷贝、副本机制、事务消息）；5 维度框架 + 场景化建议 + 实战案例；需求分析 → 技术对比 → 落地效果的完整决策链路。 |
| 问题 8：如何保证消息不丢失？从生产端、Broker、消费端三个环节分析 | 消息不丢失要分三段保证：生产端同步发送、确认和重试；Broker 端持久化、复制和足够副本确认；消费端处理成功后再提交 offset，并配合幂等消费、失败重试、死信队列和监控补偿。 |
| 问题 9：消费者如何保证幂等？有哪些常见方案？ | 消费者幂等的核心是让同一消息重复投递只生效一次，常用消息唯一 ID、业务唯一键、数据库唯一索引、去重表、Redis setnx、状态机幂等和乐观锁；写操作要先判重再落库，外部调用要带幂等键。 |
| 问题 10：延迟消息的实现原理？RocketMQ 和 Kafka 分别怎么做？ | 详细说明 18 级别机制、SCHEDULE_TOPIC、定时扫描流程；提供 3 种替代方案 + 代码示例 + 优缺点分析；完整的订单超时关闭方案（生产者 + 消费者 + 优化点）。 |
| 事务消息与最终一致性 | 事务消息用于解决本地事务和消息发送的一致性，典型流程是发送半消息、执行本地事务、提交或回滚消息，异常时由 Broker 回查事务状态；消费端必须幂等，并通过补偿和对账保证最终一致。 |
| 问题 11：RocketMQ 事务消息的原理？如何实现分布式事务的最终一致性？ | RocketMQ 事务消息通过半消息、执行本地事务、提交或回滚消息、Broker 事务回查来保证本地事务和消息投递最终一致；关键是本地事务状态可查询、消费端幂等、异常时可补偿。 |
| 消息队列的高可用架构 | 深入剖析 ISR、Dledger、Raft 协议、选举策略；全面理解 Kafka 和 RocketMQ 的高可用架构设计。 |
| 问题 12：Kafka/RocketMQ 如何保证高可用？主从同步、ISR 机制、选举策略是怎样的？ | 深入剖析 ISR、Dledger、Raft 协议、选举策略；全面理解 Kafka 和 RocketMQ 的高可用架构设计。 |
| 问题 13：消息队列在微服务架构中承担什么角色？有哪些典型应用场景？ | 事件驱动架构设计原则 + CQRS + Saga 模式；完整的订单编排流程 + 代码 + 优化点；MQ + Redis + ES + 事务消息的整合方案。 |
| 问题 15：Kafka 的零拷贝、顺序写、Page Cache 是如何协作实现高吞吐的？ | 深入理解顺序写快 100 倍的原因、DMA 传输原理；能画出 4 次 vs 2 次拷贝的完整流程图；理解刷盘策略、Page Cache、文件系统的综合调优。 |
| Kafka 分区分配策略 | 知道会重平衡，不清楚 Eager vs Cooperative；理解 Stop the World vs 增量式重平衡。 |
| 问题 16：Kafka 消费者的分区分配策略有哪些？Range、RoundRobin、Sticky、CooperativeSticky 有什么区别？ | 知道会重平衡，不清楚 Eager vs Cooperative；理解 Stop the World vs 增量式重平衡。 |
| 问题 17：Kafka 的 Exactly-Once 语义是如何实现的？幂等生产者和事务的底层原理？ | 深入理解 Sequence Number 的检测逻辑和限制；理解 TransactionCoordinator 的角色和完整流程；理解 consume-transform-produce 的完整链路。 |
| RocketMQ 存储模型深入 | 所有 Topic 的消息都顺序写入同一个 CommitLog 文件；单文件固定 1GB，写满后滚动创建新文件；顺序写入，磁盘 IO 性能最优，写入吞吐高；每个 Topic 的每个 Queue 对应一个 ConsumeQueue；每条记录固定 20 字节：8 字节 CommitLog offset + 4 字节消息大小 + 8 字节 Tag hashcode。 |
| 问题 18：RocketMQ 的 CommitLog + ConsumeQueue + IndexFile 是如何协作的？与 Kafka 存储模型有什么区别？ | 所有 Topic 的消息都顺序写入同一个 CommitLog 文件；单文件固定 1GB，写满后滚动创建新文件；顺序写入，磁盘 IO 性能最优，写入吞吐高；每个 Topic 的每个 Queue 对应一个 ConsumeQueue；每条记录固定 20 字节：8 字节 CommitLog offset + 4 字节消息大小 + 8 字节 Tag hashcode。 |
| RocketMQ 消息过滤机制 | ConsumeQueue 每条记录存储 8 字节 Tag hashcode；Consumer 拉取消息时，Broker 用 Tag hashcode 快速比对；Hashcode 冲突概率低（使用 MurmurHash），误判率极低；消息实际拉取到 Consumer 后，再比对完整 Tag 字符串；Consumer 订阅时发送 SQL92 表达式，如 TAGS is not null and TAGS in ('TagA', 'TagB')。 |
| 问题 19：RocketMQ 的 Tag 过滤和 SQL92 过滤的原理与性能差异？ | ConsumeQueue 每条记录存储 8 字节 Tag hashcode；Consumer 拉取消息时，Broker 用 Tag hashcode 快速比对；Hashcode 冲突概率低（使用 MurmurHash），误判率极低；消息实际拉取到 Consumer 后，再比对完整 Tag 字符串；Consumer 订阅时发送 SQL92 表达式，如 TAGS is not null and TAGS in ('TagA', 'TagB')。 |
| 问题 20：Kafka/RocketMQ 的生产端如何调优？batch、压缩、acks、linger.ms 等参数怎么配？ | linger.ms 控制等待时间，batch.size 控制批次大小；两者任一达到阈值即发送（先到先发）；高吞吐场景：linger.ms=50-100ms，batch.size=1MB；低延迟场景：linger.ms=0-10ms，batch.size=32KB；日志采集场景：使用 Zstd，压缩率高，节省带宽。 |
| 消费端性能瓶颈排查与优化 | Kafka：单线程 poll，避免并发提交 offset 问题，处理逻辑需要多线程；RocketMQ：线程池消费，框架自动管理并发和 offset 提交；单条消费：1000 条消息 = 1000 次网络 IO + 1000 次事务提交；批量消费（100 条/批次）：1000 条消息 = 10 次网络 IO + 10 次事务提交；消费速度提升 5-10 倍（无需等待 DB 写入）。 |
| 问题 21：消费端处理速度跟不上怎么办？如何定位和优化消费者的性能瓶颈？ | Kafka：单线程 poll，避免并发提交 offset 问题，处理逻辑需要多线程；RocketMQ：线程池消费，框架自动管理并发和 offset 提交；单条消费：1000 条消息 = 1000 次网络 IO + 1000 次事务提交；批量消费（100 条/批次）：1000 条消息 = 10 次网络 IO + 10 次事务提交；消费速度提升 5-10 倍（无需等待 DB 写入）。 |
| Saga 分布式事务编排 | 由一个中心协调器（Saga Coordinator）控制整个流程；协调器调用各个服务的正向操作和补偿操作；优点：流程清晰、易于监控、补偿逻辑集中；缺点：协调器单点、耦合度高；没有中心协调器，各服务通过事件驱动协作。 |
| 问题 22：消息队列如何实现 Saga 分布式事务编排？与 TCC、本地消息表方案有什么区别？ | 由一个中心协调器（Saga Coordinator）控制整个流程；协调器调用各个服务的正向操作和补偿操作；优点：流程清晰、易于监控、补偿逻辑集中；缺点：协调器单点、耦合度高；没有中心协调器，各服务通过事件驱动协作。 |
| 消息队列实现缓存一致性 | 步骤 1 成功，步骤 2 失败 → 缓存是旧数据；并发场景：线程 A 更新 DB（未删缓存），线程 B 读取缓存（旧数据）→ 不一致；步骤 1 成功，步骤 2 失败 → 缓存为空，DB 未更新 → 下次读取缓存空，加载 DB 新数据（可接受）；并发场景：线程 A 删缓存，线程 B 读缓存（空）→ 加载 DB 旧数据，线程 A 更新 DB → 缓存是旧数据；删除缓存失败 → 缓存永久不一致。 |
| 问题 23：如何用消息队列实现分布式缓存更新的最终一致性？Canal + MQ 方案怎么做？ | 步骤 1 成功，步骤 2 失败 → 缓存是旧数据；并发场景：线程 A 更新 DB（未删缓存），线程 B 读取缓存（旧数据）→ 不一致；步骤 1 成功，步骤 2 失败 → 缓存为空，DB 未更新 → 下次读取缓存空，加载 DB 新数据（可接受）；并发场景：线程 A 删缓存，线程 B 读缓存（空）→ 加载 DB 旧数据，线程 A 更新 DB → 缓存是旧数据；删除缓存失败 → 缓存永久不一致。 |
| 问题 24：消息队列的多租户隔离与流量治理怎么做？Topic 命名规范、限流、权限控制？ | 给出完整的命名格式和示例，涵盖环境、业务线、功能、版本；RocketMQ Namespace + Kafka 多集群双方案；给出 Kafka SASL+ACL 和 RocketMQ ACL 的完整配置示例。 |
| 问题 25：消息队列如何做到不停机平滑迁移和集群扩容？ | 给出完整的迁移计划生成、执行、验证、限速命令；完整的扩容流程：新增 Broker、生成计划、执行、验证；消息对账 + offset 校验的完整代码实现。 |
| 问题 26：生产环境消息丢了，怎么快速定位是哪个环节丢的？ | 简单提 Producer/Broker/Consumer；完整的发送回调 + 重试策略 + 排查 SQL；消息落盘检查 + 副本同步检查 + 磁盘故障检查的命令。 |
| 问题 27：消息队列的消费倾斜（数据热点）问题怎么解决？某个分区消费特别慢怎么办？ | 完整的监控脚本 + lag 计算逻辑 + 告警机制；key 分布分析脚本 + 消费者耗时监控；完整的 HotKeyPartitioner 实现 + 配置示例。 |
